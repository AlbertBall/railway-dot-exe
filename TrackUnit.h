...
//functions defined in .cpp file
    ...
    ///
    /// 'variable' indicates that the element may be named and if so may be
    /// plotted striped or solid depending on whether the name has been set
...
//externally defined functions
    ...
    /// Used in SearchForAndUpdateLocationName to check for adjacent named elements to a given element at HLoc & VLoc with a specific SpeedTag,
    ...
    /// True for a blank (SpeedTag == 0) element at a specific Trackvector position,
    ...
    /// Attempt to link the track and return true if successful, if unsuccessful return error flag
    /// and position of the first element that can't be linked together with an appropriate message.
    ///
    /// This is a link checking (FinalCall false) or linking (FinalCall true) function, with messages, called by
    ...
    ///
    ///  This is also a link checking (FinalCall false) or linking (FinalCall true) function, without messages,  that is
    ...
    /// a train split
    ///
    /// Only one length is needed to return true, but this
    ...
    /// Check whether there is a platform present at HLoc & VLoc at the same side as the signal represented by SpeedTag,
    /// if so return true, and also return a pointer to the appropriate platform graphic
    /// (same as a normal platform graphic but with a bit cut out for the signal)
    bool PlatformOnSignalSide(int Caller, int HLoc, int VLoc, int SpeedTag, Graphics::TBitmap* &SignalPlatformGraphic);
    /// When track is being built it is entered into the TrackVector in the order in which it is
    ...
    /// Called by RepositionAndMapTrack to reset the connecting elements of all set gaps (their
    ...
    /// Return a reference to the inactive track element pointed to by NextTrackElementPtr
    /// (during zoomed-in or out track rebuilding, or writing image files), return true if there is a next one or false if not
    bool ReturnNextInactiveTrackElement(int Caller, TTrackElement &Next);
    /// Return a reference to the active track element pointed to by NextTrackElementPtr
    /// (during zoomed-in or out track rebuilding, or writing image files), return true if there is a next one or false if not
    ...
    /// if TrackElement isn't a named element just return its normal graphic
    Graphics::TBitmap *RetrieveStripedNamedLocationGraphicsWhereRelevant(int Caller, TTrackElement TrackElement);
    /// Return the link array position for the element at StartTVPosition that gives the closest link to the element a
    /// EndTVPosition. NB the StartTVPosition is expected to be a single track element as only positions 0 & 1 are checked
    ...
    /// if the element is points and they are set against the exit then &Derail is returned true
    ...
    /// Searches LocationNameMultiMap to check if the element pointed to by the TTrackVectorIterator has the name LocationName.
    ///
    /// If it finds it the pointer TLocationNameMultiMapIterator is returned.  If it fails ErrorString is set to an appropriate text to allow
    ...
    /// Examine TrackVector, InactiveTrackVector and TextVector, and set the values that indicate the
    ...
    /// Changes the LocationName in the name multimap to NewName at the location pointed to by the TLocationNameMultiMapIterator from whatever it
    ...
    /// After an element has been erased from the TrackVector, all the later elements are moved down one.  This function decrements the position values for all values above
    ...
    /// With large railways only part of the railway is displayed on screen, and this function converts true (relative to the whole railway) H & V positions
    ...
    /// Converse of GetScreenPositionsFromTruePos except that in this function HLoc & VLoc are expressed in track elements (i.e. 16x16 pixel squares)
    ...
    /// Plot points on screen according to how they are set (Attribute value), or, with both fillets if BothFillets is true
    /// (the fillet is the bit that appears to move when points are changed)
    ...
    /// that is waiting to have its matching gap selected (see also ShowSelectedGap)
...
enum TTruncateReturnType {NotInRoute, InRouteTrue, InRouteFalse}; ///< a flag used during route truncation to indicate the nature of the
                                                                  ///< selected element, it could be not in a route (NotInRoute), in a route
                                                                  ///< but invalid (InRouteFalse), or in a route and valid (InRouteTrue)
...
/// A descendent of TOnePrefDir used for routes.  Used during contruction of a route (ConstructRoute)
