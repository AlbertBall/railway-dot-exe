...
TTrain::TTrain(int Caller, int RearStartElementIn, int RearStartExitPosIn,
    AnsiString InputCode, int StartSpeedIn, int MassIn,
    double MaxRunningSpeedIn, double MaxBrakeRateIn, double PowerAtRailIn,
    TTrainMode TrainModeIn, TTrainDataEntry *TrainDataEntryPtrIn,
    int RepeatNumberIn, int IncrementalMinutesIn, int IncrementalDigitsIn,
    int SignallerMaxSpeedIn) : RearStartElement(RearStartElementIn),
    RearStartExitPos(RearStartExitPosIn), HeadCode(InputCode),
    StartSpeed(StartSpeedIn), Mass(MassIn), MaxRunningSpeed(MaxRunningSpeedIn),
    MaxBrakeRate(MaxBrakeRateIn), PowerAtRail(PowerAtRailIn),
    TrainMode(TrainModeIn), TrainDataEntryPtr(TrainDataEntryPtrIn),
    RepeatNumber(RepeatNumberIn), IncrementalMinutes(IncrementalMinutesIn),
    IncrementalDigits(IncrementalDigitsIn),
    SignallerMaxSpeed(SignallerMaxSpeedIn)
    /*
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",TTrain," + AnsiString(RearStartElementIn) + "," +
        AnsiString(RearStartExitPosIn) + "," + AnsiString(InputCode) + "," +
        AnsiString(StartSpeedIn) + "," + AnsiString(MassIn) + "," +
...
void TTrain::DeleteTrain(int Caller)
    ...
    TrainController->LogEvent("" + AnsiString(Caller) + ",DeleteTrain," +
        HeadCode);
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",DeleteTrain," + HeadCode);
    ...
    for (int x = 0; x < 4; x++) {
        if (BackgroundPtr[x] == 0) {
            throw Exception("Error in attempting to delete BackgroundPtr[" +
                AnsiString(x) + "]");
        }
    ...
    for (int x = 0; x < 4; x++) {
        if (HeadCodeGrPtr[x] == 0) {
            throw Exception("Error in attempting to delete HeadCodeGrPtr[" +
                AnsiString(x) + "]");
        }
...
void TTrain::PlotStartPosition(int Caller)
     ...
     */ {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",PlotStartPosition," + HeadCode);
    ...
    if (Track->TrackElementAt(163, RearStartElement).TrackType == Continuation)
    {
        ...
        if (ExitSpeedFull < EntrySpeed) {
            double TempEntrySpeed =
                sqrt((MaxExitSpeed * MaxExitSpeed) +
                (3.6 * 3.6 * MaxBrakeRate * ElementLength)); // half braking
    ...
    else // not starting at a continuation
    {
        ...
        MidEntryPos = Track->GetAnyElementOppositeLinkPos(0, MidElement,
            MidExitPos, Derailed);
        LeadElement = Track->TrackElementAt(166, MidElement).Conn[MidExitPos];
        LeadEntryPos = Track->TrackElementAt(167, MidElement)
            .ConnLinkPos[MidExitPos];
        LeadExitPos = Track->GetAnyElementOppositeLinkPos(1, LeadElement,
            LeadEntryPos, Derailed);

        EntrySpeed = StartSpeed;
        MaxExitSpeed = StartSpeed; // initial value
        EntryTime = TrainController->TTClockTime;
        bool TempDerail = false; // dummy
        NextElementPosition = Track->TrackElementAt(168, LeadElement)
            .Conn[Track->GetAnyElementOppositeLinkPos(2, LeadElement,
            LeadEntryPos, TempDerail)];
        NextEntryPos = Track->TrackElementAt(169, LeadElement)
            .ConnLinkPos[Track->GetAnyElementOppositeLinkPos(3, LeadElement,
            LeadEntryPos, TempDerail)];
        ...
        if ((Track->TrackElementAt(492, LeadElement).TrackType == Buffers) &&
            ...
            && !StoppedWithoutPower) {
            FrontElementSpeedLimit = Track->TrackElementAt(494, LeadElement)
                .SpeedLimit01; // use 01 for convenience, not used
            FrontElementLength = Track->TrackElementAt(495, LeadElement)
                .Length01; // use 01 for convenience, not used
            EntrySpeed = 0;
            ExitSpeedHalf = 0;
            ExitSpeedFull = 0;
            MaxExitSpeed = 0;
        ...
        else if ((Track->TrackElementAt(513,
            LeadElement).TrackType == Continuation) &&
            (Track->TrackElementAt(514,
            LeadElement).Config[LeadExitPos] == End)) {
            FrontElementSpeedLimit = Track->TrackElementAt(509, LeadElement)
                .SpeedLimit01; // use 01 for convenience, not used
            FrontElementLength = Track->TrackElementAt(510, LeadElement)
                .Length01; // use 01 for convenience, not used
            ExitSpeedHalf = EntrySpeed;
            ExitSpeedFull = EntrySpeed;
            MaxExitSpeed = EntrySpeed;
            BrakeRate = 0;
            ExitTimeHalf = TrainController->TTClockTime +
                TDateTime(1.8 * (double)
                FrontElementLength / EntrySpeed / 86400);
            ExitTimeFull = TrainController->TTClockTime +
                TDateTime(3.6 * (double)
                FrontElementLength / EntrySpeed / 86400);
        }
        ...
        {
            if ((Track->TrackElementAt(170,
                NextElementPosition).Config[Track->GetNonPointsOppositeLinkPos
                (NextEntryPos)] == Signal) && (Track->TrackElementAt(171,
                NextElementPosition).Attribute == 0) && !StoppedWithoutPower) {
                FrontElementSpeedLimit = Track->TrackElementAt(172, LeadElement)
                    .SpeedLimit01; // use 01 for convenience, not used
                FrontElementLength = Track->TrackElementAt(173, LeadElement)
                    .Length01; // use 01 for convenience, not used
                EntrySpeed = 0;
                ExitSpeedHalf = 0;
                ExitSpeedFull = 0;
                ...
                // moved from inside if(!StoppedAtLocation) at v2.4.0 to ensure this flag set regardless of location stop
                if (!StoppedAtLocation)
                    // stopped at location takes precedence wrt background colour
                {
                    PlotTrainWithNewBackgroundColour(33, clSignalStopBackground,
                        Display);
                ...
                if (NextEntryPos > 1) {
                    ElementLength =
                        Track->TrackElementAt(174, NextElementPosition)
                        .Length23;
                    SpeedLimit = Track->TrackElementAt(175, NextElementPosition)
                        .SpeedLimit23;
                }
                else {
                    ElementLength =
                        Track->TrackElementAt(176, NextElementPosition)
                        .Length01;
                    SpeedLimit = Track->TrackElementAt(177, NextElementPosition)
                        .SpeedLimit01;
                }
                if (EntrySpeed > SpeedLimit)
                    EntrySpeed = SpeedLimit;
                ...
                AnsiString TimeString =
                    Utilities->Format96HHMMSS(TestTime); // test
                FirstHalfMove = true;
                ...
                if (ExitSpeedFull < EntrySpeed) {
                    double TempEntrySpeed =
                        sqrt((MaxExitSpeed * MaxExitSpeed) +
                        (3.6 * 3.6 * MaxBrakeRate * ElementLength));
                    // half braking
                    if (TempEntrySpeed < EntrySpeed) {
                        EntrySpeed = TempEntrySpeed;
                        SetTrainMovementValues(3, NextElementPosition,
                            NextEntryPos);
                    }
                }
            }
        }
        if (Track->TrackElementAt(178, LeadElement).Config[LeadExitPos]
            == NotSet) {
    ...
    {
        Straddle = LeadMid;
        GetOffsetValues(1, HOffset[0], VOffset[0], Track->TrackElementAt(179,
            LeadElement).Link[LeadExitPos]);
        GetOffsetValues(2, HOffset[1], VOffset[1], Track->TrackElementAt(180,
            LeadElement).Link[LeadEntryPos]);
        if (LowEntryValue(Track->TrackElementAt(181,
            LeadElement).Link[LeadEntryPos])) {
            for (int x = 0; x < 4; x++) {
        ...
        if (TrainMode == Timetable)
            RailGraphics->ChangeForegroundColour(19, HeadCodePosition[0],
            FrontCodePtr, clFrontCodeTimetable, BackgroundColour);
        else
            RailGraphics->ChangeForegroundColour(20, HeadCodePosition[0],
            FrontCodePtr, clFrontCodeSignaller, BackgroundColour);
        HeadCodePosition[0] = FrontCodePtr;
        // pick up background bitmaps [0] & [1] & plot HeadCodes [0] & [1]

        PickUpBackgroundBitmap(0, HOffset[0], VOffset[0], LeadElement,
            LeadEntryPos, BackgroundPtr[0]);
        PickUpBackgroundBitmap(1, HOffset[1], VOffset[1], LeadElement,
            LeadEntryPos, BackgroundPtr[1]);
        ...
        GetOffsetValues(3, HOffset[2], VOffset[2], Track->TrackElementAt(184,
            MidElement).Link[MidExitPos]);
        GetOffsetValues(4, HOffset[3], VOffset[3], Track->TrackElementAt(185,
            MidElement).Link[MidEntryPos]);

        // pick up background bitmaps [2] & [3]

        PickUpBackgroundBitmap(2, HOffset[2], VOffset[2], MidElement,
            MidEntryPos, BackgroundPtr[2]);
        PickUpBackgroundBitmap(3, HOffset[3], VOffset[3], MidElement,
            MidEntryPos, BackgroundPtr[3]);
...
void TTrain::UnplotTrain(int Caller) {
    ...
    if (!Plotted)
        return;
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",UnplotTrain," + HeadCode);

    if (Straddle == MidLag) {
        if (MidElement > -1) {
            PlotBackgroundGraphic(0, 0, Display);
            PlotBackgroundGraphic(1, 1, Display);
            PlotAlternativeTrackRouteGraphic(2, MidElement, MidEntryPos,
                HOffset[0], VOffset[0], MidLag);
            PlotAlternativeTrackRouteGraphic(3, MidElement, MidEntryPos,
                HOffset[1], VOffset[1], LeadMidLag);
        ...
        if (LagElement > -1) {
            ...
            PlotAlternativeTrackRouteGraphic(4, LagElement, LagEntryPos,
                HOffset[2], VOffset[2], MidLag);
            PlotAlternativeTrackRouteGraphic(5, LagElement, LagEntryPos,
                HOffset[3], VOffset[3], LeadMidLag);
            // to force plot of locked route marker, needed once only for the element
        }
    }
        ...
        if (LeadElement > -1) {
            ...
            PlotAlternativeTrackRouteGraphic(6, LeadElement, LeadEntryPos,
                HOffset[0], VOffset[0], LeadMidLag);
            // to force plot of locked route marker, needed once only for the element
        }
        if (MidElement > -1) {
            PlotBackgroundGraphic(5, 1, Display);
            PlotBackgroundGraphic(6, 2, Display);
            PlotAlternativeTrackRouteGraphic(7, MidElement, MidEntryPos,
                HOffset[1], VOffset[1], MidLag);
            PlotAlternativeTrackRouteGraphic(8, MidElement, MidEntryPos,
                HOffset[2], VOffset[2], LeadMidLag);
            // to force plot of locked route marker, needed once only for the element
        }
        if (LagElement > -1) {
            PlotBackgroundGraphic(7, 3, Display);
            PlotAlternativeTrackRouteGraphic(9, LagElement, LagEntryPos,
                HOffset[3], VOffset[3], LeadMidLag);
            // to force plot of locked route marker, needed once only for the element
        }
    }
        ...
        if (LeadElement > -1) {
            ...
            PlotAlternativeTrackRouteGraphic(10, LeadElement, LeadEntryPos,
                HOffset[0], VOffset[0], MidLag);
            PlotAlternativeTrackRouteGraphic(11, LeadElement, LeadEntryPos,
                HOffset[1], VOffset[1], LeadMidLag);
            // to force plot of locked route marker, needed once only for the element
        }
        if (MidElement > -1) {
            ...
            PlotAlternativeTrackRouteGraphic(12, MidElement, MidEntryPos,
                HOffset[2], VOffset[2], MidLag);
            PlotAlternativeTrackRouteGraphic(13, MidElement, MidEntryPos,
                HOffset[3], VOffset[3], LeadMidLag);
            // to force plot of locked route marker, needed once only for the element
        }
    }
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",UpdateTrain," + HeadCode);
    UpdateCounter++;
    // 100 counts = 5secs (used in splits to prevent too frequent length checks)
    ...
    {
        if (!TrainFailed && !TrainOnContinuation(0) && (RandRange > 0) && (PowerAtRail > 1) && !((TrainMode == Timetable) && TimetableFinished) && !Crashed && !Derailed && !((TrainMode == Signaller) && Stopped()))
        // RandomFailureCounter resets to 0 every 53 secs, if RandRange is 0 then there are no failures
        // don't fail if:   (a) on a continuation (entering or leaving);
        //                  (b) already failed;
...
*/
    ...
    // default values - these needed for route checker below, though not used here
    Graphics::TBitmap *EntryDirectionGraphicPtr =
        RailGraphics->bmTransparentBgnd;

    if (!SignallerStoppingFlag)
        SignallerStopBrakeRate = 0;

    if (Crashed || Derailed) {
        if (Crashed && Track->IsLCAtHV(41, Track->TrackElementAt(875,
            LeadElement).HLoc, Track->TrackElementAt(876, LeadElement).VLoc)) {
            PlotTrain(7, Display);
    ...
    if (!Crashed) {
        if ((Straddle == LeadMid) && (LeadElement > -1) &&
            (Track->TrackElementAt(602, LeadElement).TrackType == Buffers)) {
            if (ExitSpeedFull > 1) {
                Crashed = true;
                RailGraphics->ChangeForegroundColour(24, HeadCodePosition[0],
                    HeadCodePosition[0], clB0G0R0, BackgroundColour);
                PlotTrainWithNewBackgroundColour(40, clCrashedBackground,
                    Display); // red
                TrainController->LogActionError(35, HeadCode, "",
                    FailBufferCrash, Track->TrackElementAt(719,
                    LeadElement).ElementID);
    ...
    if (!Crashed) {
        if ((Straddle == LeadMid) && (LeadElement > -1) && (ExitSpeedFull > 1))
        {
            ...
            if (Track->IsLCAtHV(40, H, V) && !Track->IsLCBarrierDownAtHV(2,
                H, V)) {
                Crashed = true;
                RailGraphics->ChangeForegroundColour(26, HeadCodePosition[0],
                    HeadCodePosition[0], clB0G0R0, BackgroundColour);
                PlotTrainWithNewBackgroundColour(47, clCrashedBackground,
                    Display); // red
                TrainController->LogActionError(54, HeadCode, "",
                    FailLevelCrossingCrash, Track->TrackElementAt(877,
                    LeadElement).ElementID);
                // no need for missed action logs - will be sent when train removed
            }
        }
    ...
    if (StoppedAtBuffers && !BufferZoomOutFlashRequired && !StoppedAtLocation &&
        !Crashed && !Derailed) {
        PlotTrainWithNewBackgroundColour(37, clBufferStopBackground, Display);
    }

    // set or reset HoldAtLocationInTTMode
    if ((TrainMode == Timetable) && StoppedAtLocation &&
        (ActionVectorEntryPtr->Command != ""))
    ...
    if (TrainMode == Timetable) {
        if (!DepartureTimeSet && StoppedAtLocation)
            // && !StoppedAtBuffers) - set times whether or not at buffers
        {
            if (ActionVectorEntryPtr->DepartureTime > TDateTime(-1)) {
                ReleaseTime = TrainController->GetRepeatTime(0,
                    ActionVectorEntryPtr->DepartureTime, RepeatNumber,
                    IncrementalMinutes);
    ...
    if (TrainMode == Timetable) {
        if ((ActionVectorEntryPtr->Command != "Frh") &&
            (ActionVectorEntryPtr->Command != "Frh-sh")) {
            RemainHereLogNotSent = true;
        }
        if (HoldAtLocationInTTMode) {
            // ignore TimeLoc & TTLoc departures
            // Action logs given in functions
            if ((TrainController->TTClockTime >= GetTrainTime(0,
                ActionVectorEntryPtr->EventTime)) &&
                (TrainController->TTClockTime >= LastActionTime +
                TDateTime(30.0 / 86400))) {
            ...
            if (TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
                .EventReported == FailLocTooShort) {
                TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
    ...
    if (TrainMode == Timetable)
    {
        if (StoppedAtBuffers)
        {
            ...
            AnsiString BufferLocation = Track->TrackElementAt(604, LeadElement)
                .ActiveTrackElementName;
            if (BufferLocation == "")
                BufferLocation =
                    Track->TrackElementAt(605, Track->TrackElementAt(606,
                LeadElement).Conn[1]).ActiveTrackElementName;
            ...
            if ((BufferLocation == "") || (BufferLocation != ExpectedLocation))
            {
                ...
                PlotTrainWithNewBackgroundColour(38, clBufferAttentionNeeded,
                    Display);
                if (TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
                    .EventReported != FailUnexpectedBuffers) {
                    TrainController->LogActionError(1, HeadCode, "",
                        FailUnexpectedBuffers, Track->TrackElementAt(603,
                        LeadElement).ElementID);
            ...
            else if ((BufferLocation != "") &&
                (BufferLocation == ExpectedLocation)
                && DepartureTimeSet && !StoppedAtLocation &&
                (TrainController->TTClockTime > ReleaseTime)) {
                BufferZoomOutFlashRequired = true;
                if (TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
                    .EventReported != FailBuffersPreventingStart) {
                    PlotTrainWithNewBackgroundColour(39,
                        clBufferAttentionNeeded, Display);
                    TrainController->LogActionError(2, HeadCode, "",
                        FailBuffersPreventingStart, ExpectedLocation);
                    // SendMissedActionLogs(-1, ActionVectorEntryPtr);//-1 is a marker for send messages for all remaining entries, including Fer if present
                    // Drop missed actions so user can still use sig mode to get back on track
                    TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
                        .EventReported = FailBuffersPreventingStart;
    ...
    if ((NextElementPosition > -1) && (NextEntryPos > -1))
        // may be buffers or continuation
    {
        /*
    ...
    if (TrainMode == Timetable)
    {
        if (StoppedAtLocation && !StoppedAtBuffers && !Crashed && !Derailed && !HoldAtLocationInTTMode && !TrainFailed)
            ...
            if (TrainController->TTClockTime >= ReleaseTime)
            {
                ...
                PlotTrainWithNewBackgroundColour(20, clNormalBackground,
                Display);
                AnsiString StationName;
                if (Track->TrackElementAt(193,
                    LeadElement).ActiveTrackElementName != "")
                {
                    StationName = Track->TrackElementAt(194, LeadElement)
                        .ActiveTrackElementName;
                }
                else if (Track->TrackElementAt(195,
                    MidElement).ActiveTrackElementName != "")
                {
                    StationName = Track->TrackElementAt(196, MidElement)
                        .ActiveTrackElementName;
                }
                else {
                    throw Exception
                ...
                int NextElementPosition =
                    Track->TrackElementAt(199, LeadElement)
                    .Conn[Track->GetNonPointsOppositeLinkPos(LeadEntryPos)];
                int NextEntryPos = Track->TrackElementAt(200, LeadElement)
                    .ConnLinkPos[Track->GetNonPointsOppositeLinkPos
                    (LeadEntryPos)];
                FirstHalfMove = true;
                StoppedAtLocation = false;
                ...
                {
                    if ((Track->TrackElementAt(720,
                        NextElementPosition)
                        .Config[Track->GetNonPointsOppositeLinkPos(NextEntryPos)
                        ] == Signal) && (Track->TrackElementAt(721,
                        NextElementPosition).Attribute == 0)) {
                        StoppedAtSignal = true;
                        if (!StoppedWithoutPower)
                            // if stopped without power just keep existing background colour
                        {
                            PlotTrainWithNewBackgroundColour(41,
                                clSignalStopBackground, Display);
                ...
                if (ActionVectorEntryPtr->FormatType == TimeTimeLoc) {
                    TimeTimeLocArrived = false;
                    LogAction(27, HeadCode, "", Depart, StationName,
                        ActionVectorEntryPtr->DepartureTime, false);
                    // no warning for TimeTimeLoc departure
                }
                else {
                    LogAction(6, HeadCode, "", Depart, StationName,
                        ActionVectorEntryPtr->DepartureTime,
                        ActionVectorEntryPtr->Warning);
                }
                DepartureTimeSet = false;
                 ...
                 */
                if ((Track->TrackElementAt(201,
                    LeadElement).TrackType == Buffers) && !StoppedWithoutPower)
                {
                    StoppedAtBuffers = true;
                }
                ...
                {
                    if (Track->TrackElementAt(724,
                        LeadElement).TrackType != Continuation) {
                        SetTrainMovementValues(12, NextElementPosition,
                            NextEntryPos);
                        // NextElement is the element to be entered
                    }
                    else {
                        SetTrainMovementValues(13, LeadElement,
                            LeadEntryPos);
    ...
    {
        if (LeadElement > -1) {
            int NextPos = Track->TrackElementAt(649, LeadElement)
                .Conn[LeadExitPos];
            int NextEntryPos = Track->TrackElementAt(650, LeadElement)
        ...
        // give message + plot background when ready to move half past the signal
    {
        if (NextElementPosition > -1) {
            if ((Track->TrackElementAt(662,
                NextElementPosition).Config[Track->GetNonPointsOppositeLinkPos
                (NextEntryPos)] == Signal) && (Track->TrackElementAt(663,
                NextElementPosition).Attribute == 0)) {
                AnsiString LocID =
                    AnsiString(Track->TrackElementAt(664, NextElementPosition)
                    .ElementID);
                TrainController->LogActionError(3, HeadCode, "",
                    FailSPAD, LocID);
                // if goes past 2 signals then give message twice
                PlotTrainWithNewBackgroundColour(17, clSPADBackground,
                    Display); // Yellow
            }
        }
    }
    ...
    if (Straddle == LeadMidLag)
        // During this function train moves fully onto 2 elements, Lead & Mid, so set next 2 moves from here
        // for the element after Lead
    {
        ...
        if ((TrainMode == Signaller) && (SignallerStoppingFlag ||
            StepForwardFlag)) {
            if (ExitSpeedFull == 0) {
                ...
                if (Loc == "") {
                    ...
                    Loc = "at " + Loc;
                LogAction(30, HeadCode, "", SignallerStop, Loc,
                    TrainController->TTClockTime, false); // false for warning
            }
        }
        if (LeadElement > -1) // if an exit continuation then not set
        {
            if ((Track->TrackElementAt(202, LeadElement).TrackType != Points) ||
                ((LeadEntryPos != 0) && (LeadEntryPos != 2))) {
                LeadExitPos = Track->GetNonPointsOppositeLinkPos(LeadEntryPos);
            }
            else if ((Track->TrackElementAt(203,
                LeadElement).TrackType == Points) && ((LeadEntryPos == 0) ||
                (LeadEntryPos == 2))) {
            ...
            NextElementPosition = Track->TrackElementAt(205, LeadElement)
                .Conn[LeadExitPos];
            NextEntryPos = Track->TrackElementAt(206, LeadElement)
                .ConnLinkPos[LeadExitPos];
        }
        else {
            NextElementPosition = -1;
        ...
        {
            if ((Track->TrackElementAt(207,
                NextElementPosition).Config[Track->GetNonPointsOppositeLinkPos
                (NextEntryPos)] == Signal) && (Track->TrackElementAt(208,
                NextElementPosition).Attribute == 0) && (ExitSpeedFull < 1)
                && !StoppedAtLocation) {
                StoppedAtSignal = true;
                if (!StoppedWithoutPower)
                    // leave background as is if no power, but set StoppedAtSignal
                {
                    PlotTrainWithNewBackgroundColour(34, clSignalStopBackground,
                        Display);
                }
        ...
        if (!Stopped())
        {
            ...
            else if ((LeadElement > -1) && (Track->TrackElementAt(209,
                LeadElement).TrackType == Continuation)) {
                SetTrainMovementValues(9, LeadElement, LeadEntryPos);
                // Use LeadElement for calcs if lead is a continuation
            }
            else if ((MidElement > -1) && (Track->TrackElementAt(210,
                MidElement).TrackType == Continuation)) {
                SetTrainMovementValues(10, MidElement, MidEntryPos);
                // Use MidElement for calcs if mid is a continuation
            }
            else if ((LagElement > -1) && (Track->TrackElementAt(211,
                LagElement).TrackType == Continuation)) {
    ...
    if (Straddle == LeadMid) {
            ...
            // i.e. next time Straddle reaches LeadMid
        {
            Derailed = true;
            DerailPending = false;
            RailGraphics->ChangeForegroundColour(21, HeadCodePosition[0],
                HeadCodePosition[0], clB0G0R0, BackgroundColour);
            PlotTrainWithNewBackgroundColour(6, clDerailedBackground, Display);
            TrainController->LogActionError(4, HeadCode, "", FailDerailed,
                Track->TrackElementAt(595, LeadElement).ElementID);
        ...
        if (LeadElement > -1) {
                ...
                // if don't exclude entry continuations then can't progress past it
            {
                ...
                CheckAndCancelRouteForWrongEndEntry(2, LeadElement,
                    LeadEntryPos);
                if (Stopped()) {
                    if (TrainFailurePending) // ok, stopped so PlotElements set
                    {
    ...
    { // remove route elements - this section moved from below, was under LagElement > -1 condition but needs to cover LagElement == -1
        if (AllRoutes->GetRouteTypeAndGraphics(2, LeadElement, LeadEntryPos,
            EXGraphicPtr, EntryDirectionGraphicPtr)
            == TAllRoutes::NotAutoSigsRoute) // Trains may not be in a route
            ...
            // element during this function
        {
            ...
            int TempELink = Track->TrackElementAt(215, LeadElement)
                .Link[LeadEntryPos];
            TAllRoutes::TRouteElementPair FirstPair, SecondPair;
            FirstPair = AllRoutes->GetRouteElementDataFromRoute2MultiMap(10,
                TempH, TempV, SecondPair);
            if ((FirstPair.first > -1) && (AllRoutes->GetFixedRouteAt(143,
                FirstPair.first).GetFixedPrefDirElementAt(153,
                FirstPair.second).GetELink() == TempELink)) {
                AllRoutes->RemoveRouteElement(10, TempH, TempV, TempELink);
            }
            else if ((SecondPair.first > -1) && (AllRoutes->GetFixedRouteAt(144,
                SecondPair.first).GetFixedPrefDirElementAt(154,
                SecondPair.second).GetELink() == TempELink)) {
                AllRoutes->RemoveRouteElement(11, TempH, TempV, TempELink);
            }
        }

        if (AllRoutes->GetRouteTypeAndGraphics(3, MidElement, MidEntryPos,
            EXGraphicPtr, EntryDirectionGraphicPtr)
            == TAllRoutes::NotAutoSigsRoute) // Trains may not be in a route
        {
            int TempH = Track->TrackElementAt(216, MidElement).HLoc;
            int TempV = Track->TrackElementAt(217, MidElement).VLoc;
            int TempELink = Track->TrackElementAt(218, MidElement)
                .Link[MidEntryPos];
            TAllRoutes::TRouteElementPair FirstPair, SecondPair;
            FirstPair = AllRoutes->GetRouteElementDataFromRoute2MultiMap(11,
                TempH, TempV, SecondPair);
            if ((FirstPair.first > -1) && (AllRoutes->GetFixedRouteAt(145,
                FirstPair.first).GetFixedPrefDirElementAt(155,
                FirstPair.second).GetELink() == TempELink)) {
                AllRoutes->RemoveRouteElement(12, TempH, TempV, TempELink);
            }
            else if ((SecondPair.first > -1) && (AllRoutes->GetFixedRouteAt(146,
                SecondPair.first).GetFixedPrefDirElementAt(156,
                SecondPair.second).GetELink() == TempELink)) {
                AllRoutes->RemoveRouteElement(13, TempH, TempV, TempELink);
            }
        }

        if (AllRoutes->GetRouteTypeAndGraphics(4, LagElement, LagEntryPos,
            EXGraphicPtr, EntryDirectionGraphicPtr)
            == TAllRoutes::NotAutoSigsRoute) // Trains may not be in a route
        {
            int TempH = Track->TrackElementAt(219, LagElement).HLoc;
            int TempV = Track->TrackElementAt(220, LagElement).VLoc;
            int TempELink = Track->TrackElementAt(221, LagElement)
                .Link[LagEntryPos];
            TAllRoutes::TRouteElementPair FirstPair, SecondPair;
            FirstPair = AllRoutes->GetRouteElementDataFromRoute2MultiMap(12,
                TempH, TempV, SecondPair);
            if ((FirstPair.first > -1) && (AllRoutes->GetFixedRouteAt(147,
                FirstPair.first).GetFixedPrefDirElementAt(157,
                FirstPair.second).GetELink() == TempELink)) {
                AllRoutes->RemoveRouteElement(14, TempH, TempV, TempELink);
            }
            else if ((SecondPair.first > -1) && (AllRoutes->GetFixedRouteAt(148,
                SecondPair.first).GetFixedPrefDirElementAt(158,
                SecondPair.second).GetELink() == TempELink)) {
        ...
        // not entering at a continuation so can deal with train leaving the lag element
    {
            ...
            // below are the actions required only when the train is not straddling 3 elements (will move onto 2 elements if enters with LeadMidLag)
        {
            ...
            if (AllRoutes->
                IsElementInLockedRouteGetPrefDirElementGetLockedVectorNumber(6,
                LagElement, LagExitPos, PrefDirElement, LockedVectorNumber)) {
                Display->PlotOutput(22,
                    (Track->TrackElementAt(222, LagElement).HLoc * 16),
                    (Track->TrackElementAt(223, LagElement).VLoc * 16),
                    RailGraphics->LockedRouteCancelPtr
                    [PrefDirElement.GetELink()]);
            }
            ...
            if (ContinuationExit(2, LagElement, LagExitPos)) {
                ...
                if (AllRoutes->GetRouteTypeAndNumber(10, LagElement, LagExitPos,
                    RouteNumber) == TAllRoutes::AutoSigsRoute) {
                    TTrainController::TContinuationAutoSigEntry
                        ContinuationAutoSigEntry;
                    ContinuationAutoSigEntry.RouteNumber = RouteNumber;
                    // calc distance from & inc last signal to exit
                    int LastElement = LagElement, LastExitPos = LagExitPos,
                        CumDistance = 0;
                    ...
                    while ((Track->TrackElementAt(913,
                        LastElement).Config[LastExitPos] != Signal) &&
                        (CumDistance < 1200) && (Track->TrackElementAt(897,
                        ...
                        // leave CumDistance as it was in these circumstances.
                    {
                        if (LastExitPos < 2)
                            CumDistance +=
                                Track->TrackElementAt(685, LastElement)
                                .Length01;
                        else
                            CumDistance +=
                                Track->TrackElementAt(686, LastElement)
                                .Length23;
                        NewLastElement = Track->TrackElementAt(687, LastElement)
                            .Conn[Track->GetNonPointsOppositeLinkPos
                            (LastExitPos)];
                        if (NewLastElement == -1)
                        ...
                        NewLastExitPos = Track->TrackElementAt(688, LastElement)
                            .ConnLinkPos[Track->GetNonPointsOppositeLinkPos
                            (LastExitPos)];
                        if (NewLastExitPos == -1) {
                            throw Exception
                    ...
                    if (CumDistance < 1200) {
                        CumDistance += Track->TrackElementAt(689, LastElement)
                            .Length01; // only need 01 for signal
                    }
                    ...
                    ContinuationAutoSigEntry.FirstDelay =
                        3.6 * double(FirstDistance) / ExitSpeedFull;
                    // speed in km/h so mult by 3.6 to bring to secs
                    ContinuationAutoSigEntry.SecondDelay =
                        ContinuationAutoSigEntry.FirstDelay +
                        4320.0 / ExitSpeedFull;
                    // 4320.0 = 3.6 * 1200, .0 to make it a double
                    ContinuationAutoSigEntry.ThirdDelay =
                        ContinuationAutoSigEntry.SecondDelay +
                        4320.0 / ExitSpeedFull;
                    ContinuationAutoSigEntry.AccessNumber = 0;
                    ContinuationAutoSigEntry.PassoutTime =
                        TrainController->TTClockTime;
                    if (!TrainController->ContinuationAutoSigVector.empty()) {
                        TTrainController::TContinuationAutoSigVectorIterator
                            VectorIT;
                        for (VectorIT =
                            TrainController->ContinuationAutoSigVector.begin();
                        VectorIT != TrainController->ContinuationAutoSigVector.
                            end(); VectorIT++) {
                    ...
                    TrainController->ContinuationAutoSigVector.push_back
                        (ContinuationAutoSigEntry);
                }
            ...
            if ((LeadElement > -1) && (Track->TrackElementAt(224,
                LeadElement).Config[LeadExitPos] == Signal))
                // changed to lead so reset early
            {
                ...
                if (!Display->ZoomOutFlag)
                    Track->PlotSignal(4, Track->TrackElementAt(226,
                    LeadElement), Display);
                // covers signal resetting in same direction
            }
            if (Track->TrackElementAt(521,
                LagElement).Config[LagExitPos] == Signal) {
                if (AllRoutes->GetRouteTypeAndGraphics(5, LagElement,
                    LagEntryPos, EXGraphicPtr, EntryDirectionGraphicPtr)
                    == TAllRoutes::AutoSigsRoute) {
                    Display->PlotOutput(23, Track->TrackElementAt(227,
                        LagElement).HLoc * 16, Track->TrackElementAt(228,
                        LagElement).VLoc * 16, EXGraphicPtr);
                    Display->PlotOutput(24, Track->TrackElementAt(229,
                        LagElement).HLoc * 16, Track->TrackElementAt(230,
                        LagElement).VLoc * 16, EntryDirectionGraphicPtr);
                    TPrefDirElement PrefDirElement;
                    ...
                    if (AllRoutes->
                        ...
                        8, LagElement, LagExitPos, PrefDirElement,
                        LockedVectorNumber)) {
                        Display->PlotOutput(25,
                            (Track->TrackElementAt(231, LagElement).HLoc * 16),
                            (Track->TrackElementAt(232, LagElement).VLoc * 16),
                            RailGraphics->LockedRouteCancelPtr
                            [PrefDirElement.GetELink()]);
                    }
                    if (!(AllRoutes->
                        IsElementInLockedRouteGetPrefDirElementGetLockedVectorNumber(
                        9, LagElement, LagExitPos, PrefDirElement,
                        LockedVectorNumber))) {
                        AllRoutes->SetTrailingSignalsOnAutoSigsRoute(0,
                            LagElement, LagExitPos);
                    }
                }
            }
            else if ((LeadElement > -1) && (Track->TrackElementAt(233,
                LeadElement).TrackType == SignalPost)) {
                Track->TrackElementAt(234, LeadElement).Attribute = 0; // red
                Track->TrackElementAt(672, LeadElement).CallingOnSet = false;
                // don't plot if zoomed out
                if (!Display->ZoomOutFlag)
                    Track->PlotSignal(5, Track->TrackElementAt(235,
                    LeadElement), Display);
            ...
            else if (Track->TrackElementAt(522,
                LagElement).TrackType == SignalPost) {
                if (AllRoutes->GetRouteTypeAndGraphics(6, LagElement,
                    LagEntryPos, EXGraphicPtr, EntryDirectionGraphicPtr)
                    == TAllRoutes::AutoSigsRoute) {
                    Display->PlotOutput(26, Track->TrackElementAt(236,
                        LagElement).HLoc * 16, Track->TrackElementAt(237,
                        LagElement).VLoc * 16, EXGraphicPtr);
                    Display->PlotOutput(27, Track->TrackElementAt(238,
                        LagElement).HLoc * 16, Track->TrackElementAt(239,
                        LagElement).VLoc * 16, EntryDirectionGraphicPtr);
                    ...
                    int RouteNumber;
                    AllRoutes->GetRouteTypeAndNumber(29, LagElement,
                        LagEntryPos, RouteNumber);
                    ...
                    int RouteNumber2;
                    AllRoutes->GetRouteTypeAndNumber(30, LeadElement,
                        LeadEntryPos, RouteNumber2);
                    // already know it's an autosigsroute, this is just to get the RouteNumber
                    if ((TrainMode == Signaller) &&
                        (RouteNumber2 != RouteNumber))
                        // note that if not in a route (as likely) then RouteNumber2 set to -1          )
                    {
                        AllRoutes->GetFixedRouteAt(217, RouteNumber)
                            .SetRouteSignals(10);
                ...
                if (AllRoutes->
                    IsElementInLockedRouteGetPrefDirElementGetLockedVectorNumber(
                    10, LagElement, LagExitPos, PrefDirElement,
                    LockedVectorNumber)) {
                    Display->PlotOutput(28,
                        (Track->TrackElementAt(240, LagElement).HLoc * 16),
                        (Track->TrackElementAt(241, LagElement).VLoc * 16),
                        RailGraphics->LockedRouteCancelPtr
                        [PrefDirElement.GetELink()]);
                }
            }
        ...
        TPrefDirElement PrefDirElement;
        if (!(AllRoutes->
            IsElementInLockedRouteGetPrefDirElementGetLockedVectorNumber(7,
            LagElement, LagExitPos, PrefDirElement, LockedVectorNumber))) {
            int RouteNumber; // holder for call below - not used
            if (AllRoutes->GetRouteTypeAndNumber(9, LagElement, LagExitPos,
                RouteNumber) == TAllRoutes::NoRoute) {
                if (Utilities->clTransparent == TColor(0xFFFFFF))
                    // change to black for a white background
                {
                    RailGraphics->ChangeSpecificColour(0, BackgroundPtr[3],
                        BackgroundPtr[3], clB5G3R0, clB0G0R0);
                    // only applies for AutoSigs Route,
                }
                else // change to white for a dark background
                {
                    RailGraphics->ChangeSpecificColour(2, BackgroundPtr[3],
                        BackgroundPtr[3], clB5G3R0, clB5G5R5);
                    // only applies for AutoSigs Route,
                }
        ...
        PlotAlternativeTrackRouteGraphic(1, LagElement, LagEntryPos, HOffset[3],
            VOffset[3], Straddle);
    ...
    {
        if (LowEntryValue(Track->TrackElementAt(242,
            LeadElement).Link[LeadEntryPos])) {
            for (int x = 0; x < 4; x++) {
                HeadCodePosition[x] = HeadCodeGrPtr[3 - x];
            }
    ...
    if (TrainMode == Timetable) {
        RailGraphics->ChangeForegroundColour(22, HeadCodePosition[0],
            FrontCodePtr, clFrontCodeTimetable, BackgroundColour); // red
    }
    else {
        RailGraphics->ChangeForegroundColour(23, HeadCodePosition[0],
            FrontCodePtr, clFrontCodeSignaller, BackgroundColour); // blue
    }
    HeadCodePosition[0] = FrontCodePtr;
    ...
    if (LeadElement > -1) {
        ...
        {
            GetOffsetValues(5, HOffset[0], VOffset[0],
                Track->TrackElementAt(243, LeadElement).Link[LeadEntryPos]);
            // pick up new background bitmap [0]
            PickUpBackgroundBitmap(4, HOffset[0], VOffset[0], LeadElement,
                LeadEntryPos, BackgroundPtr[0]);
            int LeadElementTrainID = Track->TrackElementAt(244, LeadElement)
                .TrainIDOnElement;
                ...
                // check if own ID for entry at continuation, else crashes into itself!
            {
                ...
                int OtherTrainEntryPos =
                    TrainController->EntryPos(0, LeadElementTrainID,
                LeadElement);
                if (OtherTrainEntryPos == -1) {
                    throw Exception("Error - OtherTrainEntryPos not set");
                }
                if ((Track->TrackElementAt(246,
                    LeadElement).TrackType != Bridge) ||
                    (LeadEntryPos == OtherTrainEntryPos) ||
                    // LeadEntryPos for rear end crashes
                    (LeadExitPos == OtherTrainEntryPos))
                    // LeadExitPos for head-on crashes
                {
                    TrainCrashedInto = Track->TrackElementAt(247, LeadElement)
                        .TrainIDOnElement;
            ...
            else if (MidElement > -1) // will be -1 for continuation entries
            {
                ...
                int MidExitLinkNum = Track->TrackElementAt(889, MidElement)
                    .Link[MidExitPos];
                int MidHLoc = Track->TrackElementAt(890, MidElement).HLoc;
                int MidVLoc = Track->TrackElementAt(891, MidElement).VLoc;
                int OtherTrainID = -1;
                if ((MidExitLinkNum == 1) || (MidExitLinkNum == 3) ||
                    (MidExitLinkNum == 7) || (MidExitLinkNum == 9)) {
                    if (Track->DiagonalFouledByTrain(0, MidHLoc, MidVLoc,
                        MidExitLinkNum, OtherTrainID)) {
            ...
            GetOffsetValues(6, HOffset[0], VOffset[0],
                Track->TrackElementAt(248, LeadElement).Link[LeadExitPos]);
            // pick up new background bitmap [0]
            PickUpBackgroundBitmap(5, HOffset[0], VOffset[0], LeadElement,
                LeadEntryPos, BackgroundPtr[0]);
        }
        PlotElement[0] = LeadElement;
        PlotEntryPos[0] = LeadEntryPos;
    ...
    {
        RailGraphics->ChangeForegroundColour(0, HeadCodePosition[0],
            HeadCodePosition[0], clB0G0R0, BackgroundColour);
        PlotTrainWithNewBackgroundColour(7, clCrashedBackground,
            Display); // red
        TrainController->TrainVectorAtIdent(5, TrainCrashedInto).Crashed = true;
        TrainController->TrainVectorAtIdent(34, TrainCrashedInto)
            .CallingOnFlag = false;
        // in case was set, need to disable call on if call on button had been pressed
        RailGraphics->ChangeForegroundColour(1,
            TrainController->TrainVectorAtIdent(6, TrainCrashedInto)
            .HeadCodePosition[0], TrainController->TrainVectorAtIdent(7,
            TrainCrashedInto).HeadCodePosition[0], clB0G0R0,
            TrainController->TrainVectorAtIdent(8, TrainCrashedInto)
            .BackgroundColour);
        TrainController->TrainVectorAtIdent(9, TrainCrashedInto)
            .PlotTrainWithNewBackgroundColour(8, clCrashedBackground,
            Display); // red
        TrainController->LogActionError(5, HeadCode,
            TrainController->TrainVectorAtIdent(13, TrainCrashedInto).HeadCode,
            FailCrashed, Track->TrackElementAt(621, LeadElement).ElementID);
        Straddle = LeadMidLag;
    ...
    if (TrainMode == Timetable) {
        if (Straddle == LeadMidLag) {
            if ((LeadElement > -1) && (MidElement > -1) && !TimetableFinished) {
                ...
                AnsiString LocName = Track->TrackElementAt(249, LeadElement)
                    .ActiveTrackElementName;
                ...
                if (TTVPos > -1) // -1 if can't find it or if name is ""
                {
                    ...
                    TTrackElement LeadTrackElement =
                        Track->TrackElementAt(258, LeadElement);
                    TTrackElement NextTrackElement; // default for now
                    bool TrainAtStopLinkPos1 =
                        (LeadTrackElement.StationEntryStopLinkPos1 ==
                        LeadEntryPos);
                    bool TrainAtStopLinkPos2 =
                        (LeadTrackElement.StationEntryStopLinkPos2 ==
                        LeadEntryPos);
                    bool ForwardConnection =
                        (LeadTrackElement.Conn[LeadExitPos] > -1);
                    ...
                    {
                        NextTrackElement =
                            Track->TrackElementAt(262,
                            LeadTrackElement.Conn[LeadExitPos]);
                        NextElementEntryPos =
                            LeadTrackElement.ConnLinkPos[LeadExitPos];
                        NextElementExitPos =
                            Track->GetNonPointsOppositeLinkPos
                            (NextElementEntryPos);
                        // this is only for signals so no need to worry about points ambiguity
                        TrainOnNextElement =
                            (NextTrackElement.TrainIDOnElement > -1);
                        StopSignalAtNextElement =
                            ((NextTrackElement.Config[NextElementExitPos]
                            == Signal) && (NextTrackElement.Attribute == 0));
                    }
                    ...
                    if (TrainAtStopLinkPos1 || TrainAtStopLinkPos2 ||
                        (ForwardConnection && (TrainOnNextElement ||
                        StopSignalAtNextElement))) {
                        if (TTVPos > 0) {
                            SendMissedActionLogs(1, TTVPos,
                            ActionVectorEntryPtr);
                        ...
                        if (StopRequired) {
                            ...
                            // in zoom out mode
                            if (!TrainFailed) {
                                PlotTrainWithNewBackgroundColour(12,
                                    clStationStopBackground, Display);
                                // pale green
                            }
                            LogAction(8, HeadCode, "", Arrive, LocName,
                                ActionVectorEntryPtr->ArrivalTime,
                                ActionVectorEntryPtr->Warning);
                            if (ActionVectorEntryPtr->FormatType == TimeTimeLoc)
                            {
                            ...
                            LogAction(25, HeadCode, "", Pass, LocName,
                                ActionVectorEntryPtr->EventTime,
                                ActionVectorEntryPtr->Warning);
                        }
                        if ((ActionVectorEntryPtr->FormatType == TimeLoc) ||
                            (ActionVectorEntryPtr->FormatType == PassTime)) {
                            ActionVectorEntryPtr++;
                        }
    ...
    {
        throw Exception
            ("Error, Straddle shouldn't be LeadMid prior to resetting at exit from UpdateTrain function"
            );
    }
...
void TTrain::SetHeadCodeGraphics(int Caller, AnsiString Code) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",SetHeadCodeGraphics," + HeadCode);
    if (Code.Length() != 4)
    ...
    {
        for (int x = 0; x < 4; x++) {
            RailGraphics->ChangeBackgroundColour3(0, HeadCodeGrPtr[x],
                HeadCodeGrPtr[x], BackgroundColour, clB5G5R5);
        }
    }
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",GetLeadElement," + HeadCode);
    DerailPending = false;
    LeadElement = Track->TrackElementAt(269, MidElement).Conn[MidExitPos];
    LeadEntryPos = Track->TrackElementAt(270, MidElement)
        .ConnLinkPos[MidExitPos];
    if (Track->TrackElementAt(271, LeadElement).TrackType == Points) {
        ...
        if ((LeadEntryPos == 0) && (Track->TrackElementAt(272,
            LeadElement).Attribute == 0))
            LeadExitPos = 1;
        ...
        else if ((LeadEntryPos == 2) && (Track->TrackElementAt(273,
            LeadElement).Attribute == 0)) {
            LeadEntryPos = 0;
            LeadExitPos = 1;
        }
        ...
        else if ((LeadEntryPos == 1) && (Track->TrackElementAt(274,
            LeadElement).Attribute == 0))
            LeadExitPos = 0;
        ...
        else if ((LeadEntryPos == 3) && (Track->TrackElementAt(275,
            LeadElement).Attribute == 0)) {
            LeadExitPos = 0;
            DerailPending = true;
        }
...
void TTrain::GetOffsetValues(int Caller, int &HOffset, int &VOffset,
    int Link) const {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",GetOffsetValues," + AnsiString(Link) + "," +
        HeadCode);
    switch (Link) {
    case 1: {
            HOffset = 0;
...
bool TTrain::LowEntryValue(int EntryLink) const {
     ...
     */
    if ((EntryLink == 1) || (EntryLink == 2) || (EntryLink == 4) ||
        (EntryLink == 7))
        return true;
    else
        return false;
        ...
        if (FoundFlag)
        {
            if (Track->InactiveTrackElementAt(25, IMPair.first).TrackType == NamedNonStationLocation)
            {
                GraphicPtr->Canvas->CopyRect(DestRect,
                    Track->InactiveTrackElementAt(26,
                    IMPair.first).GraphicPtr->Canvas, SourceRect);
...
//This was an attempt to pick up the actual 8x8 graphic from the display, and overwrite it with the reconstructed track, and it works but for
//EXGraphicPtr & EntryDirectionGraphicPtr which are present on routes.  It wouldn't matter for autosig routes because they are replotted
//but for others they shouldn't be.  Leave in in case an easy way to remove these pointers comes to mind.
/*
...
void TTrain::PickUpBackgroundBitmap(int Caller, int HOffset, int VOffset, int Element, int EntryPos, Graphics::TBitmap *GraphicPtr) const
{
    ...
    // default values
    Graphics::TBitmap *EntryDirectionGraphicPtr = RailGraphics->bmTransparentBgnd;
    TTrackElement TempElement = Track->TrackElementAt(286, Element); //this is a copy of the element passed into the function
    ...
    else if (TempElement.TrackType == SignalPost)
    {
        ...
        if (FoundFlag)
        {
            ...
            else if (Track->InactiveTrackElementAt(116,
                IMPair.first).TrackType == LevelCrossing) {
                TempGraphic->Assign(TempElement.GraphicPtr);
                TempGraphic->Transparent = true;
                TempGraphic->TransparentColor = Utilities->clTransparent;
                // note that can't be an AutoSigsRoute
...
void TTrain::PlotTrainGraphic(int Caller, int ArrayNumber, TDisplay *Disp)
{
    ...
    SetTrainElementID(0, PlotElement[ArrayNumber],
        PlotEntryPos[ArrayNumber]);
    // set before plot so gap flashing stops first
    Disp->PlotOutput(29,
        ((Track->TrackElementAt(295, PlotElement[ArrayNumber]).HLoc * 16) +
        HOffset[ArrayNumber]),
        ((Track->TrackElementAt(296, PlotElement[ArrayNumber]).VLoc * 16) +
        VOffset[ArrayNumber]), HeadCodePosition[ArrayNumber]);
...
bool TTrain::BufferAtExit(int Caller, int Element, int ExitPos) const {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",BufferAtExit," + AnsiString(Element) + "," +
        AnsiString(ExitPos) + "," + HeadCode);
...
bool TTrain::ContinuationExit(int Caller, int Element, int ExitPos) const {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",ContinuationExit," + AnsiString(Element) + "," +
        AnsiString(ExitPos) + "," + HeadCode);
...
bool TTrain::IsTrainIDOnBridgeTrackPos01(int Caller,
    unsigned int TrackVectorPosition)
    // test whether this train on a bridge on trackpos 0 & 1
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",IsTrainIDOnBridgeTrackPos01," +
        AnsiString(TrackVectorPosition) + "," + HeadCode);
    ...
    if (Track->TrackElementAt(305, TrackVectorPosition)
        .TrainIDOnBridgeTrackPos01 == TrainID) {
        if (Track->TrackElementAt(306, TrackVectorPosition)
            .TrainIDOnBridgeTrackPos23 == TrainID) {
...
bool TTrain::IsTrainIDOnBridgeTrackPos23(int Caller,
    unsigned int TrackVectorPosition)
    // test whether this train on a bridge on trackpos 2 & 3
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",IsTrainIDOnBridgeTrackPos23," +
        AnsiString(TrackVectorPosition) + "," + HeadCode);
    ...
    if (Track->TrackElementAt(309, TrackVectorPosition)
        .TrainIDOnBridgeTrackPos23 == TrainID) {
...
void TTrain::SetTrainElementID(int Caller, unsigned int TrackVectorPosition,
    int EntryPos) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",SetTrainElementID," +
        AnsiString(TrackVectorPosition) + "," + AnsiString(EntryPos) + "," +
        HeadCode);
    ...
    if (Track->GapFlashFlag) {
        if ((LeadElement == Track->GapFlashGreenPosition) ||
            (LeadElement == Track->GapFlashRedPosition)) {
    ...
    if (Track->TrackElementAt(311, TrackVectorPosition).TrackType == Bridge) {
        ...
        if (EntryPos < 2)
            Track->TrackElementAt(312, TrackVectorPosition)
                .TrainIDOnBridgeTrackPos01 = TrainID;
        else
            Track->TrackElementAt(313, TrackVectorPosition)
                .TrainIDOnBridgeTrackPos23 = TrainID;
    }
...
void TTrain::ResetTrainElementID(int Caller, unsigned int TrackVectorPosition,
    int EntryPos) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",ResetTrainElementID," +
        AnsiString(TrackVectorPosition) + "," + AnsiString(EntryPos) + "," +
        HeadCode);
    if (Track->TrackElementAt(314, TrackVectorPosition).TrackType != Bridge) {
        ...
        if (EntryPos < 2)
            Track->TrackElementAt(316, TrackVectorPosition)
                .TrainIDOnBridgeTrackPos01 = -1;
        else
            Track->TrackElementAt(317, TrackVectorPosition)
                .TrainIDOnBridgeTrackPos23 = -1;
        if ((EntryPos < 2) && (Track->TrackElementAt(318,
            TrackVectorPosition).TrainIDOnBridgeTrackPos23 > -1))
            // i.e. other train on track 2&3
        {
            Track->TrackElementAt(319, TrackVectorPosition).TrainIDOnElement =
                Track->TrackElementAt(320, TrackVectorPosition)
                .TrainIDOnBridgeTrackPos23;
        }
        else if ((EntryPos > 1) && (Track->TrackElementAt(321,
            TrackVectorPosition).TrainIDOnBridgeTrackPos01 > -1))
            // i.e. other train on track 1&2
        {
            Track->TrackElementAt(322, TrackVectorPosition).TrainIDOnElement =
                Track->TrackElementAt(323, TrackVectorPosition)
                .TrainIDOnBridgeTrackPos01;
        }
        else
            Track->TrackElementAt(324, TrackVectorPosition)
                .TrainIDOnElement = -1;
    }
...
void TTrain::PlotAlternativeTrackRouteGraphic(int Caller,
    unsigned int ElementVecNum, int ElementEntryPos, int HOffset, int VOffset,
    TStraddle StraddleValue) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",PlotAlternativeTrackRouteGraphic," +
        AnsiString(ElementVecNum) + "," + AnsiString(ElementEntryPos) + "," +
        AnsiString(HOffset) + "," + AnsiString(VOffset) + "," +
        AnsiString(StraddleValue) + "," + HeadCode);
    int LockedVectorNumber;
        ...
        // LinkPos doesn't have to be the entry position for the above check
    {
        ...
        RoutePair1 = AllRoutes->GetRouteElementDataFromRoute2MultiMap(13,
            Track->TrackElementAt(327, ElementVecNum).HLoc,
            Track->TrackElementAt(328, ElementVecNum).VLoc, RoutePair2);
        int FirstELink, SecondELink = -1;
        FirstELink = AllRoutes->GetFixedRouteAt(149, RoutePair1.first)
            .GetFixedPrefDirElementAt(159, RoutePair1.second).GetELink();
        // must be at least one
        if (RoutePair2.first > -1)
            SecondELink = AllRoutes->GetFixedRouteAt(150, RoutePair2.first)
                .GetFixedPrefDirElementAt(160, RoutePair2.second).GetELink();
        TPrefDirElement RouteElement;
        // Graphics::TBitmap *RouteGraphic;
        if (FirstELink == Track->TrackElementAt(329,
            ElementVecNum).Link[ElementEntryPos])
            // i.e. other track is in RoutePair2
        {
            ...
            if (SecondELink == Track->TrackElementAt(330,
                ElementVecNum).Link[ElementEntryPos])
            ...
            RouteElement = AllRoutes->GetFixedRouteAt(152, RoutePair2.first)
                .GetFixedPrefDirElementAt(162, RoutePair2.second);
        }
        else // other track is in RoutePair1
        {
            ...
            RouteElement = AllRoutes->GetFixedRouteAt(154, RoutePair1.first)
                .GetFixedPrefDirElementAt(164, RoutePair1.second);
        }
        ...
        DestGraphic->Canvas->CopyRect(DestRect,
            RouteElement.GetRouteEXGraphicPtr()->Canvas, SourceRect);
        Display->PlotOutput(31,
            (Track->TrackElementAt(331, ElementVecNum).HLoc * 16) + HOffset,
            (Track->TrackElementAt(332, ElementVecNum).VLoc * 16) + VOffset,
            DestGraphic);
        ...
        if (StraddleValue == LeadMidLag) {
            if (AllRoutes->
                IsElementInLockedRouteGetPrefDirElementGetLockedVectorNumber(11,
                RouteElement.GetTrackVectorPosition(),
                RouteElement.GetXLinkPos(), PrefDirElement, LockedVectorNumber))
            {
                Display->PlotOutput(32,
                    (Track->TrackElementAt(333, ElementVecNum).HLoc * 16),
                    (Track->TrackElementAt(334, ElementVecNum).VLoc * 16),
                    RailGraphics->LockedRouteCancelPtr
                    [RouteElement.GetELink()]);
            }
        }
        delete DestGraphic;
    }
    ...
    {
        if (Track->TrackElementAt(336, ElementVecNum)
            .TrainIDOnBridgeTrackPos01 > -1) {
            TrainController->TrainVectorAtIdent(31, Track->TrackElementAt(337,
                ElementVecNum).TrainIDOnBridgeTrackPos01).PlotTrain(2, Display);
        }
    }
    else // other train is on track 23
    {
        if (Track->TrackElementAt(338, ElementVecNum)
            .TrainIDOnBridgeTrackPos23 > -1) {
            TrainController->TrainVectorAtIdent(32, Track->TrackElementAt(339,
                ElementVecNum).TrainIDOnBridgeTrackPos23).PlotTrain(3, Display);
        }
    }
...
void TTrain::CheckAndCancelRouteForWrongEndEntry(int Caller, int Element,
    int EntryPos) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CheckAndCancelRouteForWrongEndEntry," +
        AnsiString(Element) + "," + AnsiString(EntryPos) + "," + HeadCode);
    int RouteNumber;
    bool WrongRoute = false;
    TPrefDirElement RouteElement;
    TAllRoutes::TRouteElementPair RoutePair;
    TAllRoutes::TRoute2MultiMapIterator Route2MultiMapIterator;
    if (AllRoutes->GetRouteTypeAndNumber(11, Element, EntryPos,
        RouteNumber) == TAllRoutes::NoRoute)
    ...
    {
        if ((Track->TrackElementAt(340, Element).TrackType == Crossover) ||
            (Track->TrackElementAt(341, Element).TrackType == Points)) {
            if (AllRoutes->GetRouteTypeAndNumber(12, Element, (3 - EntryPos),
                RouteNumber) != TAllRoutes::NoRoute)
                // (3-EntryPos) guarantees other route (0->3; 1->2; 2->1; 3->0)
            {
                if (AllRoutes->GetFixedRouteAt(179, RouteNumber).PrefDirSize()
                    > 2) { // don't call for stub end routes
                    TrainController->LogActionError(49, HeadCode, "",
                        RouteForceCancelled, Track->TrackElementAt(799,
                        Element).ElementID);
                }
                AllRoutes->GetModifiableRouteAt(13, RouteNumber)
                    .ForceCancelRoute(1);
        ...
        if ((LinkNumber == 1) || (LinkNumber == 3) || (LinkNumber == 7) ||
            (LinkNumber == 9)) {
            if (AllRoutes->DiagonalFouledByRoute(0, TrackElement.HLoc,
                TrackElement.VLoc, LinkNumber)) {
                ...
                if (LinkNumber == 1) {
                    if (AllRoutes->FindRouteNumberFromRoute2MultiMapNoErrors(0,
                        TrackElement.HLoc - 1, TrackElement.VLoc, 3,
                        RouteNumber)) {
                        if (AllRoutes->GetFixedRouteAt(207,
                            RouteNumber).PrefDirSize() > 2)
                        { // don't call for stub end routes
                            TrainController->LogActionError(55, HeadCode, "",
                                RouteForceCancelled, TrackElement.ElementID);
                            LogActionErrorCalled = true;
                        }
                        AllRoutes->GetModifiableRouteAt(20, RouteNumber)
                            .ForceCancelRoute(3);
                    }
                    if (AllRoutes->FindRouteNumberFromRoute2MultiMapNoErrors(1,
                        TrackElement.HLoc, TrackElement.VLoc - 1, 7,
                        RouteNumber))
                        // not else in case have different routes on each diagonal, though shouldn't be possible
                    {
                        if (!LogActionErrorCalled && AllRoutes->GetFixedRouteAt
                            (208, RouteNumber).PrefDirSize() > 2)
                        { // don't call for stub end routes
                            TrainController->LogActionError(56, HeadCode, "",
                                RouteForceCancelled, TrackElement.ElementID);
                        }
                        AllRoutes->GetModifiableRouteAt(21, RouteNumber)
                            .ForceCancelRoute(4);
                    }
                }
                ...
                // for LinkNumber = 3, potentially fouled diagonals are at H+1, V, Lk 1 & H, V-1 Lk 9
                    ...
                    if (AllRoutes->FindRouteNumberFromRoute2MultiMapNoErrors(2,
                        TrackElement.HLoc + 1, TrackElement.VLoc, 1,
                        RouteNumber)) {
                        if (AllRoutes->GetFixedRouteAt(209,
                            RouteNumber).PrefDirSize() > 2)
                        { // don't call for stub end routes
                            TrainController->LogActionError(57, HeadCode, "",
                                RouteForceCancelled, TrackElement.ElementID);
                            LogActionErrorCalled = true;
                        }
                        AllRoutes->GetModifiableRouteAt(22, RouteNumber)
                            .ForceCancelRoute(5);
                    }
                    if (AllRoutes->FindRouteNumberFromRoute2MultiMapNoErrors(3,
                        TrackElement.HLoc, TrackElement.VLoc - 1, 9,
                        RouteNumber))
                        // not else in case have different routes on each diagonal, though shouldn't be possible
                    {
                        if (!LogActionErrorCalled && AllRoutes->GetFixedRouteAt
                            (210, RouteNumber).PrefDirSize() > 2)
                        { // don't call for stub end routes
                            TrainController->LogActionError(58, HeadCode, "",
                                RouteForceCancelled, TrackElement.ElementID);
                        }
                        AllRoutes->GetModifiableRouteAt(23, RouteNumber)
                            .ForceCancelRoute(6);
                    }
                }
                ...
                // for LinkNumber = 7, potentially fouled diagonals are at H-1, V, Lk 9 & H, V+1 Lk 1
                    ...
                    if (AllRoutes->FindRouteNumberFromRoute2MultiMapNoErrors(4,
                        TrackElement.HLoc - 1, TrackElement.VLoc, 9,
                        RouteNumber)) {
                        if (AllRoutes->GetFixedRouteAt(211,
                            RouteNumber).PrefDirSize() > 2)
                        { // don't call for stub end routes
                            TrainController->LogActionError(59, HeadCode, "",
                                RouteForceCancelled, TrackElement.ElementID);
                            LogActionErrorCalled = true;
                        }
                        AllRoutes->GetModifiableRouteAt(24, RouteNumber)
                            .ForceCancelRoute(7);
                    }
                    if (AllRoutes->FindRouteNumberFromRoute2MultiMapNoErrors(5,
                        TrackElement.HLoc, TrackElement.VLoc + 1, 1,
                        RouteNumber))
                        // not else in case have different routes on each diagonal, though shouldn't be possible
                    {
                        if (!LogActionErrorCalled && AllRoutes->GetFixedRouteAt
                            (212, RouteNumber).PrefDirSize() > 2)
                        { // don't call for stub end routes
                            TrainController->LogActionError(60, HeadCode, "",
                                RouteForceCancelled, TrackElement.ElementID);
                        }
                        AllRoutes->GetModifiableRouteAt(25, RouteNumber)
                            .ForceCancelRoute(8);
                    }
                }
                ...
                // for LinkNumber = 9, potentially fouled diagonals are at H+1, V, Lk 7 & H, V+1 Lk 3
                    ...
                    if (AllRoutes->FindRouteNumberFromRoute2MultiMapNoErrors(6,
                        TrackElement.HLoc + 1, TrackElement.VLoc, 7,
                        RouteNumber)) {
                        if (AllRoutes->GetFixedRouteAt(213,
                            RouteNumber).PrefDirSize() > 2)
                        { // don't call for stub end routes
                            TrainController->LogActionError(61, HeadCode, "",
                                RouteForceCancelled, TrackElement.ElementID);
                            LogActionErrorCalled = true;
                        }
                        AllRoutes->GetModifiableRouteAt(26, RouteNumber)
                            .ForceCancelRoute(9);
                    }
                    if (AllRoutes->FindRouteNumberFromRoute2MultiMapNoErrors(7,
                        TrackElement.HLoc, TrackElement.VLoc + 1, 3,
                        RouteNumber))
                        // not else in case have different routes on each diagonal, though shouldn't be possible
                    {
                        if (!LogActionErrorCalled && AllRoutes->GetFixedRouteAt
                            (214, RouteNumber).PrefDirSize() > 2)
                        { // don't call for stub end routes
                            TrainController->LogActionError(62, HeadCode, "",
                                RouteForceCancelled, TrackElement.ElementID);
                        }
                        AllRoutes->GetModifiableRouteAt(27, RouteNumber)
                            .ForceCancelRoute(10);
                    }
                }
            }
    ...
    for (unsigned int x = 0; x < AllRoutes->GetFixedRouteAt(155,
        RouteNumber).PrefDirSize(); x++) {
        RouteElement = AllRoutes->GetFixedRouteAt(156, RouteNumber)
            .GetFixedPrefDirElementAt(165, x);
    ...
    { // don't call for stub end routes
        TrainController->LogActionError(50, HeadCode, "", RouteForceCancelled,
            Track->TrackElementAt(800, Element).ElementID);
    }
...
void TTrain::PlotTrainWithNewBackgroundColour(int Caller,
    TColor NewBackgroundColour, TDisplay *Disp) {
    if (BackgroundColour == NewBackgroundColour)
        return; // don't replot if already correct
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",PlotTrainWithNewBackgroundColour," +
        AnsiString(NewBackgroundColour));
    bool ColourError = false, ColourError2 = false;
    RailGraphics->ChangeBackgroundColour(1, FrontCodePtr, FrontCodePtr,
        NewBackgroundColour, BackgroundColour, ColourError);
    if (ColourError)
        ColourError2 = true;
    for (int x = 0; x < 4; x++) {
        RailGraphics->ChangeBackgroundColour(2, HeadCodeGrPtr[x],
            HeadCodeGrPtr[x], NewBackgroundColour, BackgroundColour,
            ColourError);
        if (ColourError)
            ColourError2 = true;
    }
...
void TTrain::SetTrainMovementValues(int Caller, int TrackVectorPosition,
    int EntryPos)
    /*
     Note:  Within the loop BrakeRate can only increase and MaxExitSpeed can only reduce

     Summary:  Called during PlotStartPosition to set initial values & during UpdateTrain when Straddle is LeadMidLag,
    ...
    {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",SetTrainMovementValues," +
        AnsiString(TrackVectorPosition) + "," + AnsiString(EntryPos) + "," +
        HeadCode);
    int EntryHalfLength, CurrentElementHalfLength, NextElementHalfLength,
        CumulativeLength = 0, CurrentTrackVectorPosition = TrackVectorPosition;
    int DistanceAtHalfBraking, DistanceAtThreeQuarterBraking, ExitPos,
        NextTrackVectorPosition, NextEntryPos;
    bool RedSignalFlag = false, BuffersFlag = false, StationFlag = false,
        BuffersOrContinuationNowFlag = false, ContinuationNextFlag = false,
        TrainInFrontInSignallerModeFlag = false;
    double LimitingSpeed, FrontElementMaxSpeed,
        MaxExitSpeedAtHalfBrakingSquared, MaxExitSpeedAtHalfBraking,
        NextSpeedLimit, TempBrakeRate;
    ...
    if (Track->OtherTrainOnTrack(2, CurrentTrackVectorPosition, EntryPos,
        TrainID) && (EntrySpeed < 1)) {
        FrontElementSpeedLimit =
            Track->TrackElementAt(344, CurrentTrackVectorPosition).SpeedLimit01;
        FrontElementLength =
            Track->TrackElementAt(345, CurrentTrackVectorPosition).Length01;
        if (EntryPos > 1) {
            FrontElementSpeedLimit =
                Track->TrackElementAt(346, CurrentTrackVectorPosition)
                .SpeedLimit23;
            FrontElementLength =
                Track->TrackElementAt(347, CurrentTrackVectorPosition).Length23;
        }
        EntrySpeed = 0;
        ExitSpeedHalf = 0;
        ExitSpeedFull = 0;
    ...
    if (CurrentTrackVectorPosition > -1) {
        if (Track->TrackElementAt(855, CurrentTrackVectorPosition)
            .TrackType == Points) // this test & section added at v0.6
        {
            if ((EntryPos == 0) || (EntryPos == 2)) {
                if (Track->TrackElementAt(856, CurrentTrackVectorPosition)
                    .Attribute == 0) {
                    CurrentElementHalfLength =
                        (Track->TrackElementAt(857, CurrentTrackVectorPosition)
                        .Length01) / 2;
                    FrontElementSpeedLimit =
                        Track->TrackElementAt(858, CurrentTrackVectorPosition)
                        .SpeedLimit01;
                }
                else {
                    CurrentElementHalfLength =
                        (Track->TrackElementAt(859, CurrentTrackVectorPosition)
                        .Length23) / 2;
                    FrontElementSpeedLimit =
                        Track->TrackElementAt(860, CurrentTrackVectorPosition)
                        .SpeedLimit23;
                }
            }
            else if (EntryPos == 1) {
                CurrentElementHalfLength =
                    (Track->TrackElementAt(861, CurrentTrackVectorPosition)
                    .Length01) / 2;
                FrontElementSpeedLimit =
                    Track->TrackElementAt(862, CurrentTrackVectorPosition)
                    .SpeedLimit01;
            }
            else // == 3
            {
                CurrentElementHalfLength =
                    (Track->TrackElementAt(863, CurrentTrackVectorPosition)
                    .Length23) / 2;
                FrontElementSpeedLimit =
                    Track->TrackElementAt(864, CurrentTrackVectorPosition)
                    .SpeedLimit23;
            }
        }
        else {
            if (EntryPos > 1) {
                CurrentElementHalfLength =
                    (Track->TrackElementAt(348, CurrentTrackVectorPosition)
                    .Length23) / 2;
                FrontElementSpeedLimit =
                    Track->TrackElementAt(349, CurrentTrackVectorPosition)
                    .SpeedLimit23;
            }
            else {
                CurrentElementHalfLength =
                    (Track->TrackElementAt(350, CurrentTrackVectorPosition)
                    .Length01) / 2;
                FrontElementSpeedLimit =
                    Track->TrackElementAt(351, CurrentTrackVectorPosition)
                    .SpeedLimit01;
            }
        }
    ...
    if (LimitingSpeed > MaxRunningSpeed)
        LimitingSpeed = MaxRunningSpeed;
    FrontElementMaxSpeed = LimitingSpeed;

    // check if running past a red signal without permission
    if ((Track->TrackElementAt(352, CurrentTrackVectorPosition)
        .Config[Track->GetNonPointsOppositeLinkPos(EntryPos)] == Signal) &&
        (Track->TrackElementAt(353, CurrentTrackVectorPosition).Attribute == 0)
        && (EntrySpeed > 1) && !AllowedToPassRedSignal) {
    ...
    if (!SPADFlag) {
        ...
        ExitSpeedFull =
            3.6 * Power(((3 * EntryHalfLength * AValue * AValue) +
            (EntrySpeed * EntrySpeed * EntrySpeed / 3.6 / 3.6 / 3.6)),
        0.333334);
        double ExitSpeedAtMaxBraking;
        if ((EntrySpeed * EntrySpeed) <=
            (2 * MaxBrakeRate * FrontElementLength)) {
            ExitSpeedAtMaxBraking = 0;
        }
        else {
            ExitSpeedAtMaxBraking =
                sqrt((EntrySpeed * EntrySpeed) -
                2 * MaxBrakeRate * FrontElementLength);
        }
        if (ExitSpeedFull > LimitingSpeed)
            ExitSpeedFull = ExitSpeedAtMaxBraking;
        // changed at v2.4.0, was ExitSpeedFull = LimitingSpeed; but that allowed very high brake rates when
        // took signaller control of a fast failed train with signaller limiting speed 30km/h
        DistanceAtHalfBraking =
            ExitSpeedFull * ExitSpeedFull / 3.6 / 3.6 / MaxBrakeRate;
        DistanceAtThreeQuarterBraking =
            ExitSpeedFull * ExitSpeedFull / 3.6 / 3.6 / 1.5 / MaxBrakeRate;
        // used for signaller stops
        ...
        do {
            ...
            if ((CumulativeLength >= DistanceAtThreeQuarterBraking) &&
                (TrainMode == Signaller) && SignallerStoppingFlag) {
                ...
                double TempBR =
                    EntrySpeed * EntrySpeed / 2 / 3.6 / 3.6 / CumulativeLength;
            ...
            if (Track->TrackElementAt(374, CurrentTrackVectorPosition)
                .TrackType == Buffers) {
            ...
            if (TrainMode == Timetable) {
                bool StopRequired = false;
                if (!TimetableFinished && (NameInTimetableBeforeCDT(12,
                    Track->TrackElementAt(375, CurrentTrackVectorPosition)
                    .ActiveTrackElementName, StopRequired) > -1) &&
                    ((Track->TrackElementAt(376,
                    CurrentTrackVectorPosition)
                    .StationEntryStopLinkPos1 == EntryPos) ||
                    (Track->TrackElementAt(377,
                    CurrentTrackVectorPosition)
                    .StationEntryStopLinkPos2 == EntryPos))) {
            ...
            if (((Track->TrackElementAt(354, CurrentTrackVectorPosition)
                .TrackType == Buffers) || (Track->TrackElementAt(355,
                CurrentTrackVectorPosition).TrackType == Continuation)) &&
                (EntryPos == 1)) {
                BuffersOrContinuationNowFlag = true;
            }
            if (!BuffersOrContinuationNowFlag && !BuffersFlag &&
                !StationFlag) // skip if buffers or station flags already set
            {
                if (Track->TrackElementAt(356, CurrentTrackVectorPosition)
                    .TrackType == Points) {
                    if ((EntryPos == 0) || (EntryPos == 2)) {
                        if (Track->TrackElementAt(357,
                            CurrentTrackVectorPosition).Attribute == 0)
                            ExitPos = 1;
                        else
                            ExitPos = 3;
                    }
                ...
                NextTrackVectorPosition =
                    Track->TrackElementAt(358, CurrentTrackVectorPosition)
                    .Conn[ExitPos];
                NextEntryPos =
                    Track->TrackElementAt(359, CurrentTrackVectorPosition)
                    .ConnLinkPos[ExitPos];
                if (NextTrackVectorPosition > -1) {
                    if (Track->TrackElementAt(845, NextTrackVectorPosition)
                        .TrackType == Points)
                        // this test & section added at v0.6
                    {
                        if ((NextEntryPos == 0) || (NextEntryPos == 2)) {
                            if (Track->TrackElementAt(846,
                                NextTrackVectorPosition).Attribute == 0) {
                                NextElementHalfLength =
                                    (Track->TrackElementAt(847,
                                    NextTrackVectorPosition).Length01) / 2;
                                NextSpeedLimit =
                                    Track->TrackElementAt(848,
                                    NextTrackVectorPosition).SpeedLimit01;
                            }
                            else {
                                NextElementHalfLength =
                                    (Track->TrackElementAt(849,
                                    NextTrackVectorPosition).Length23) / 2;
                                NextSpeedLimit =
                                    Track->TrackElementAt(850,
                                    NextTrackVectorPosition).SpeedLimit23;
                            }
                        }
                        else if (NextEntryPos == 1) {
                            NextElementHalfLength =
                                (Track->TrackElementAt(851,
                                NextTrackVectorPosition).Length01) / 2;
                            NextSpeedLimit =
                                Track->TrackElementAt(852,
                                NextTrackVectorPosition).SpeedLimit01;
                        }
                        else // == 3
                        {
                            NextElementHalfLength =
                                (Track->TrackElementAt(853,
                                NextTrackVectorPosition).Length23) / 2;
                            NextSpeedLimit =
                                Track->TrackElementAt(854,
                                NextTrackVectorPosition).SpeedLimit23;
                        }
                    }
                    else {
                        if (NextEntryPos > 1) {
                            NextElementHalfLength =
                                (Track->TrackElementAt(360,
                                NextTrackVectorPosition).Length23) / 2;
                            NextSpeedLimit =
                                Track->TrackElementAt(361,
                                NextTrackVectorPosition).SpeedLimit23;
                        }
                        else {
                            NextElementHalfLength =
                                (Track->TrackElementAt(362,
                                NextTrackVectorPosition).Length01) / 2;
                            NextSpeedLimit =
                                Track->TrackElementAt(363,
                                NextTrackVectorPosition).SpeedLimit01;
                        }
                    }
                }
                ...
                bool StopRequired;
                if (Track->TrackElementAt(364, NextTrackVectorPosition)
                    .Config[Track->GetNonPointsOppositeLinkPos(NextEntryPos)]
                    == Signal) {
                    if (Track->TrackElementAt(365, NextTrackVectorPosition)
                        .Attribute == 0)
                        // no need to add in the length of element to CumulativeLength
                            RedSignalFlag = true;
                ...
                else if ((TrainMode == Timetable) && !TimetableFinished &&
                    (NameInTimetableBeforeCDT(10, Track->TrackElementAt(368,
                    CurrentTrackVectorPosition).ActiveTrackElementName,
                    StopRequired) > -1) && Track->OtherTrainOnTrack(3,
                    NextTrackVectorPosition, NextEntryPos, TrainID)) {
                    // no need to add in the length of element to CumulativeLength
                    if (StopRequired)
                        StationFlag = true;
                }
                // check if next element contains a train & in Signaller mode (always stops for train in front if in signaller mode)
                else if ((TrainMode == Signaller) && Track->OtherTrainOnTrack(4,
                    NextTrackVectorPosition, NextEntryPos, TrainID))
                ...
                else if (Track->TrackElementAt(366, NextTrackVectorPosition)
                    .TrackType == Buffers) {
                    // need to add in the length of that element to CumulativeLength
                    CumulativeLength +=
                        Track->TrackElementAt(367, NextTrackVectorPosition)
                        .Length01;
                    BuffersFlag = true;
                }
                // check if next element is a station stop
                else if ((TrainMode == Timetable) && !TimetableFinished &&
                    (NameInTimetableBeforeCDT(11, Track->TrackElementAt(370,
                    NextTrackVectorPosition).ActiveTrackElementName,
                    StopRequired) > -1) && ((Track->TrackElementAt(371,
                    NextTrackVectorPosition)
                    .StationEntryStopLinkPos1 == EntryPos) ||
                    (Track->TrackElementAt(372,
                    NextTrackVectorPosition)
                    .StationEntryStopLinkPos2 == EntryPos))) {
                    ...
                    if (StopRequired) {
                        StationFlag = true;
                        CumulativeLength +=
                            Track->TrackElementAt(373, NextTrackVectorPosition)
                            .Length01;
                    }
                }
            }
            if (RedSignalFlag || BuffersFlag || StationFlag ||
                TrainInFrontInSignallerModeFlag || SignallerStopRequired ||
                StepForwardFlag) // no more loops
            {
                    ...
                    // use constant acceleration in calculating half time point
                {
                    MaxExitSpeed = 0;
                    double MaxHalfSpeed;
                    double MaxHalfSpeedAtHalfBraking =
                        3.6 * sqrt(MaxBrakeRate * FrontElementLength / 2);
                        ...
                        // been braking smoothly at less that 50% braking rate, 2x should prevent all but extreme cases
                    {
                        ExitSpeedHalf =
                            3.6 * Power
                            (((1.5 * EntryHalfLength * AValue * AValue) +
                            (EntrySpeed * EntrySpeed * EntrySpeed / 3.6 / 3.6 /
                            3.6)), 0.333334);
                        ...
                        { // [km/h/3.6 = m/s]
                            ExitTimeHalf =
                                EntryTime +
                                TDateTime(((ExitSpeedHalf * ExitSpeedHalf) -
                                (EntrySpeed * EntrySpeed)) / 3.6 / 3.6 /
                                (AValue * AValue) / 86400.0);
                        }
                        else {
                            ExitTimeHalf =
                                EntryTime +
                                TDateTime(EntryHalfLength * 3.6 / EntrySpeed /
                                86400.0);
                        }
                        ...
                        ExitSpeedFull = 0;
                        TempBrakeRate =
                            ExitSpeedHalf * ExitSpeedHalf / 2 / 3.6 / 3.6 /
                            EntryHalfLength;
                        ...
                        ExitTimeFull =
                            ExitTimeHalf +
                            TDateTime(ExitSpeedHalf / 3.6 / BrakeRate /
                            86400.0);
                        if (HalfSpeedLimited)
                ...
                MaxExitSpeedAtHalfBrakingSquared =
                    3.6 * 3.6 * MaxBrakeRate *
                    (CumulativeLength - FrontElementLength);
                if (MaxExitSpeedAtHalfBrakingSquared < 10)
                    MaxExitSpeedAtHalfBraking = 0;
                else
                    MaxExitSpeedAtHalfBraking =
                        sqrt(MaxExitSpeedAtHalfBrakingSquared);
                ...
                if ((EntrySpeed > MaxExitSpeed) || SignallerStopRequired ||
                    (SignallerStopBrakeRate > 0.01)) // need to brake
                {
                    ExitSpeedHalfSquared = (EntrySpeed * EntrySpeed) -
                        (3.6 * 3.6 * 2 * BrakeRate * EntryHalfLength);
                    ...
                    ExitTimeHalf =
                        EntryTime + TDateTime
                        ((EntrySpeed - ExitSpeedHalf)
                        / 3.6 / BrakeRate / 86400.0);
                    ExitSpeedFullSquared = (EntrySpeed * EntrySpeed) -
                        (3.6 * 3.6 * 4 * BrakeRate * EntryHalfLength);
                ...
                {
                    ExitSpeedHalfSquared = (EntrySpeed * EntrySpeed) -
                        (3.6 * 3.6 * BrakeRate * FrontElementLength);
                    ExitSpeedHalf = sqrt(ExitSpeedHalfSquared);
                    ExitTimeHalf =
                        EntryTime + TDateTime
                        ((EntrySpeed - ExitSpeedHalf) /
                    ...
                    ExitSpeedFullSquared = (EntrySpeed * EntrySpeed) -
                        (3.6 * 3.6 * 2 * BrakeRate * FrontElementLength);
                    ExitSpeedFull = sqrt(ExitSpeedFullSquared);
                    ExitTimeFull =
                        EntryTime + TDateTime
                        ((EntrySpeed - ExitSpeedFull) /
                    ...
                    // without the power need above condition or have hours of delay times, above added at v2.4.0
                {
                    ...
                    ExitSpeedFull =
                        3.6 * Power
                        (((3 * EntryHalfLength * AValue * AValue) +
                        (EntrySpeed * EntrySpeed * EntrySpeed / 3.6 / 3.6 / 3.6)
                        ), 0.333334);
                        ...
                        // can accelerate continually over the half length
                    {
                            ...
                            // i.e both (ExitSpeedHalf <= MaxExitSpeed) & (ExitSpeedFull <= MaxExitSpeed)
                        {
                            ExitTimeFull =
                                EntryTime +
                                TDateTime(((ExitSpeedFull * ExitSpeedFull) -
                                (EntrySpeed * EntrySpeed)) / 3.6 / 3.6 /
                                (AValue * AValue) / 86400.0);
                        }
                            ...
                            // accelerate to MaxExitSpeed then reamin at this speed for rest of element
                        {
                            ...
                            double DistanceToMax =
                                ((MaxExitSpeed * MaxExitSpeed * MaxExitSpeed) -
                                (EntrySpeed * EntrySpeed * EntrySpeed))
                                / 3.6 / 3.6 / 3.6 / (1.5 * AValue * AValue);
                            double RemainingDistance =
                                double(FrontElementLength) - DistanceToMax;
                            double DeltaRemainingTimeInSecs =
                                3.6 * RemainingDistance / MaxExitSpeed;
                            ExitTimeFull =
                                EntryTime +
                                TDateTime((DeltaExitTimeToMaxInSecs +
                                DeltaRemainingTimeInSecs) / 86400.0);
                        }
                    }
                        ...
                        // second halves of the element
                    {
                        ...
                        double DistanceToMax =
                            ...
                            / 3.6 / 3.6 / 3.6 / (1.5 * AValue * AValue);
                        double RemainingDistance =
                            double(FrontElementLength / 2) - DistanceToMax;
                        // remaining distance to half length
                        double DeltaRemainingTimeInSecs =
                            3.6 * RemainingDistance / MaxExitSpeed;
                        ExitTimeHalf =
                            EntryTime + TDateTime
                            ((DeltaExitTimeToMaxInSecs +
                            DeltaRemainingTimeInSecs) / 86400.0);
                        ExitSpeedFull = MaxExitSpeed;
                        ExitTimeFull =
                            ExitTimeHalf +
                            TDateTime(3.6 * EntryHalfLength / MaxExitSpeed /
                            86400.0);
                    }
                }
                ...
                MaxExitSpeedAtHalfBrakingSquared =
                    (LimitingSpeed * LimitingSpeed) +
                    (3.6 * 3.6 * MaxBrakeRate * (CumulativeLength -
                    FrontElementLength));
                if (MaxExitSpeedAtHalfBrakingSquared < 10)
                    MaxExitSpeedAtHalfBraking = 0;
                else
            ...
            if (!BuffersOrContinuationNowFlag) {
                ...
                if (Track->TrackElementAt(378, NextTrackVectorPosition)
                    .TrackType == Continuation) {
                    ContinuationNextFlag = true;
                }
            }
        }
        while (((CumulativeLength - FrontElementLength) < DistanceAtHalfBraking)
            && ((!BuffersOrContinuationNowFlag && !ContinuationNextFlag) ||
        ...
        if (BrakeRate > 0.01) {
            ...
            ExitTimeHalf =
                EntryTime + TDateTime
                ((EntrySpeed - ExitSpeedHalf) / 3.6 / BrakeRate / 86400.0);
            ExitSpeedFullSquared = (EntrySpeed * EntrySpeed) -
                (3.6 * 3.6 * 4 * BrakeRate * EntryHalfLength);
            if (ExitSpeedFullSquared < 10)
                ExitSpeedFull = 0;
            ...
            ExitTimeFull =
                EntryTime + TDateTime
                ((EntrySpeed - ExitSpeedFull) / 3.6 / BrakeRate / 86400.0);
        }
        else {
            // for accelerating the energy input rate (PowerAtRail) is constant so the following formuli are used:-
            // (1)  V^2/(3.6^2) - U^2/(3.6^2) = A^2T;  (2)  V = 3.6 * ((1.5*S*A^2) + U^3/ (3.6)^3)^0.333334;
            ...
            BrakeRate = 0;
            ExitSpeedHalf =
                3.6 * Power(((1.5 * EntryHalfLength * AValue * AValue) +
                (EntrySpeed * EntrySpeed * EntrySpeed / 3.6 / 3.6 / 3.6)),
                0.333334);
            ExitSpeedFull =
                3.6 * Power(((3 * EntryHalfLength * AValue * AValue) +
            ...
            if (ExitSpeedHalf <= MaxExitSpeed) {
                ...
                { // [km/h/3.6 = m/s]
                    ...
                    ExitTimeHalf =
                        EntryTime + TDateTime
                        (EntryHalfLength * 3.6 / EntrySpeed / 86400.0);
                }
                    ...
                    // (ExitSpeedHalf <= MaxExitSpeed) & (ExitSpeedFull <= MaxExitSpeed)
                {
                    ...
                    { // [km/h/3.6 = m/s]
                        ...
                        ExitTimeFull =
                            EntryTime + TDateTime
                            (2 * EntryHalfLength * 3.6 / EntrySpeed / 86400.0);
                    }
                }
                else // (ExitSpeedHalf <= MaxExitSpeed) & (ExitSpeedFull > MaxExitSpeed)
                {
                    ...
                    {
                        DeltaExitTimeToMaxInSecs =
                            2 * EntryHalfLength * 3.6 / EntrySpeed;
                        // these not really accurate but will be good enough
                        DistanceToMax = EntryHalfLength;
                    }
                    double RemainingDistance = double(FrontElementLength) -
                        DistanceToMax;
                    double DeltaRemainingTimeInSecs =
                        3.6 * RemainingDistance / MaxExitSpeed;
                    ExitTimeFull =
                        EntryTime + TDateTime
                        ((DeltaExitTimeToMaxInSecs + DeltaRemainingTimeInSecs)
                        / 86400.0);
                }
            }
            else // ExitSpeedHalf > MaxExitSpeed, ExitSpeedFull must be > MaxExitSpeed
            {
                ...
                double RemainingDistance = double(FrontElementLength / 2) -
                    DistanceToMax; // remaining distance to half length
                double DeltaRemainingTimeInSecs =
                    3.6 * RemainingDistance / MaxExitSpeed;
                ExitTimeHalf =
                    EntryTime + TDateTime
                ...
                ExitSpeedFull = MaxExitSpeed;
                ExitTimeFull =
                    ExitTimeHalf + TDateTime
                    (3.6 * EntryHalfLength / MaxExitSpeed / 86400.0);
            }
        }
    }
    ...
    else // SPADFlag set
    {
        ...
        if (ExitSpeedHalfSquared < 10)
            ...
            ExitSpeedHalf = sqrt(ExitSpeedHalfSquared);
        ExitTimeHalf = EntryTime + TDateTime
            ((EntrySpeed - ExitSpeedHalf) / 3.6 / BrakeRate / 86400.0);
        ExitSpeedFullSquared = (EntrySpeed * EntrySpeed) -
            (3.6 * 3.6 * 4 * BrakeRate * EntryHalfLength);
        if (ExitSpeedFullSquared < 10)
            ...
            ExitSpeedFull = sqrt(ExitSpeedFullSquared);
        ExitTimeFull = EntryTime + TDateTime
            ((EntrySpeed - ExitSpeedFull) / 3.6 / BrakeRate / 86400.0);
        ...
        if (ExitSpeedFull > 0) {
            if (Track->TrackElementAt(746, CurrentTrackVectorPosition)
                .TrackType == Points) {
                if ((EntryPos == 0) || (EntryPos == 2)) {
                    if (Track->TrackElementAt(747, CurrentTrackVectorPosition)
                        .Attribute == 0)
                        ExitPos = 1;
                    else
                        ExitPos = 3;
                ...
                ExitPos = Track->GetNonPointsOppositeLinkPos(EntryPos);
            NextTrackVectorPosition =
                Track->TrackElementAt(748, CurrentTrackVectorPosition)
                .Conn[ExitPos];
            NextEntryPos = Track->TrackElementAt(749,
                CurrentTrackVectorPosition).ConnLinkPos[ExitPos];
            if (NextTrackVectorPosition > -1) // not a continuation or buffer
            {
                int NextElementLength;
                if (NextEntryPos > 1) {
                    NextElementLength =
                        (Track->TrackElementAt(750, NextTrackVectorPosition)
                        .Length23);
                }
                else {
                    NextElementLength =
                        (Track->TrackElementAt(751, NextTrackVectorPosition)
                        .Length01);
                }
                double NextStoppingSpeed =
                    sqrt(3.6 * 3.6 * 2 * BrakeRate * NextElementLength);
    ...
    { // bring to a stop in 20 elements at 100km/h & assume each 100m long for calculating exit times but if on a continuation maintain speed
        if (LeadElement > -1) {
                ...
                // don't stop on a continuation either entering or leaving
            {
                ...
                ExitTimeHalf =
                    EntryTime + TDateTime
                    ((50 * 3.6 / (EntrySpeed) / 86400.0));
                // assume length is 50m for ease of calc
                ExitTimeFull =
                    ExitTimeHalf + TDateTime
                    ((50 * 3.6 / (ExitSpeedHalf) / 86400.0));
        ...
        if (EntrySpeed > 7.5) // keep going for at least another element
        {
            ...
            ExitTimeHalf =
                EntryTime + TDateTime
                ((50 * 3.6 / (EntrySpeed - 1.25) / 86400.0));
            // assume length is 50m for ease of calc
            ExitTimeFull =
                ExitTimeHalf + TDateTime
                ((50 * 3.6 / (ExitSpeedHalf - 1.25) / 86400.0));
            Utilities->CallLogPop(2129);
            return;
        }
        else // stop on this element, floating label had displayed last exit speed full on 2nd half move so with zero when fully on element will appear to have slowed at steady rate
        {
            ExitSpeedHalf = 0;
            ExitSpeedFull = 0;
            MaxExitSpeed = LimitingSpeed;
...
int TTrain::NameInTimetableBeforeCDT(int Caller, AnsiString Name, bool &Stop)
     ...
     */ {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",NameInTimetableBeforeCDT," + Name + "," +
        HeadCode);
    Stop = false;
    ...
    for (TActionVectorEntry * Ptr = ActionVectorEntryPtr;
    Ptr < &TrainDataEntryPtr->ActionVector.back(); Ptr++) {
        if ((Ptr->Command == "cdt") || (Ptr->FormatType == Repeat)) {
            break;
        }
        if ((Ptr->ArrivalTime > TDateTime(-1)) && (Ptr->LocationName == Name)) {
            if ((Ptr->FormatType == TimeLoc) ||
                (Ptr->FormatType == TimeTimeLoc)) {
                Stop = true;
                Utilities->CallLogPop(960);
                return (Ptr - ActionVectorEntryPtr);
            }
        }
        if ((Ptr->EventTime > TDateTime(-1)) && (Ptr->LocationName == Name) &&
            (Ptr->Command == "pas")) {
...
bool TTrain::ClearToNextSignal(int Caller)
     ...
     */ {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",ClearToNextSignal" + "," + HeadCode);
    int ReturnVal = 0;
    ...
    while (true) {
        if ((Track->TrackElementAt(382, CurrentTrackVectorPosition)
            .TrainIDOnElement > -1) && (Track->TrackElementAt(383,
            CurrentTrackVectorPosition).TrainIDOnElement != TrainID)) {
            ReturnVal = 1;
            break;
        }
        if (((Track->TrackElementAt(384, CurrentTrackVectorPosition)
            .TrackType == Buffers) || (Track->TrackElementAt(385,
            CurrentTrackVectorPosition).TrackType == Continuation)) &&
            (EntryPos == 1)) {
            ReturnVal = 2;
            break;
        }
        if ((EntryPos < 2) && (Track->TrackElementAt(386,
            CurrentTrackVectorPosition).Config[1 - EntryPos] == Signal) &&
            (Track->TrackElementAt(529, CurrentTrackVectorPosition)
            .Attribute != 4)) // Attr 4 == call-on signal
        {
            ReturnVal = 3;
            break;
        }
        if ((Track->TrackElementAt(387, CurrentTrackVectorPosition)
            .TrackType == Bridge) || (Track->TrackElementAt(388,
            CurrentTrackVectorPosition).TrackType == Crossover)) {
            if ((EntryPos < 2) && (Track->TrackElementAt(523,
                CurrentTrackVectorPosition).TempTrackMarker01))
                // must be a loop - reached same point as examined earlier
            {
                ReturnVal = 4;
                break;
            }
            else if ((EntryPos > 1) && (Track->TrackElementAt(524,
            ...
            if ((Track->TrackElementAt(525, CurrentTrackVectorPosition)
                .TempTrackMarker01) || (Track->TrackElementAt(526,
                CurrentTrackVectorPosition).TempTrackMarker23)) {
                ReturnVal = 4;
                break;
            }
        ...
        if (EntryPos < 2)
            Track->TrackElementAt(389, CurrentTrackVectorPosition)
                .TempTrackMarker01 = true;
        else
            Track->TrackElementAt(527, CurrentTrackVectorPosition)
                .TempTrackMarker23 = true;

        if (Track->TrackElementAt(390, CurrentTrackVectorPosition)
            .TrackType == Points) {
            if ((EntryPos == 0) || (EntryPos == 2)) {
                if (Track->TrackElementAt(391, CurrentTrackVectorPosition)
                    .Attribute == 0)
                    ExitPos = 1;
                else
                    ExitPos = 3;
            ...
            ExitPos = Track->GetNonPointsOppositeLinkPos(EntryPos);
        NextTrackVectorPosition =
            Track->TrackElementAt(392, CurrentTrackVectorPosition)
            .Conn[ExitPos];
        NextEntryPos = Track->TrackElementAt(393, CurrentTrackVectorPosition)
            .ConnLinkPos[ExitPos];
        CurrentTrackVectorPosition = NextTrackVectorPosition;
        EntryPos = NextEntryPos;
    }
...
bool TTrain::CallingOnAllowed(int Caller)
    /*
     ...
     change points outside the route or have a route conflict if another route is set.
     */ {
    if (Track->RouteFlashFlag)
        return false;
    // don't want to create a new route from the stop signal if one is already in construction as
    // some of the callingon route elements may be involved
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CallingOnAllowed" + "," + HeadCode);
    bool PlatformFoundFlag = false, StopRequired = false, SkipRouteCheck =
        false, RouteOrPartRouteSet = false; // last added at v1.2.0
    int CurrentTrackVectorPosition = LeadElement, NextTrackVectorPosition,
        ElementNumber = 0, Distance = 0;
    ...
    // not used here
    AnsiString LeadStationName = Track->TrackElementAt(395, LeadElement)
        .ActiveTrackElementName; // still OK even if ""
    // must be stopped at a signal but not at a location & still in timetable (a)

    if (StoppedAtLocation || !StoppedAtSignal || TimetableFinished || Crashed ||
        Derailed || DerailPending || StoppedAfterSPAD ||
        StoppedForTrainInFront)
        // no need to check for SignallerStopped as this only called in Timetable mode
    {
        Utilities->CallLogPop(711);
        return false;
    }
    while (true) {
        TTrackElement &CurrentTrackElement =
            Track->TrackElementAt(396, CurrentTrackVectorPosition);
        ...
        if ((CurrentTrackElement.TrainIDOnElement > -1) &&
            (CurrentTrackElement.TrainIDOnElement != TrainID)
            && !PlatformFoundFlag) {
        ...
        if ((CurrentTrackElement.TrainIDOnElement > -1) &&
            ...
            TTrain OtherTrain = TrainController->TrainVectorAtIdent(12,
                CurrentTrackElement.TrainIDOnElement);
            ...
            if (OtherTrain.LeadElement == CurrentTrackVectorPosition) {
                AnsiString OtherCommand =
                    OtherTrain.ActionVectorEntryPtr->Command;
                if ((OtherCommand == "Fjo") || (OtherCommand == "jbo") ||
                    (OtherCommand == "cdt") || (OtherCommand == "Frh") ||
        ...
        if ((EntryPos < 2) && (CurrentTrackElement.Config[1 -
            EntryPos] == Signal) &&
            (CurrentTrackVectorPosition != Track->TrackElementAt(404,
        ...
        if ((CurrentTrackElement.ActiveTrackElementName != "") &&
            (CurrentTrackElement.ActiveTrackElementName != LeadStationName) &&
            (NameInTimetableBeforeCDT(15,
            CurrentTrackElement.ActiveTrackElementName, StopRequired) > -1)) {
            if (StopRequired) {
                if ((CurrentTrackElement.TrackType != Points) ||
                    ((CurrentTrackElement.TrackType == Points) &&
         ...
         */
        if (ElementNumber > 1) {
            if (AllRoutes->GetRouteTypeAndNumber(26, CurrentTrackVectorPosition,
                EntryPos, RouteID) != AllRoutes->NoRoute)
                RouteOrPartRouteSet = true;
            else
                RouteOrPartRouteSet = false;
        }
        if (!SkipRouteCheck && !RouteOrPartRouteSet) {
            if (AllRoutes->TrackIsInARoute(16, CurrentTrackVectorPosition,
                EntryPos)) // must be a conflicting route
            {
                Utilities->CallLogPop(1859);
                return false;
            }
            int ExitLink = CurrentTrackElement.Link[ExitPos];
            if ((ExitLink == 1) || (ExitLink == 3) || (ExitLink == 7) ||
                (ExitLink == 9)) {
                if (AllRoutes->DiagonalFouledByRouteOrTrain(6,
                    CurrentTrackElement.HLoc, CurrentTrackElement.VLoc,
                    ExitLink)) {
    ...
    TAllRoutes::TCallonEntry CallonEntry(RouteOrPartRouteSet,
        RouteStartPosition, PlatformPosition);
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",GetTrainHeadCode" + "," + HeadCode);
    AnsiString RepeatHeadCode = TrainController->GetRepeatHeadCode(0, HeadCode,
...
TDateTime TTrain::GetTrainTime(int Caller, TDateTime Time) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",GetTrainTime," + Utilities->Format96HHMMSS
        (Time));
    TDateTime RepeatTime = TrainController->GetRepeatTime(1, Time, RepeatNumber,
        IncrementalMinutes);
...
{ // Used to check for a stopped adjacent train for use in PopUp menu //new at v2.4.0
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",IsThereAnAdjacentTrain" + "," + HeadCode);
    ...
    {
        FrontAdjacentTrackElement =
            Track->TrackElementAt(965, (Track->TrackElementAt(966, LeadElement).Conn[LeadExitPos]));
        FrontValid = true;
    }
    if (Track->TrackElementAt(967, MidElement).Conn[MidEntryPos] > -1)
    {
        RearAdjacentTrackElement =
            Track->TrackElementAt(968,
            (Track->TrackElementAt(969, MidElement).Conn[MidEntryPos]));
        RearValid = true;
    }
...
void TTrain::LogAction(int Caller, AnsiString OwnHeadCode,
    AnsiString OtherHeadCode, TActionType ActionType, AnsiString LocationName,
     ...
     */ {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",LogAction," + OwnHeadCode + "," + OtherHeadCode +
    ...
    if (ActionType == SignallerPassRedSignal)
        ActionLog = " received signaller authority to pass red signal";
    ...
    if (Warning) {
        BaseLog = Utilities->Format96HHMMSS(ActualTime) + " WARNING: " +
            HeadCode + ActionLog + OtherHeadCode + LocationName;
        WarningBaseLog = HeadCode + ActionLog + OtherHeadCode + LocationName;
    }
    else {
        BaseLog = Utilities->Format96HHMMSS(ActualTime) + ": " + HeadCode +
            ActionLog + OtherHeadCode + LocationName;
    }
    bool TimePerformance = true;
    if ((ActionType == TakeSignallerControl) ||
        (ActionType == RestoreTimetableControl) || (ActionType == RemoveTrain)
        || (ActionType == SignallerMoveForwards) ||
        (ActionType == SignallerChangeDirection) ||
        (ActionType == SignallerPassRedSignal) ||
        (ActionType == SignallerControlStop) || (ActionType == SignallerStop) ||
        (ActionType == SignallerLeave) || (ActionType == SignallerStepForward)
        || (ActionType == SignallerJoin) || (ActionType == TrainFailure) ||
        (ActionType == RepairFailedTrain))
    ...
    if (TimePerformance) {
        double MinsLate =
            ((double)(ActualTime - GetTrainTime(1, TimetableNonRepeatTime))
            ) * 1440;
...
void TTrain::TrainHasFailed(int Caller) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",TrainHasFailed," + HeadCode);
    ...
    if (LeadElement > -1)
    {
        LocName = Track->TrackElementAt(972, LeadElement)
            .ActiveTrackElementName;
    }
    if ((LocName == "") && (MidElement > -1))
    {
        LocName = Track->TrackElementAt(973, MidElement)
            .ActiveTrackElementName;
    }
    ...
    TrainController->StopTTClockMessage(81,
        HeadCode + " has suffered an onboard power failure at " + LocName);
    TrainFailed = true;
    TrainFailurePending = false;
    OriginalPowerAtRail = PowerAtRail;
    PowerAtRail = 0.08;
...
void TTrain::FrontTrainSplit(int Caller) {
    ...
    TrainController->LogEvent("" + AnsiString(Caller) + ",FrontTrainSplit" +
        "," + HeadCode);
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",FrontTrainSplit" + "," + HeadCode);
    if (PowerAtRail < 1)
        // new at v2.4.0 (ActionVectorEntryPtr not incremented so can split when power restored
    {
        if (!ZeroPowerNoFrontSplitMessage)
            TrainController->StopTTClockMessage(82,
            HeadCode + ": A train without power can't split");
        ZeroPowerNoFrontSplitMessage = true;
        Utilities->CallLogPop(2137);
        return;
    }
    AnsiString LocationName = Track->TrackElementAt(555, LeadElement)
        .ActiveTrackElementName;
    if (LocationName == "")
        LocationName = Track->TrackElementAt(837, MidElement)
            .ActiveTrackElementName;
    int FirstNamedElementPos, SecondNamedElementPos, FirstNamedLinkedElementPos,
    ...
    AnsiString OtherHeadCode = TrainController->GetRepeatHeadCode(1,
        ActionVectorEntryPtr->OtherHeadCode, RepeatNumber, IncrementalDigits);
    // determine all positions & exits
    if (LocationName != "") {
        // if message given only call at ~5 sec intervals
        if (!(TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
            .EventReported == FailLocTooShort) || (UpdateCounter == 0)) {
            FirstNamedElementPos = LeadElement;
            if (!Track->ThisNamedLocationLongEnoughForSplit(0, LocationName,
                FirstNamedElementPos,
                // check if possible with LeadElement as First
                SecondNamedElementPos, FirstNamedLinkedElementPos,
                SecondNamedLinkedElementPos)) {
                FirstNamedElementPos = MidElement;
                if (!Track->ThisNamedLocationLongEnoughForSplit(1, LocationName,
                    FirstNamedElementPos,
                    // if not then accept second if possible (though if Lead no good hard to see how Mid could work, but leave in)
                    SecondNamedElementPos, FirstNamedLinkedElementPos,
                    SecondNamedLinkedElementPos)) {
                    if (TrainDataEntryPtr->TrainOperatingDataVector.at
                        (RepeatNumber).EventReported != FailLocTooShort) {
                        TrainController->LogActionError(6, HeadCode, "",
                            FailLocTooShort, LocationName);
                        TrainDataEntryPtr->TrainOperatingDataVector.at
                            (RepeatNumber).EventReported = FailLocTooShort;
                    }
                ...
                int LeadNumAtLoc = 0;
                if (Track->TrackElementAt(758,
                    LeadPosA).ActiveTrackElementName == LocationName)
                    LeadNumAtLoc++;
                if (Track->TrackElementAt(759,
                    LeadPosB).ActiveTrackElementName == LocationName)
                    LeadNumAtLoc++;
                if (Track->TrackElementAt(760,
                    LeadPosC).ActiveTrackElementName == LocationName)
                    LeadNumAtLoc++;
                if (Track->TrackElementAt(761,
                    LeadPosD).ActiveTrackElementName == LocationName)
                    LeadNumAtLoc++;
                if (LeadNumAtLoc < 4) {
                    FirstNamedElementPos = MidElement;
                    if (!Track->ThisNamedLocationLongEnoughForSplit(4,
                        LocationName, FirstNamedElementPos,
                        SecondNamedElementPos, FirstNamedLinkedElementPos,
                    ...
                    {
                        int MidNumAtLoc = 0;
                        if (Track->TrackElementAt(762, FirstNamedElementPos)
                            .ActiveTrackElementName == LocationName)
                            MidNumAtLoc++;
                        if (Track->TrackElementAt(763,
                            FirstNamedLinkedElementPos)
                            .ActiveTrackElementName == LocationName)
                            MidNumAtLoc++;
                        if (Track->TrackElementAt(764,
                            FirstNamedLinkedElementPos)
                            .ActiveTrackElementName == LocationName)
                            MidNumAtLoc++;
                        if (Track->TrackElementAt(765,
                            SecondNamedLinkedElementPos)
                            .ActiveTrackElementName == LocationName)
                            MidNumAtLoc++;
    ...
    RearTrainExitPos = -1;
    for (int x = 0; x < 4; x++) {
        if (Track->TrackElementAt(584, RearTrainRearPosition)
            .Conn[x] == RearTrainFrontPosition) {
            RearTrainExitPos = x;
            break;
        }
    }
    ...
    FrontTrainExitPos = -1;
    for (int x = 0; x < 4; x++) {
        if (Track->TrackElementAt(585, FrontTrainRearPosition)
            .Conn[x] == FrontTrainFrontPosition) {
            FrontTrainExitPos = x;
            break;
        }
    }
    ...
    int TrainIDOnRearOfRearTrain, TrainIDOnFrontOfRearTrain,
        TrainIDOnRearOfFrontTrain, TrainIDOnFrontOfFrontTrain;
    TTrackElement RearMostElement =
        Track->TrackElementAt(574, RearTrainRearPosition);
    ...
    TrainIDOnFrontOfRearTrain =
        Track->TrackElementAt(575, RearTrainFrontPosition).TrainIDOnElement;
    // can't be a bridge
    TrainIDOnRearOfFrontTrain =
        Track->TrackElementAt(576, FrontTrainRearPosition).TrainIDOnElement;
    // can't be a bridge
    // FrontTrainFrontPosition = Track->TrackElementAt(578,FrontTrainRearPosition).Conn[FrontTrainExitPos];
    TTrackElement FrontMostElement =
        Track->TrackElementAt(577, FrontTrainFrontPosition);
    ...
    if (((TrainIDOnRearOfRearTrain > -1) &&
        (TrainIDOnRearOfRearTrain != TrainID)) ||
        ((TrainIDOnFrontOfRearTrain > -1) &&
        (TrainIDOnFrontOfRearTrain != TrainID)) ||
        ((TrainIDOnRearOfFrontTrain > -1) &&
        (TrainIDOnRearOfFrontTrain != TrainID)) ||
        ((TrainIDOnFrontOfFrontTrain > -1) &&
        (TrainIDOnFrontOfFrontTrain != TrainID))) {
        if (TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
            .EventReported != FailSplitDueToOtherTrain) {
            TrainController->LogActionError(8, HeadCode, "",
                FailSplitDueToOtherTrain, LocationName);
            TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
                .EventReported = FailSplitDueToOtherTrain;
        }
    ...
    LogAction(9, HeadCode, OtherHeadCode, FrontSplit,
        ActionVectorEntryPtr->LocationName, ActionVectorEntryPtr->EventTime,
        ActionVectorEntryPtr->Warning);
    ActionVectorEntryPtr++;
    LastActionTime = TrainController->TTClockTime;
    ...
    AValue = sqrt(2 * PowerAtRail / Mass);
    // shouln't change but include in case not set earlier
    ...
    if (!TrainController->AddTrain(0, FrontTrainRearPosition,
        FrontTrainFrontPosition, OtherHeadCode, 0, Mass, MaxRunningSpeed,
        MaxBrakeRate, PowerAtRail, "Timetable",
        OldActionVectorEntryPtr->LinkedTrainEntryPtr, RepeatNumber,
        IncrementalMinutes, IncrementalDigits, SignallerMaxSpeed, false))
        // false for SignallerControl
    {
        Utilities->CallLogPop(1721);
    ...
    TTrainOperatingData &TTOD =
        OldActionVectorEntryPtr->LinkedTrainEntryPtr->TrainOperatingDataVector.
        at(RepeatNumber); // this is for the newly created train
...
void TTrain::RearTrainSplit(int Caller) {
    ...
    TrainController->LogEvent("" + AnsiString(Caller) + ",RearTrainSplit" +
        "," + HeadCode);
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",RearTrainSplit" + "," + HeadCode);
    if (PowerAtRail < 1)
        // new at v2.4.0 (ActionVectorEntryPtr not incremented so can split when power restored
    {
        if (!ZeroPowerNoRearSplitMessage)
            TrainController->StopTTClockMessage(83,
            HeadCode + ": A train without power can't split");
        ZeroPowerNoRearSplitMessage = true;
        Utilities->CallLogPop(2138);
        return;
    }
    AnsiString LocationName = Track->TrackElementAt(587, LeadElement)
        .ActiveTrackElementName;
    if (LocationName == "")
        LocationName = Track->TrackElementAt(838, MidElement)
            .ActiveTrackElementName;
    int FirstNamedElementPos, SecondNamedElementPos, FirstNamedLinkedElementPos,
        SecondNamedLinkedElementPos;
    int RearTrainRearPosition, RearTrainFrontPosition, RearTrainExitPos;
    int FrontTrainRearPosition, FrontTrainFrontPosition, FrontTrainExitPos;
    AnsiString OtherHeadCode = TrainController->GetRepeatHeadCode(2,
        ActionVectorEntryPtr->OtherHeadCode, RepeatNumber, IncrementalDigits);
    // determine all positions & exits
    if (LocationName != "") {
        // if message given only call at ~5 sec intervals
        if (!(TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
            .EventReported == FailLocTooShort) || (UpdateCounter == 0)) {
            FirstNamedElementPos = LeadElement;
            if (!Track->ThisNamedLocationLongEnoughForSplit(2, LocationName,
                FirstNamedElementPos, SecondNamedElementPos,
                FirstNamedLinkedElementPos, SecondNamedLinkedElementPos)) {
                FirstNamedElementPos = MidElement;
                if (!Track->ThisNamedLocationLongEnoughForSplit(3, LocationName,
                    FirstNamedElementPos, SecondNamedElementPos,
                    FirstNamedLinkedElementPos, SecondNamedLinkedElementPos)) {
                    if (TrainDataEntryPtr->TrainOperatingDataVector.at
                        (RepeatNumber).EventReported != FailLocTooShort) {
                        TrainController->LogActionError(9, HeadCode, "",
                            FailLocTooShort, LocationName);
                        TrainDataEntryPtr->TrainOperatingDataVector.at
                            (RepeatNumber).EventReported = FailLocTooShort;
                    }
                ...
                // count number of positions that are at the location
                int LeadNumAtLoc = 0;
                if (Track->TrackElementAt(767,
                    LeadPosA).ActiveTrackElementName == LocationName)
                    LeadNumAtLoc++;
                if (Track->TrackElementAt(768,
                    LeadPosB).ActiveTrackElementName == LocationName)
                    LeadNumAtLoc++;
                if (Track->TrackElementAt(769,
                    LeadPosC).ActiveTrackElementName == LocationName)
                    LeadNumAtLoc++;
                if (Track->TrackElementAt(770,
                    LeadPosD).ActiveTrackElementName == LocationName)
                    LeadNumAtLoc++;
                if (LeadNumAtLoc < 4) {
                    FirstNamedElementPos = MidElement;
                    if (!Track->ThisNamedLocationLongEnoughForSplit(5,
                        LocationName, FirstNamedElementPos,
                        SecondNamedElementPos, FirstNamedLinkedElementPos,
                        SecondNamedLinkedElementPos)) { // restore originals
                        FirstNamedElementPos = LeadPosA;
                    ...
                    {
                        int MidNumAtLoc = 0;
                        if (Track->TrackElementAt(771, FirstNamedElementPos)
                            .ActiveTrackElementName == LocationName)
                            MidNumAtLoc++;
                        if (Track->TrackElementAt(772,
                            FirstNamedLinkedElementPos)
                            .ActiveTrackElementName == LocationName)
                            MidNumAtLoc++;
                        if (Track->TrackElementAt(773,
                            FirstNamedLinkedElementPos)
                            .ActiveTrackElementName == LocationName)
                            MidNumAtLoc++;
                        if (Track->TrackElementAt(774,
                            SecondNamedLinkedElementPos)
                            .ActiveTrackElementName == LocationName)
                            MidNumAtLoc++;
                        if (LeadNumAtLoc > MidNumAtLoc)
    ...
    RearTrainExitPos = -1;
    for (int x = 0; x < 4; x++) {
        if (Track->TrackElementAt(588, RearTrainRearPosition)
            .Conn[x] == RearTrainFrontPosition) {
            RearTrainExitPos = x;
            break;
        }
    ...
    FrontTrainExitPos = -1;
    for (int x = 0; x < 4; x++) {
        if (Track->TrackElementAt(589, FrontTrainRearPosition)
            .Conn[x] == FrontTrainFrontPosition) {
            FrontTrainExitPos = x;
            break;
        }
    ...
    int TrainIDOnRearOfRearTrain, TrainIDOnFrontOfRearTrain,
        TrainIDOnRearOfFrontTrain, TrainIDOnFrontOfFrontTrain;
    TTrackElement RearMostElement =
        Track->TrackElementAt(590, RearTrainRearPosition);
    if ((RearMostElement.TrackType == Bridge) && (RearTrainExitPos > 1))
        TrainIDOnRearOfRearTrain = RearMostElement.TrainIDOnBridgeTrackPos23;
    else if ((RearMostElement.TrackType == Bridge) && (RearTrainExitPos < 2))
        TrainIDOnRearOfRearTrain = RearMostElement.TrainIDOnBridgeTrackPos01;
    ...
    // RearTrainFrontPosition = RearMostElement.Conn[RearTrainExitPos];
    TrainIDOnFrontOfRearTrain =
        Track->TrackElementAt(591, RearTrainFrontPosition).TrainIDOnElement;
    // can't be a bridge
    TrainIDOnRearOfFrontTrain =
        Track->TrackElementAt(592, FrontTrainRearPosition).TrainIDOnElement;
    // can't be a bridge
    // FrontTrainFrontPosition = Track->TrackElementAt(593,FrontTrainRearPosition).Conn[FrontTrainExitPos];
    TTrackElement FrontMostElement =
        Track->TrackElementAt(594, FrontTrainFrontPosition);
    if ((FrontMostElement.TrackType == Bridge) && (FrontTrainExitPos > 1))
        TrainIDOnFrontOfFrontTrain = FrontMostElement.TrainIDOnBridgeTrackPos23;
    else if ((FrontMostElement.TrackType == Bridge) && (FrontTrainExitPos < 2))
        TrainIDOnFrontOfFrontTrain = FrontMostElement.TrainIDOnBridgeTrackPos01;
    ...
    if (((TrainIDOnRearOfRearTrain > -1) &&
        ...
        if (TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
            .EventReported != FailSplitDueToOtherTrain) {
            TrainController->LogActionError(11, HeadCode, "",
                FailSplitDueToOtherTrain, LocationName);
            TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
                .EventReported = FailSplitDueToOtherTrain;
        }
    ...
    LogAction(10, HeadCode, OtherHeadCode, RearSplit,
        ActionVectorEntryPtr->LocationName, ActionVectorEntryPtr->EventTime,
        ActionVectorEntryPtr->Warning);
    ActionVectorEntryPtr++;
    LastActionTime = TrainController->TTClockTime;
    ...
    if (!TrainController->AddTrain(1, RearTrainRearPosition,
        RearTrainFrontPosition, OtherHeadCode, 0, Mass, MaxRunningSpeed,
        MaxBrakeRate, PowerAtRail, "Timetable",
        OldActionVectorEntryPtr->LinkedTrainEntryPtr, RepeatNumber,
        IncrementalMinutes, IncrementalDigits, SignallerMaxSpeed, false))
        // false for SignallerControl
    {
        Utilities->CallLogPop(1722);
        // if fails either a throw will have been sent in AddTrain or start position failed prob because of
        // another train, in which case a message will have been sent to the perf log, also might well clear later
        // when other train moves away
        return;
...
void TTrain::FinishJoin(int Caller) {
    ...
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",FinishJoin" + "," + HeadCode);
    if (TrainFailed)
        // new at v2.4.0 (ActionVectorEntryPtr not incremented so can join when repaired) Can FinishJoin if zero power & not failed, as for empty stock
    {
        if (!FailedTrainNoFinishJoinMessage)
            TrainController->StopTTClockMessage(84,
            HeadCode +
            ": A failed train can't join another under timetable control");
    ...
    TTrain *TrainToJoin;
    AnsiString JBOHeadCode = TrainController->GetRepeatHeadCode(3,
        ActionVectorEntryPtr->OtherHeadCode, RepeatNumber, IncrementalDigits);
    if (!TrainToJoinIsAdjacent(0, TrainToJoin)) {
        if (TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
            .EventReported != WaitingForJBO) {
            ...
            TrainController->LogActionError(51, HeadCode, JBOHeadCode,
                WaitingForJBO, ActionVectorEntryPtr->LocationName);
            TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
                .EventReported = WaitingForJBO;
        }
...
void TTrain::JoinedBy(int Caller) {
    ...
    if (!TrainToBeJoinedByIsAdjacent(0, TrainToBeJoinedBy)) {
        if (TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
            ...
            TrainController->LogActionError(52, HeadCode, FJOHeadCode,
                WaitingForFJO, ActionVectorEntryPtr->LocationName);
            TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
                .EventReported = WaitingForFJO;
        }
    ...
    double OtherBrakeForce =
        TrainToBeJoinedBy->MaxBrakeRate * TrainToBeJoinedBy->Mass;
    double OwnBrakeForce = MaxBrakeRate * Mass;
    double CombinedBrakeRate = (OtherBrakeForce + OwnBrakeForce) /
        (TrainToBeJoinedBy->Mass + Mass);
    Mass += TrainToBeJoinedBy->Mass;
    MaxBrakeRate = CombinedBrakeRate;
    PowerAtRail += TrainToBeJoinedBy->PowerAtRail;
    AValue = sqrt(2 * PowerAtRail / Mass);

    TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber).EventReported =
        NoEvent;
    TrainToBeJoinedBy->TrainDataEntryPtr->TrainOperatingDataVector.at
        (RepeatNumber).EventReported = NoEvent;
    ...
    LogAction(11, HeadCode, FJOHeadCode, JoinedByOther,
        ActionVectorEntryPtr->LocationName, ActionVectorEntryPtr->EventTime,
        ActionVectorEntryPtr->Warning);
    ActionVectorEntryPtr++;
    LastActionTime = TrainController->TTClockTime;
...
void TTrain::ChangeTrainDirection(int Caller) {
    TrainController->LogEvent("" + AnsiString(Caller) +
        ",ChangeTrainDirection" + "," + HeadCode);
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",ChangeTrainDirection" + "," + HeadCode);
    if (PowerAtRail < 1)
        // new at v2.4.0 (ActionVectorEntryPtr not incremented so can change direction when power restored)
    {
        if (!ZeroPowerNoCDTMessage)
            TrainController->StopTTClockMessage(86,
            HeadCode +
            ": A train without power can't change direction under timetable control");
    ...
    LogAction(12, HeadCode, "", ChangeDirection,
        ActionVectorEntryPtr->LocationName, ActionVectorEntryPtr->EventTime,
        ActionVectorEntryPtr->Warning);
    ActionVectorEntryPtr++;
    LastActionTime = TrainController->TTClockTime;
    ...
    // change to new train, give new service message
{
    TrainController->LogEvent("" + AnsiString(Caller) + ",NewTrainService" +
        "," + HeadCode);
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",NewTrainService" + "," + HeadCode);
    if (PowerAtRail < 1)
        // new at v2.4.0 (ActionVectorEntryPtr not incremented so can form new service when power restored)
    {
        if (!ZeroPowerNoNewServiceMessage)
            TrainController->StopTTClockMessage(87,
            HeadCode + ": A train without power can't form a new service");
        ZeroPowerNoNewServiceMessage = true;
        Utilities->CallLogPop(2142);
        return;
    }
    AnsiString NewHeadCode = TrainController->GetRepeatHeadCode(5,
        ActionVectorEntryPtr->OtherHeadCode, RepeatNumber, IncrementalDigits);
    LogAction(13, HeadCode, NewHeadCode, NewService,
        ActionVectorEntryPtr->LocationName, ActionVectorEntryPtr->EventTime,
        ActionVectorEntryPtr->Warning);
    UnplotTrain(3);
    ...
    TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber).TrainID =
        TrainID;
    TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber).RunningEntry =
        Running;
...
void TTrain::RemainHere(int Caller) {
    ...
    {
        TrainController->LogEvent(Utilities->TimeStamp() + AnsiString(Caller) +
            ",RemainHere" + "," + HeadCode);
        RemainHereLogNotSent = false;
    }
    if (!TerminatedMessageSent) {
        Display->PerformanceLog(5,
            Utilities->Format96HHMMSS(TrainController->TTClockTime) + ": " +
            HeadCode + " terminated at " + ActionVectorEntryPtr->LocationName);
        LastActionTime = TrainController->TTClockTime;
        TerminatedMessageSent = true;
    }
    TimetableFinished = true;
...
void TTrain::SendMissedActionLogs(int Caller, int IncNum,
    TActionVectorEntry *Ptr)
    /*
    ...
    TrainController->LogEvent("" + AnsiString(Caller) +
        ",SendMissedActionLogs," + AnsiString(IncNum) + "," + HeadCode);
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",SendMissedActionLogs," + AnsiString(IncNum) +
        "," + HeadCode);
    if (IncNum > 0) {
        for (int x = 0; x < IncNum; x++) {
            ...
            if ((Ptr->FormatType == TimeLoc) && (Ptr->ArrivalTime >
                TDateTime(-1))) {
                TrainController->LogActionError(12, HeadCode, "",
                    FailMissedArrival, Ptr->LocationName);
            }
            // arrival & departure
            if (Ptr->FormatType == TimeTimeLoc) {
                TrainController->LogActionError(13, HeadCode, "",
                    FailMissedArrival, Ptr->LocationName);
            }
            // departure
            else if ((Ptr->FormatType == TimeLoc) &&
                (Ptr->ArrivalTime == TDateTime(-1))) {
                continue; // skip TimeLoc departures, message given for arrivals
            }
            // pass
            else if (Ptr->FormatType == PassTime) {
                TrainController->LogActionError(36, HeadCode, "",
                    FailMissedPass, Ptr->LocationName);
            }
            // split
            else if ((Ptr->Command == "fsp") || (Ptr->Command == "rsp")) {
                TrainController->LogActionError(14, HeadCode, "",
                    FailMissedSplit, Ptr->LocationName);
            }
            // jbo
            else if (Ptr->Command == "jbo") {
                TrainController->LogActionError(15, HeadCode, "", FailMissedJBO,
                    Ptr->LocationName);
            }
            ...
            else if ((Ptr->Command == "Fns") || (Ptr->Command == "Frh") ||
                (Ptr->Command == "Fer") || (Ptr->Command == "Fjo") ||
                (Ptr->Command == "Snt") || (Ptr->Command == "Sfs") ||
                (Ptr->Command == "Snt-sh") || (Ptr->Command == "Sns") ||
                (Ptr->Command == "Sns-sh") || (Ptr->Command == "Sns-fsh") ||
                (Ptr->Command == "cdt") || (Ptr->Command == "Frh-sh") ||
                (Ptr->Command == "Fns-sh") || (Ptr->Command == "F-nshs") ||
                (Ptr->FormatType == Repeat)) {
                throw Exception
                    ("Error - illegal command in SendMissedActionLogs for IncNum = " +
                    AnsiString(IncNum) + ", and command = " + Ptr->Command);
            }
        }
    }
    else {
        ...
        while (true) // finish commands & repeats break out of loop
        {
            ...
            // Fer & included
                ...
                TrainController->LogActionError(19, HeadCode, "",
            ...
            // Fjo
                ...
                TrainController->LogActionError(16, HeadCode, "",
                    FailMissedJoinOther, Ptr->LocationName);
                break;
            }
            // Frh
            else if (Ptr->Command == "Frh") {
                if (!TerminatedMessageSent) {
                    TrainController->LogActionError(17, HeadCode, "",
                        FailMissedTerminate, Ptr->LocationName);
                    TerminatedMessageSent = true;
                }
                break;
            }
            // Frh-sh
                ...
                if (!TerminatedMessageSent) {
                    TrainController->LogActionError(53, HeadCode, "",
                        FailMissedNewService, Ptr->LocationName);
                    TerminatedMessageSent = true;
                }
                break;
            }
            // Fns, F-nshs, Fns-sh
            else if ((Ptr->Command == "Fns") || (Ptr->Command == "F-nshs") ||
                (Ptr->Command == "Fns-sh")) {
                TrainController->LogActionError(18, HeadCode, "",
                    FailMissedNewService, Ptr->LocationName);
                break;
            }
            ...
            if ((Ptr->FormatType == TimeLoc) && (Ptr->ArrivalTime >
                ...
                if (IsTrainTerminating(1)) {
                    TrainController->LogActionError(20, HeadCode, "",
                        FailMissedTerminate, Ptr->LocationName);
                    TerminatedMessageSent = true;
                }
                else {
                    TrainController->LogActionError(21, HeadCode, "",
                        FailMissedArrival, Ptr->LocationName);
                }
            }
            // arrival & departure
            else if (Ptr->FormatType == TimeTimeLoc) {
                TrainController->LogActionError(22, HeadCode, "",
                    FailMissedArrival, Ptr->LocationName);
            }
            // departure
            else if ((Ptr->FormatType == TimeLoc) &&
            ...
            // pass
                ...
                TrainController->LogActionError(37, HeadCode, "",
                    FailMissedPass, Ptr->LocationName);
            }
            // split
            else if ((Ptr->Command == "fsp") || (Ptr->Command == "rsp")) {
                TrainController->LogActionError(23, HeadCode, "",
                    FailMissedSplit, Ptr->LocationName);
            }
            // jbo
            else if (Ptr->Command == "jbo") {
                TrainController->LogActionError(24, HeadCode, "", FailMissedJBO,
                    Ptr->LocationName);
            }
            // cdt
            else if (Ptr->Command == "cdt") {
                TrainController->LogActionError(25, HeadCode, "",
                    FailMissedChangeDirection, Ptr->LocationName);
            }
            // Errors
            else if ((Ptr->Command == "Snt-sh") || (Ptr->Command == "Sfs") ||
                (Ptr->Command == "Sns") || (Ptr->Command == "Sns-sh") ||
                (Ptr->Command == "Sns-fsh") ||
                ((Ptr->Command == "Snt") && !Ptr->SignallerControl)) {
                throw Exception
                    ("Error - illegal command in SendMissedActionLogs for IncNum = " +
                    AnsiString(IncNum) + ", and command = " + Ptr->Command);
            }
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",TrainToJoinIsAdjacent" + "," + HeadCode);
    TTrainDataEntry *TrainToJoinTDEntry =
        ActionVectorEntryPtr->LinkedTrainEntryPtr;
    if (TrainToJoinTDEntry->TrainOperatingDataVector.at(RepeatNumber)
        .RunningEntry != Running) {
        Utilities->CallLogPop(1024);
        return false;
    }
    TrainToJoin = &(TrainController->TrainVectorAtIdent(33,
        TrainToJoinTDEntry->TrainOperatingDataVector.at(RepeatNumber).TrainID));
    if (TrainToJoin->StoppedAtLocation && (TrainToJoin->TrainMode == Timetable)
        && (TrainToJoin->ActionVectorEntryPtr->Command == "jbo")) {
        if ((Track->TrackElementAt(610,
            LeadElement).Conn[LeadExitPos] == TrainToJoin->LeadElement) ||
            (Track->TrackElementAt(611,
            LeadElement).Conn[LeadExitPos] == TrainToJoin->MidElement) ||
            (Track->TrackElementAt(612,
            MidElement).Conn[MidEntryPos] == TrainToJoin->LeadElement) ||
            (Track->TrackElementAt(613,
            MidElement).Conn[MidEntryPos] == TrainToJoin->MidElement)) {
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",TrainToBeJoinedByIsAdjacent" + "," + HeadCode);
    TTrainDataEntry *TrainToBeJoinedByTDEntry =
        ActionVectorEntryPtr->LinkedTrainEntryPtr;
    if (TrainToBeJoinedByTDEntry->TrainOperatingDataVector.at(RepeatNumber)
        .RunningEntry != Running) {
        Utilities->CallLogPop(1027);
        return false;
    }
    TrainToBeJoinedBy =
        &(TrainController->TrainVectorAtIdent(15,
        TrainToBeJoinedByTDEntry->TrainOperatingDataVector.at(RepeatNumber)
        .TrainID));
    if (TrainToBeJoinedBy->StoppedAtLocation &&
        (TrainToBeJoinedBy->TrainMode == Timetable) &&
        (TrainToBeJoinedBy->ActionVectorEntryPtr->Command == "Fjo")) {
        if ((Track->TrackElementAt(614,
            LeadElement).Conn[LeadExitPos] == TrainToBeJoinedBy->LeadElement) ||
            (Track->TrackElementAt(615,
            LeadElement).Conn[LeadExitPos] == TrainToBeJoinedBy->MidElement) ||
            (Track->TrackElementAt(616,
            MidElement).Conn[MidEntryPos] == TrainToBeJoinedBy->LeadElement) ||
            (Track->TrackElementAt(617,
...
void TTrain::NewShuttleFromNonRepeatService(int Caller) {
    TrainController->LogEvent("" + AnsiString(Caller) +
        ",NewShuttleFromNonRepeatService" + "," + HeadCode);
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",NewShuttleFromNonRepeatService" + "," +
        HeadCode);
    if (PowerAtRail < 1)
        // new at v2.4.0 (ActionVectorEntryPtr not incremented so can for new service when power restored)
    {
        if (!ZeroPowerNoNewShuttleFromNonRepeatMessage)
            TrainController->StopTTClockMessage(88,
            HeadCode + ": A train without power can't form a new service");
        ZeroPowerNoNewShuttleFromNonRepeatMessage = true;
        Utilities->CallLogPop(2143);
        return;
    }
    AnsiString NewHeadCode =
        ActionVectorEntryPtr->NonRepeatingShuttleLinkHeadCode;
    LogAction(15, HeadCode, NewHeadCode, NewService,
        ActionVectorEntryPtr->LocationName, ActionVectorEntryPtr->EventTime,
        ActionVectorEntryPtr->Warning);
    UnplotTrain(4);
    RearStartElement = MidElement;
    ...
    ActionVectorEntryPtr = &(TrainDataEntryPtr->ActionVector.at(0));
    HeadCode = NewHeadCode;
    IncrementalMinutes = TrainDataEntryPtr->ActionVector.back()
        .RearStartOrRepeatMins;
    IncrementalDigits = TrainDataEntryPtr->ActionVector.back()
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",RepeatShuttleOrRemainHere" + "," + HeadCode);
    if (RemainHereLogNotSent) // to prevent repeated logs
    {
        TrainController->LogEvent("" + AnsiString(Caller) +
            ",RepeatShuttleOrRemainHere" + "," + HeadCode);
        RemainHereLogNotSent = false;
    }
    ...
    {
        if (TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
            .EventReported != ShuttleFinishedRemainingHere) {
            LogAction(26, HeadCode, "", Terminate,
                ActionVectorEntryPtr->LocationName,
                ActionVectorEntryPtr->EventTime, ActionVectorEntryPtr->Warning);
            TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
                .EventReported = ShuttleFinishedRemainingHere;
            TerminatedMessageSent = true;
        ...
        // new at v2.4.0 (ActionVectorEntryPtr not incremented so can for new service when power restored)
    {
        if (!ZeroPowerNoRepeatShuttleMessage)
            TrainController->StopTTClockMessage(89,
            HeadCode + ": A train without power can't form a new service");
    ...
    AnsiString NewHeadCode = TrainController->GetRepeatHeadCode(6,
        ActionVectorEntryPtr->OtherHeadCode, TempRepeatNumber,
        IncrementalDigits);
    LogAction(16, HeadCode, NewHeadCode, NewService,
        ActionVectorEntryPtr->LocationName, ActionVectorEntryPtr->EventTime,
        ActionVectorEntryPtr->Warning);
    RepeatNumber++;
    UnplotTrain(5);
...
void TTrain::RepeatShuttleOrNewNonRepeatService(int Caller) {
    TrainController->LogEvent("" + AnsiString(Caller) +
        ",RepeatShuttleOrNewNonRepeatService" + "," + HeadCode);
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",RepeatShuttleOrNewNonRepeatService" + "," +
        HeadCode);
    if (PowerAtRail < 1)
        // new at v2.4.0 (ActionVectorEntryPtr not incremented so can for new service when power restored)
    {
        if (!ZeroPowerNoRepeatShuttleOrNewServiceMessage)
            TrainController->StopTTClockMessage(90,
            HeadCode + ": A train without power can't form a new service");
        ...
        // finished all repeats
    {
        AnsiString NewHeadCode =
            ActionVectorEntryPtr->NonRepeatingShuttleLinkHeadCode;
        LogAction(17, HeadCode, NewHeadCode, NewService,
            ActionVectorEntryPtr->LocationName, ActionVectorEntryPtr->EventTime,
            ActionVectorEntryPtr->Warning);
        RepeatNumber = 0;
        UnplotTrain(6);
        ...
        StartSpeed = 0;
        TrainDataEntryPtr =
            ActionVectorEntryPtr->NonRepeatingShuttleLinkEntryPtr;
        TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber).TrainID =
            TrainID; // but note that RepeatNumber = 0
        TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
            .RunningEntry = Running; // but note that RepeatNumber = 0
        ActionVectorEntryPtr = &(TrainDataEntryPtr->ActionVector.at(0));
        HeadCode = NewHeadCode;
        StoppedAtLocation = true;
        PlotStartPosition(9);
        PlotTrainWithNewBackgroundColour(24, clStationStopBackground,
            Display); // pale green
        ActionVectorEntryPtr++;
        LastActionTime = TrainController->TTClockTime;
        TerminatedMessageSent = false;
    ...
    // need the next repeat value in order to obtain a correct NewHeadCode, but don't increase it
    // until after LogAction or the wrong time will be used
    AnsiString NewHeadCode = TrainController->GetRepeatHeadCode(7,
        ActionVectorEntryPtr->OtherHeadCode, TempRepeatNumber,
        IncrementalDigits);
    LogAction(18, HeadCode, NewHeadCode, NewService,
        ActionVectorEntryPtr->LocationName, ActionVectorEntryPtr->EventTime,
        ActionVectorEntryPtr->Warning);
    RepeatNumber++;
    UnplotTrain(7);
...
bool TTrain::IsTrainTerminating(int Caller) {
    ...
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",IsTrainTerminating" + "," + HeadCode);
    for (unsigned int x = 1; x < TrainDataEntryPtr->ActionVector.size(); x++) {
        if ((ActionVectorEntryPtr + x) <
            &TrainDataEntryPtr->ActionVector.back()) {
            if (((ActionVectorEntryPtr + x)->Command == "Fer") ||
                ((ActionVectorEntryPtr + x)->FormatType == TimeLoc)) {
...
bool TTrain::AbleToMove(int Caller) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",AbleToMove" + "," + HeadCode);
    bool Able = true;
    if (Crashed || Derailed || StoppedAtBuffers || StoppedAtSignal ||
        StoppedWithoutPower) // StoppedWithoutPower added at v2.4.0 &
    ...
    if (LeadElement > -1) {
        int FrontPos = Track->TrackElementAt(678, LeadElement)
            .Conn[LeadExitPos];
        int FrontEntryPos = Track->TrackElementAt(679, LeadElement)
            .ConnLinkPos[LeadExitPos];
        if ((FrontPos > -1) && (TrainMode == Signaller)
            && StoppedForTrainInFront) {
            TTrackElement TrackElement = Track->TrackElementAt(680, FrontPos);
            if ((TrackElement.TrackType != Bridge) &&
                (TrackElement.TrainIDOnElement == -1)) {
                Able = true;
                StoppedForTrainInFront = false;
            }
            else if ((TrackElement.TrackType == Bridge) && (FrontEntryPos < 2)
                && (TrackElement.TrainIDOnBridgeTrackPos01 == -1)) {
                Able = true;
                StoppedForTrainInFront = false;
            }
            else if ((TrackElement.TrackType == Bridge) && (FrontEntryPos > 1)
                && (TrackElement.TrainIDOnBridgeTrackPos23 == -1)) {
                Able = true;
                StoppedForTrainInFront = false;
            }
        }
...
bool TTrain::AbleToMoveButForSignal(int Caller) {
    ...
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",AbleToMoveButForSignal" + "," + HeadCode);
    ...
    int NextEntryPos = Track->TrackElementAt(655, LeadElement)
        .ConnLinkPos[LeadExitPos];
    if (Track->OtherTrainOnTrack(5, VecPos, NextEntryPos, TrainID)) {
        ...
        return StoppedAtSignal && !(Crashed || Derailed || StoppedAtBuffers ||
            StoppedForTrainInFront || StoppedWithoutPower);
...
void TTrain::SignallerChangeTrainDirection(int Caller) {
    ...
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",SignallerChangeTrainDirection" + "," + HeadCode);
    TColor TempColour = BackgroundColour;
    UnplotTrain(8);
    RearStartElement = LeadElement;
    RearStartExitPos = LeadEntryPos;
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + "," + AnsiString
        (Ptr - &TrainDataEntryPtr->ActionVector.front()) +
    ...
    if (Ptr->FormatType == TimeTimeLoc) {
        if (TrainMode == Timetable) {
            if (!TrainAtLocation(0, LocationName) ||
                (LocationName != Ptr->LocationName))
                // not arrived yet in tt mode
            {
                RetStr = "Arrive " + Ptr->LocationName + " at " +
                    Utilities->Format96HHMM(GetTrainTime(2, Ptr->ArrivalTime));
            }
            else {
                RetStr = "Depart " + Ptr->LocationName + " at " +
                    Utilities->Format96HHMM
                    (GetTrainTime(3, Ptr->DepartureTime));
            }
        }
        else // TrainMode == Signaller
        {
            ...
            {
                RetStr = "Arrive " + Ptr->LocationName + " at " +
                    Utilities->Format96HHMM(GetTrainTime(45, Ptr->ArrivalTime));
            }
            else {
                RetStr = "Depart " + Ptr->LocationName + " at " +
                    Utilities->Format96HHMM
                    (GetTrainTime(36, Ptr->DepartureTime));
            }
        }
    }
    else if ((Ptr->FormatType == TimeLoc) &&
        (Ptr->ArrivalTime != TDateTime(-1))) {
        RetStr = "Arrive " + Ptr->LocationName + " at " +
            Utilities->Format96HHMM(GetTrainTime(4, Ptr->ArrivalTime));
    }
    else if ((Ptr->FormatType == TimeLoc) &&
        (Ptr->ArrivalTime == TDateTime(-1))) {
        RetStr = "Depart " + Ptr->LocationName + " at " +
            Utilities->Format96HHMM(GetTrainTime(5, Ptr->DepartureTime));
    }
    else if (Ptr->FormatType == PassTime) // new
    {
        RetStr = "Pass " + Ptr->LocationName + " at " + Utilities->Format96HHMM
            (GetTrainTime(31, Ptr->EventTime));
    }
    else if (Ptr->Command == "Fns") {
        RetStr = "Forms new service " + TrainController->GetRepeatHeadCode(8,
            Ptr->OtherHeadCode, RepeatNumber, IncrementalDigits) + " at " +
            Ptr->LocationName + " at " + Utilities->Format96HHMM
            (GetTrainTime(6, Ptr->EventTime));
    }
    else if (Ptr->Command == "F-nshs") {
        RetStr = "Forms new service " + Ptr->NonRepeatingShuttleLinkHeadCode +
            " at " + Ptr->LocationName + " at " + Utilities->Format96HHMM
            (GetTrainTime(32, Ptr->EventTime));
    }
    else if ((Ptr->Command == "Fns-sh") && (RepeatNumber <
        (TrainDataEntryPtr->NumberOfTrains - 1))) // not last repeat number
    {
        RetStr = "Forms new service " + TrainController->GetRepeatHeadCode(9,
            Ptr->OtherHeadCode, RepeatNumber + 1, IncrementalDigits) + " at " +
            Ptr->LocationName + " at " + Utilities->Format96HHMM
            (GetTrainTime(7, Ptr->EventTime));
        // use RepeatNumber+1 as it's the repeat number of the NEXT shuttle service that is relevant
    }
    else if ((Ptr->Command == "Fns-sh") && (RepeatNumber >=
        (TrainDataEntryPtr->NumberOfTrains - 1))) // last repeat number
    {
        RetStr = "Forms new service " + Ptr->NonRepeatingShuttleLinkHeadCode,
        + " at " + Ptr->LocationName + " at " + Utilities->Format96HHMM
            (GetTrainTime(8, Ptr->EventTime));
    }
    else if ((Ptr->Command == "Frh-sh") && (RepeatNumber <
        (TrainDataEntryPtr->NumberOfTrains - 1))) // not last repeat number
    {
        RetStr = "Forms new service " + TrainController->GetRepeatHeadCode(10,
            Ptr->OtherHeadCode, RepeatNumber + 1, IncrementalDigits) + " at " +
            Ptr->LocationName + " at " + Utilities->Format96HHMM
            (GetTrainTime(9, Ptr->EventTime));
        // use RepeatNumber+1 as it's the repeat number of the NEXT shuttle service that is relevant
    }
    else if ((Ptr->Command == "Frh-sh") && (RepeatNumber >=
        (TrainDataEntryPtr->NumberOfTrains - 1))) // last repeat number
    {
        ...
        RetStr = "Exit railway" + TrainController->GetExitLocationAndAt(1,
            Ptr->ExitList) + " at " + Utilities->Format96HHMM
            (GetTrainTime(10, Ptr->EventTime));
    }
    else if (Ptr->Command == "Fjo") {
        RetStr = "Join " + TrainController->GetRepeatHeadCode(11,
            Ptr->OtherHeadCode, RepeatNumber, IncrementalDigits) + " at " +
            Ptr->LocationName + " at " + Utilities->Format96HHMM
            (GetTrainTime(11, Ptr->EventTime));
    }
    else if (Ptr->Command == "jbo") {
        RetStr = "Joined by " + TrainController->GetRepeatHeadCode(12,
            Ptr->OtherHeadCode, RepeatNumber, IncrementalDigits) + " at " +
            Ptr->LocationName + " at " + Utilities->Format96HHMM
            (GetTrainTime(12, Ptr->EventTime));
    }
    else if (Ptr->Command == "fsp") {
        RetStr = "Front split to " + TrainController->GetRepeatHeadCode(13,
            Ptr->OtherHeadCode, RepeatNumber, IncrementalDigits) + " at " +
            Ptr->LocationName + " at " + Utilities->Format96HHMM
            (GetTrainTime(13, Ptr->EventTime));
    }
    else if (Ptr->Command == "rsp") {
        RetStr = "Rear split to " + TrainController->GetRepeatHeadCode(14,
            Ptr->OtherHeadCode, RepeatNumber, IncrementalDigits) + " at " +
            Ptr->LocationName + " at " + Utilities->Format96HHMM
            (GetTrainTime(14, Ptr->EventTime));
    }
    else if (Ptr->Command == "cdt") {
        RetStr = "Change direction at " + Ptr->LocationName + " at " +
            Utilities->Format96HHMM(GetTrainTime(15, Ptr->EventTime));
    }
    ...
    // Enter with Ptr pointing to first action to be listed (i.e. next action)
{
    ...
    do {
        ...
        if ((Ptr->FormatType == TimeTimeLoc) && FirstPass) {
            AnsiString TrainLoc = "";
            if (TrainMode == Timetable) {
                if (TrainAtLocation(1, TrainLoc) &&
                    (TrainLoc == Ptr->LocationName)) {
                    PartStr =
                        Utilities->Format96HHMM
                        (GetTrainTime(33, Ptr->DepartureTime)) +
                        ": Depart from " + Ptr->LocationName;
                }
                else if (Ptr->ArrivalTime == Ptr->DepartureTime) {
                    PartStr =
                        Utilities->Format96HHMM
                        (GetTrainTime(34, Ptr->ArrivalTime)) +
                        ": Arrive & depart from " + Ptr->LocationName;
                }
                else {
                    PartStr =
                        Utilities->Format96HHMM
                        (GetTrainTime(16, Ptr->ArrivalTime)) + ": Arrive at " +
                        Ptr->LocationName + '\n' + Utilities->Format96HHMM
                        (GetTrainTime(17, Ptr->DepartureTime)) +
                        ": Depart from " + Ptr->LocationName;
            ...
            else // TrainMode == Signaller
            {
                if (DepartureTimeSet) {
                    PartStr =
                        Utilities->Format96HHMM
                        (GetTrainTime(37, Ptr->DepartureTime)) +
                        ": Depart from " + Ptr->LocationName;
                }
                else if (Ptr->ArrivalTime == Ptr->DepartureTime) {
                    PartStr =
                        Utilities->Format96HHMM
                        (GetTrainTime(38, Ptr->ArrivalTime)) +
                        ": Arrive & depart from " + Ptr->LocationName;
                }
                else {
                    PartStr =
                        Utilities->Format96HHMM
                        (GetTrainTime(39, Ptr->ArrivalTime)) + ": Arrive at " +
                        Ptr->LocationName + '\n' + Utilities->Format96HHMM
                        (GetTrainTime(40, Ptr->DepartureTime)) +
                        ": Depart from " + Ptr->LocationName;
                    Count++; // because there are 2 entries
                }
            }
            ...
            AnsiString TrainLoc = "";
            if ((TrainAtLocation(2, TrainLoc)) &&
                (TrainLoc == Ptr->LocationName)) {
                PartStr =
                    Utilities->Format96HHMM
                    (GetTrainTime(41, Ptr->DepartureTime)) + ": Depart from " +
                    Ptr->LocationName;
            }
            else if (Ptr->ArrivalTime == Ptr->DepartureTime) {
                PartStr =
                    Utilities->Format96HHMM(GetTrainTime(42, Ptr->ArrivalTime))
                    + ": Arrive & depart from " + Ptr->LocationName;
            }
            else {
                PartStr =
                    Utilities->Format96HHMM(GetTrainTime(43, Ptr->ArrivalTime))
                    + ": Arrive at " + Ptr->LocationName + '\n' +
                    Utilities->Format96HHMM
                    (GetTrainTime(44, Ptr->DepartureTime)) + ": Depart from " +
                    Ptr->LocationName;
                Count++; // because there are 2 entries
            }
        }
        else if ((Ptr->FormatType == TimeLoc) && (Ptr->ArrivalTime != TDateTime
            (-1))) {
            PartStr = Utilities->Format96HHMM
                (GetTrainTime(18, Ptr->ArrivalTime)) + ": Arrive at " +
                Ptr->LocationName;
        }
        else if ((Ptr->FormatType == TimeLoc) && (Ptr->ArrivalTime == TDateTime
            (-1))) {
            PartStr = Utilities->Format96HHMM
                (GetTrainTime(19, Ptr->DepartureTime)) + ": Depart from " +
                Ptr->LocationName;
        }
        else if (Ptr->FormatType == PassTime) // new
        {
            PartStr = Utilities->Format96HHMM(GetTrainTime(30, Ptr->EventTime))
                + ": Pass " + Ptr->LocationName;
        }
        else if (Ptr->Command == "Fns") {
            PartStr = Utilities->Format96HHMM(GetTrainTime(20, Ptr->EventTime))
                + ": Form new service " + TrainController->GetRepeatHeadCode(15,
                Ptr->OtherHeadCode, RepeatNumber, IncrementalDigits) + " at " +
                Ptr->LocationName;
        }
        else if (Ptr->Command == "F-nshs") {
            PartStr = Utilities->Format96HHMM(GetTrainTime(35, Ptr->EventTime))
                + ": Form new service " + Ptr->NonRepeatingShuttleLinkHeadCode +
                " at " + Ptr->LocationName;
        }
        else if ((Ptr->Command == "Fns-sh") && (RepeatNumber <
            (TrainDataEntryPtr->NumberOfTrains - 1)))
            // not the last repeat number
        {
            PartStr = Utilities->Format96HHMM(GetTrainTime(21, Ptr->EventTime))
                + ": Form new service " + TrainController->GetRepeatHeadCode(16,
            ...
            (TrainDataEntryPtr->NumberOfTrains - 1))) // last repeat number
        {
            PartStr = Utilities->Format96HHMM(GetTrainTime(22, Ptr->EventTime))
                + ": Form new service " + Ptr->NonRepeatingShuttleLinkHeadCode,
            + " at " + Ptr->LocationName;
        }
        else if ((Ptr->Command == "Frh-sh") && (RepeatNumber <
            ...
            // not the last repeat number
        {
            PartStr = Utilities->Format96HHMM(GetTrainTime(23, Ptr->EventTime))
                + ": Form new service " + TrainController->GetRepeatHeadCode(17,
                Ptr->OtherHeadCode, RepeatNumber + 1, IncrementalDigits) +
                " at " + Ptr->LocationName;
            // use RepeatNumber+1 because it's the repeat number of the NEXT shuttle service that is relevant
        }
            ...
            (TrainDataEntryPtr->NumberOfTrains - 1))) // last repeat number
        {
            ...
            PartStr = Utilities->Format96HHMM(GetTrainTime(24, Ptr->EventTime))
                + ": Exit railway" + TrainController->GetExitLocationAndAt(2,
                Ptr->ExitList);
        }
        else if (Ptr->Command == "Fjo") {
            PartStr = Utilities->Format96HHMM(GetTrainTime(25, Ptr->EventTime))
                + ": Join " + TrainController->GetRepeatHeadCode(18,
                Ptr->OtherHeadCode, RepeatNumber, IncrementalDigits) + " at " +
                Ptr->LocationName;
        }
        else if (Ptr->Command == "jbo") {
            PartStr = Utilities->Format96HHMM(GetTrainTime(26, Ptr->EventTime))
                + ": Joined by " + TrainController->GetRepeatHeadCode(19,
                Ptr->OtherHeadCode, RepeatNumber, IncrementalDigits) + " at " +
                Ptr->LocationName;
        }
        else if (Ptr->Command == "fsp") {
            PartStr = Utilities->Format96HHMM(GetTrainTime(27, Ptr->EventTime))
                + ": Front split to " + TrainController->GetRepeatHeadCode(20,
                Ptr->OtherHeadCode, RepeatNumber, IncrementalDigits) + " at " +
                Ptr->LocationName;
        }
        else if (Ptr->Command == "rsp") {
            PartStr = Utilities->Format96HHMM(GetTrainTime(28, Ptr->EventTime))
                + ": Rear split to " + TrainController->GetRepeatHeadCode(21,
                Ptr->OtherHeadCode, RepeatNumber, IncrementalDigits) + " at " +
                Ptr->LocationName;
        }
        else if (Ptr->Command == "cdt") {
            PartStr = Utilities->Format96HHMM(GetTrainTime(29, Ptr->EventTime))
                + ": Change direction at " + Ptr->LocationName;
        }
        if (RetStr != "")
            RetStr = RetStr + '\n' + PartStr;
        else
    ...
    while (!TimetableFinished && (Count < 33) && ((Ptr->Command == "") ||
        ((Ptr->Command != "") && (Ptr->Command[1] != 'F'))));
...
void TTrain::SaveOneSessionTrain(int Caller, std::ofstream &OutFile) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",SaveOneSessionTrain" + "," + HeadCode);
    ...
    int TrainDataEntryValue = TrainDataEntryPtr -
        &(TrainController->TrainDataVector.at(0));
    Utilities->SaveFileInt(OutFile, TrainDataEntryValue);
    int ActionVectorEntryValue = ActionVectorEntryPtr -
        &(TrainDataEntryPtr->ActionVector.at(0));
...
void TTrain::LoadOneSessionTrain(int Caller, std::ifstream &InFile) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",LoadOneSessionTrain"); // don't have headcode yet
    ...
    if (TrainMode == Timetable) {
        if (Crashed)
            RailGraphics->ChangeForegroundColour2(0, HeadCodePosition[0],
            FrontCodePtr, clB0G0R0, BackgroundColour);
        else
            RailGraphics->ChangeForegroundColour2(1, HeadCodePosition[0],
            FrontCodePtr, clFrontCodeTimetable, BackgroundColour);
    }
    else
        RailGraphics->ChangeForegroundColour2(2, HeadCodePosition[0],
        FrontCodePtr, clFrontCodeSignaller, BackgroundColour);
    HeadCodePosition[0] = FrontCodePtr;
    // pick up background bitmaps, none if MidLag as no train plotted - entering at continuation
    if (Straddle == LeadMid) {
        if (LeadElement > -1)
            PickUpBackgroundBitmap(6, HOffset[0], VOffset[0], LeadElement,
            LeadEntryPos, BackgroundPtr[0]);
        if (LeadElement > -1)
            PickUpBackgroundBitmap(7, HOffset[1], VOffset[1], LeadElement,
            LeadEntryPos, BackgroundPtr[1]);
        if (MidElement > -1)
            PickUpBackgroundBitmap(8, HOffset[2], VOffset[2], MidElement,
            MidEntryPos, BackgroundPtr[2]);
        if (MidElement > -1)
            PickUpBackgroundBitmap(9, HOffset[3], VOffset[3], MidElement,
            MidEntryPos, BackgroundPtr[3]);
    }
    else if (Straddle == LeadMidLag) {
        if (LeadElement > -1)
            PickUpBackgroundBitmap(10, HOffset[0], VOffset[0], LeadElement,
            LeadEntryPos, BackgroundPtr[0]);
        if (MidElement > -1)
            PickUpBackgroundBitmap(11, HOffset[1], VOffset[1], MidElement,
            MidEntryPos, BackgroundPtr[1]);
        if (MidElement > -1)
            PickUpBackgroundBitmap(12, HOffset[2], VOffset[2], MidElement,
            MidEntryPos, BackgroundPtr[2]);
        if (LagElement > -1)
            PickUpBackgroundBitmap(13, HOffset[3], VOffset[3], LagElement,
            LagEntryPos, BackgroundPtr[3]);
    }

    int TrainDataEntryValue = Utilities->LoadFileInt(InFile);
    TrainDataEntryPtr = &(TrainController->TrainDataVector.at(0)) +
        TrainDataEntryValue;
    int ActionVectorEntryValue = Utilities->LoadFileInt(InFile);
    ActionVectorEntryPtr = &(TrainDataEntryPtr->ActionVector.at(0)) +
        ActionVectorEntryValue;
        ...
        // need to include this in case train exiting & no lead element
    {
        if (Track->TrackElementAt(667, LeadElement).TrackType == Continuation) {
            Track->TrackElementAt(668, LeadElement).TrainIDOnElement =
                TrainID; // no need to stop gap flashing if a continuation
        }
    }
    AValue = sqrt(2 * PowerAtRail / Mass);
    ...
    {
        AnsiString Location = LocationAndMarker.SubString(1,
            LocationAndMarker.Length() - 6);
        bool GiveMessagesFalse = false;
        bool CheckLocationsExistInRailwayTrue = true;
        if (TrainController->CheckLocationValidity(3, Location,
            GiveMessagesFalse, CheckLocationsExistInRailwayTrue))
        { // otherwise take no action
            RestoreTimetableLocation = Location;
        }
    }
    AnsiString Marker = LocationAndMarker.SubString
        (LocationAndMarker.Length() - 5, 6);
...
void TTrain::PlotTrainInZoomOutMode(int Caller, bool Flash) {
    ...
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",PlotTrainInZoomOutMode" + "," + HeadCode);
    ...
    if ((LeadElement > -1) && (!HideFlashingTrain || Flash)) {
        Display->PlotSmallOutput(4, Track->TrackElementAt(441,
            LeadElement).HLoc * 4, Track->TrackElementAt(442,
            LeadElement).VLoc * 4, SmallTrainBitmap);
    }
    if ((MidElement > -1) && (!HideFlashingTrain || Flash)) {
        Display->PlotSmallOutput(5, Track->TrackElementAt(443,
            MidElement).HLoc * 4, Track->TrackElementAt(444,
            MidElement).VLoc * 4, SmallTrainBitmap);
    }
    if ((LeadElement == -1) && (LagElement > -1) && (!HideFlashingTrain ||
        Flash)) {
        Display->PlotSmallOutput(6, Track->TrackElementAt(445,
            LagElement).HLoc * 4, Track->TrackElementAt(446,
            LagElement).VLoc * 4, SmallTrainBitmap);
    }
...
void TTrain::UnplotTrainInZoomOutMode(int Caller) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",UnplotTrainInZoomOutMode," + AnsiString(TrainID)
        + "," + HeadCode);
    if (!Display->ZoomOutFlag) {
    ...
    for (int y = 0; y < 3; y++) {
        if (OldZoomOutElement[y] > -1) {
            ...
            TTrackElement ATElement =
                Track->TrackElementAt(717, OldZoomOutElement[y]);
            TTrackElement IATElement1, IATElement2;
            // default elements to begin with
            Display->PlotSmallOutput(7, ATElement.HLoc * 4, ATElement.VLoc * 4,
                RailGraphics->smSolidBgnd); // plot the blank
            TTrack::TIMPair IMPair =
                Track->GetVectorPositionsFromInactiveTrackMap(14,
                ATElement.HLoc, ATElement.VLoc, FoundFlag);
            ...
            if (FoundFlag) {
                ...
                Display->PlotSmallOutput(8, IATElement1.HLoc * 4,
                    IATElement1.VLoc * 4, IATElement1.SmallGraphicPtr);
                if (IMPair.first != IMPair.second) {
                    IATElement2 = Track->InactiveTrackElementAt(88,
                        IMPair.second);
                    Display->PlotSmallOutput(9, IATElement2.HLoc * 4,
                        IATElement2.VLoc * 4, IATElement2.SmallGraphicPtr);
                }
            }
            Display->PlotSmallOutput(10, ATElement.HLoc * 4, ATElement.VLoc * 4,
                ATElement.SmallGraphicPtr);
        }
    }
...
bool TTrain::TrainAtLocation(int Caller, AnsiString &LocationName) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",TrainAtLocation" + "," + HeadCode);
    LocationName = "";
    if (!StoppedAtLocation) {
    ...
    if (LeadElement > -1) {
        LocationName = Track->TrackElementAt(681, LeadElement)
            .ActiveTrackElementName;
    }
    if ((LocationName == "") && (MidElement > -1)) {
        LocationName = Track->TrackElementAt(682, MidElement)
            .ActiveTrackElementName;
    }
    if ((LocationName == "") && (LagElement > -1)) {
        LocationName = Track->TrackElementAt(683, LagElement)
            .ActiveTrackElementName;
    }
    if (LocationName == "") {
...
void TTrain::PlotTrain(int Caller, TDisplay *Disp) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",PlotTrain" + "," + HeadCode);
    for (int x = 0; x < 4; x++) {
...
void TTrain::WriteTrainToImage(int Caller, Graphics::TBitmap *Bitmap) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",WriteTrainToImage" + "," + HeadCode);
    for (int x = 0; x < 4; x++) {
        if (PlotElement[x] > -1) {
            Bitmap->Canvas->Draw
                (((Track->TrackElementAt(744, PlotElement[x]).HLoc -
                Track->GetHLocMin()) * 16 + HOffset[x]),
                ((Track->TrackElementAt(745, PlotElement[x]).VLoc -
                Track->GetVLocMin()) * 16 + VOffset[x]), HeadCodePosition[x]);
        }
    }
...
bool TTrain::LinkOccupied(int Caller, int TrackVectorPosition,
    int LinkNumber) // added at v1.2.0
{
    ...
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",LinkOccupied," + AnsiString(TrackVectorPosition)
    ...
    if ((MidElement == TrackVectorPosition) && ((Track->TrackElementAt(883,
        TrackVectorPosition).Link[MidEntryPos] == LinkNumber) ||
        (Track->TrackElementAt(884, TrackVectorPosition)
        .Link[MidExitPos] == LinkNumber))) {
        Utilities->CallLogPop(2005);
        return true;
    }
    if (Straddle == LeadMid) {
        if ((LeadElement == TrackVectorPosition) && ((Track->TrackElementAt(885,
            TrackVectorPosition).Link[LeadEntryPos] == LinkNumber) ||
            (Track->TrackElementAt(886,
            TrackVectorPosition).Link[LeadExitPos] == LinkNumber))) {
            Utilities->CallLogPop(2006);
            return true;
        }
    }
    else if (Straddle == LeadMidLag) {
        if ((LeadElement == TrackVectorPosition) && (Track->TrackElementAt(887,
            TrackVectorPosition).Link[LeadEntryPos] == LinkNumber))
        ...
        else if ((LagElement == TrackVectorPosition) &&
            (Track->TrackElementAt(888, TrackVectorPosition)
            .Link[LagExitPos] == LinkNumber))
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CalcTimeToAct, " + HeadCode);
    ...
    if (!Stopped() || StoppedAtLocation)
    {
            ...
            // if -1 it's on an end element so no action needed
        {
            ...
            int FirstPosToBeMeasured = Track->TrackElementAt(953, LeadElement)
                .Conn[LeadExitPos];
            int FirstEntryPos = Track->TrackElementAt(954, LeadElement)
                .ConnLinkPos[LeadExitPos];
            ...
            bool SigControlAndCanPassRedSignal =
                ((TrainMode == Signaller) && AllowedToPassRedSignal);
            DistanceToRedSignal =
                TrainController->CalcDistanceToRedSignalandStopTime(0,
                FirstPosToBeMeasured, FirstEntryPos,
                SigControlAndCanPassRedSignal, ActionVectorEntryPtr, HeadCode,
                TrainID, CurrentStopTime, LaterStopTime, RecoverableTime,
                AvTrackSpeed);
            ...
            else // on time or running late
            {
                ...
                // if not stopped no stop times to add
                    ...
                    TotalStopTime = CurrentStopTime + LaterStopTime -
                        TimeToSubtract; // later stops so deduct as much as can
            }
            ...
            TimeToAct = TotalStopTime +
                DistanceToRedSignal * 3.6 / 60 / Speed;
    ...
    else // stopped not at location
    {
        ...
        if (StoppedAtSignal) {
            int NextElement = Track->TrackElementAt(928, LeadElement)
                .Conn[LeadExitPos];
            int NextEntryPos = Track->TrackElementAt(929, LeadElement)
                .ConnLinkPos[LeadExitPos];
            int NextExitPos;
            ...
            int NextButOneElement = Track->TrackElementAt(932, NextElement)
                .Conn[NextExitPos];
            int NextButOneEntryPos = Track->TrackElementAt(933, NextElement)
                .ConnLinkPos[NextExitPos];
            int RouteNumber; // holder for referenced value, not used
            if (AllRoutes->GetRouteTypeAndNumber(32, NextButOneElement,
                NextButOneEntryPos, RouteNumber) == TAllRoutes::AutoSigsRoute) {
                TimeToAct = -1;
            }
        }
...
bool TTrain::TrainOnContinuation(int Caller) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",TrainOnContinuation, " + HeadCode);
    if (LeadElement > -1) {
...
void TTrainController::Operate(int Caller) {
    ...
    if (!Track->RouteFlashFlag && !Track->PointFlashFlag) {
        for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
            ...
            if (AVEntry0.Command == "Snt") {
                ...
                const TActionVectorEntry &AVEntryLast =
                    TDEntry.ActionVector.at(TDEntry.ActionVector.size() - 1);
                ...
                if ((AVEntryLast.FormatType == Repeat) &&
                    (TDEntry.NumberOfTrains < 2)) {
                    throw Exception
                        ("Error - Repeat entry && less than two trains for Snt entry: " +
                        TDEntry.HeadCode);
                }
                // see above note
                for (int y = 0; y < TDEntry.NumberOfTrains; y++) {
                    TTrainOperatingData &TTOD =
                        TDEntry.TrainOperatingDataVector.at(y);
                    if (TTOD.RunningEntry != NotStarted)
                        continue;
                    if (GetRepeatTime(2, AVEntry0.EventTime, y,
                        IncrementalMinutes) > TTClockTime)
                        break; // all the rest will also be greater
                    AnsiString TrainHeadCode =
                        GetRepeatHeadCode(22, TDEntry.HeadCode, y,
                        IncrementalDigits);
                    if (AddTrain(2, AVEntry0.RearStartOrRepeatMins,
                        AVEntry0.FrontStartOrRepeatDigits, TrainHeadCode,
                        TDEntry.StartSpeed, TDEntry.Mass,
                        TDEntry.MaxRunningSpeed, TDEntry.MaxBrakeRate,
                        TDEntry.PowerAtRail, "Timetable", &TDEntry, y,
                        IncrementalMinutes, IncrementalDigits,
                        TDEntry.SignallerSpeed, AVEntry0.SignallerControl)) {
                ...
                // just start this once, shuttle repeats take care of restarts
            {
                ...
                if ((AVEntryLast.FormatType == Repeat) &&
                    (TDEntry.NumberOfTrains < 2)) {
                    throw Exception
                        ("Error - Repeat entry && less than two trains for Snt-sh entry: " +
                        TDEntry.HeadCode);
                }
                // see above note
                TTrainOperatingData &TTOD =
                    TDEntry.TrainOperatingDataVector.at(0);
                if (TTOD.RunningEntry == NotStarted) {
                    if (AVEntry0.EventTime <= TTClockTime) {
                        if (AddTrain(3, AVEntry0.RearStartOrRepeatMins,
                            AVEntry0.FrontStartOrRepeatDigits, TDEntry.HeadCode,
                            TDEntry.StartSpeed, TDEntry.Mass,
                            TDEntry.MaxRunningSpeed, TDEntry.MaxBrakeRate,
                            TDEntry.PowerAtRail, "Timetable", &TDEntry, 0,
                            IncrementalMinutes, IncrementalDigits,
                            TDEntry.SignallerSpeed, false))
                            // false for SignallerControl
                        {
                            TTOD.TrainID = TrainVector.back().TrainID;
                            TTOD.RunningEntry = Running;
    ...
    if (!TrainVector.empty()) {
        ...
        for (int x = TrainVector.size() - 1; x >= 0;
        x--) // reverse because of erase
        {
            ...
            if (Train.HasTrainGone()) {
                ...
                if (ElementFound) {
                    if (TE.ActiveTrackElementName != "") {
                        Loc = TE.ActiveTrackElementName + ", track element " +
                            TE.ElementID;
                    }
                    else {
                        Loc = "track element " + TE.ElementID;
                    ...
                    // need above first because may also have ActionVectorEntryPtr == "Fer"
                {
                    ...
                    if (SignallerTrainRemovedOnAutoSigsRoute) {
                        AllRoutes->SignallerRemovedTrainAutoRoute.
                            SetRouteSignals(9);
                        SignallerTrainRemovedOnAutoSigsRoute = false;
                    }
                    // end of addition
                    ...
                    if (!AVEntryPtr->ExitList.empty()) {
                        for (TExitListIterator ELIT =
                            AVEntryPtr->ExitList.begin();
                        ELIT != AVEntryPtr->ExitList.end(); ELIT++) {
                            if (*ELIT == Train.LagElement)
                                CorrectExit = true;
                        }
                    }
                    if (CorrectExit) {
                        Train.LogAction(19, Train.HeadCode, "", Leave, Loc,
                            AVEntryPtr->EventTime, AVEntryPtr->Warning);
                    }
                    else {
                        LogActionError(38, Train.HeadCode, "",
                            FailIncorrectExit, Loc);
                    }
                }
                else {
                    if (!AVEntryPtr->SignallerControl) {
                        LogActionError(26, Train.HeadCode, "",
                            FailUnexpectedExitRailway, Loc);
                        Train.SendMissedActionLogs(2, -2, AVEntryPtr);
                        // -2 is marker for send messages for all remaining actions except Fer if present
                    }
                    else {
                        Train.LogAction(31, Train.HeadCode, "", SignallerLeave,
                            Loc, TDateTime(0), false); // false for Warning
                    }
                }
                Train.TrainDataEntryPtr->TrainOperatingDataVector.at
                    (Train.RepeatNumber).RunningEntry = Exited;
                Train.DeleteTrain(1);
    ...
    if ((OpTimeToActUpdateCounter == 0)
        && TrainController->OpActionPanelVisible) // new v2.2.0
    {
...
void TTrainController::FinishedOperation(int Caller) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",FinishedOperation");
    if (!TrainVector.empty()) {
...
void TTrainController::ReplotTrains(int Caller, TDisplay *Disp) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",ReplotTrains");
    if (!TrainVector.empty()) {
        for (unsigned int x = 0; x < TrainVector.size(); x++) {
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",WriteTrainsToImage");
    if (!TrainVector.empty()) {
        for (unsigned int x = 0; x < TrainVector.size(); x++) {
...
void TTrainController::UnplotTrains(int Caller) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",UnplotTrains");
    if (!TrainVector.empty()) {
        for (unsigned int x = 0; x < TrainVector.size(); x++) {
...
bool TTrainController::AddTrain(int Caller, int RearPosition, int FrontPosition,
    AnsiString HeadCode, int StartSpeed, int Mass, double MaxRunningSpeed,
    double MaxBrakeRate, double PowerAtRail, AnsiString ModeStr,
    TTrainDataEntry *TrainDataEntryPtr, int RepeatNumber,
    int IncrementalMinutes, int IncrementalDigits, int SignallerSpeed,
...
{
    LogEvent("" + AnsiString(Caller) + ",AddTrain," + AnsiString(RearPosition) +
        "," + AnsiString(FrontPosition) + "," + HeadCode + "," +
        AnsiString(StartSpeed) + "," + AnsiString(Mass) + "," + ModeStr);
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",AddTrain," + AnsiString(RearPosition) + "," +
        AnsiString(FrontPosition) + "," + HeadCode + "," +
        AnsiString(StartSpeed) + "," + AnsiString(Mass) + "," + ModeStr + "," +
        HeadCode);
    ...
    if (!CheckStartAllowable(0, RearPosition, RearExitPos, HeadCode, ReportFlag,
        EventType))
    { // messages sent to performance log in CheckStartAllowable if ReportFlag true
        TrainDataEntryPtr->TrainOperatingDataVector.at(RepeatNumber)
            .EventReported = EventType;
    ...
    TTrain *NewTrain = new TTrain(0, RearPosition, RearExitPos, HeadCode,
        StartSpeed, Mass, MaxRunningSpeed, MaxBrakeRate, PowerAtRail, TrainMode,
        TrainDataEntryPtr, RepeatNumber, IncrementalMinutes, IncrementalDigits,
        SignallerSpeed);
    ...
    if (SignallerControl) {
        ...
        RailGraphics->ChangeForegroundColour(17, NewTrain->HeadCodePosition[0],
            NewTrain->FrontCodePtr, clFrontCodeSignaller,
            NewTrain->BackgroundColour);
    }
        ...
        // wouldn't have accepted the timetable
    {
            ...
            // buffer end must be ahead of train or would have failed start position check
        {
            ...
            NewTrain->PlotTrainWithNewBackgroundColour(13,
                clStationStopBackground, Display); // pale green
            NewTrain->LogAction(20, NewTrain->HeadCode, "", Create,
                NewTrain->ActionVectorEntryPtr->LocationName,
                NewTrain->ActionVectorEntryPtr->EventTime,
        ...
        // else a through station stop
            ...
            NewTrain->PlotTrainWithNewBackgroundColour(18,
                clStationStopBackground, Display); // pale green
            NewTrain->LogAction(21, NewTrain->HeadCode, "", Create,
                NewTrain->ActionVectorEntryPtr->LocationName,
                NewTrain->ActionVectorEntryPtr->EventTime,
                NewTrain->ActionVectorEntryPtr->Warning);
            if (!SignallerControl) // don't advance if SignalControlEntry
    ...
    else // unlocated entry (i.e. not a stop entry, but could still be at a named location)
    {
        ...
        TTrackElement TE = Track->TrackElementAt(530,
            NewTrain->RearStartElement);
        AnsiString Loc = "";
        if (TE.ActiveTrackElementName != "") {
        ...
        if (TE.TrackType == Continuation) {
            NewTrain->LogAction(22, NewTrain->HeadCode, "", Enter, Loc,
                NewTrain->ActionVectorEntryPtr->EventTime,
                NewTrain->ActionVectorEntryPtr->Warning);
        }
        else {
            NewTrain->LogAction(23, NewTrain->HeadCode, "", Create, Loc,
                NewTrain->ActionVectorEntryPtr->EventTime,
                NewTrain->ActionVectorEntryPtr->Warning);
        }
        if (!SignallerControl) // don't advance if SignalControlEntry
    ...
    if (NewTrain->LeadElement > -1) {
        NewTrain->CheckAndCancelRouteForWrongEndEntry(3, NewTrain->LeadElement,
            NewTrain->LeadEntryPos);
    }
    if (NewTrain->MidElement > -1) {
        NewTrain->CheckAndCancelRouteForWrongEndEntry(4, NewTrain->MidElement,
            NewTrain->MidEntryPos);
    }
    ...
    if (NewTrain->LeadElement > -1) {
        if (AllRoutes->GetRouteTypeAndNumber(13, NewTrain->LeadElement,
            NewTrain->LeadEntryPos, RouteNumber) == TAllRoutes::AutoSigsRoute)
        { // below added in place of SetRouteSignals in v2.4.0 as don't want to set signals from start of route for a new train addition
            int RouteStartPosition;
            TAllRoutes::TRouteElementPair FirstPair, SecondPair;
            FirstPair = AllRoutes->GetRouteElementDataFromRoute2MultiMap(21,
                Track->TrackElementAt(955, FrontPosition).HLoc,
            ...
            AllRoutes->SetAllRearwardsSignals(10, 0, RouteNumber,
                RouteStartPosition);
            SignalsSet = true;
        ...
        {
            TPrefDirElement TempPDE =
                AllRoutes->GetFixedRouteAt(181, RouteNumber)
                .GetFixedPrefDirElementAt(194, 0);
            int FirstTVPos = TempPDE.GetTrackVectorPosition();
            int FirstELinkPos = TempPDE.GetELinkPos();
            while (TempPDE.GetTrackVectorPosition() != (unsigned int)
                (NewTrain->LeadElement)) {
                AllRoutes->RemoveRouteElement(16, TempPDE.HLoc, TempPDE.VLoc,
                    TempPDE.GetELink());
                TempPDE = AllRoutes->GetFixedRouteAt(182, RouteNumber)
                    .GetFixedPrefDirElementAt(195, 0);
            }
            if (TempPDE.GetTrackVectorPosition() == (unsigned int)
                (NewTrain->LeadElement)) {
                AllRoutes->RemoveRouteElement(17, TempPDE.HLoc, TempPDE.VLoc,
                    TempPDE.GetELink());
            ...
            if (Track->TrackElementAt(820, FirstTVPos).Conn[FirstELinkPos] > -1)
            {
                ...
                if (AllRoutes->GetRouteTypeAndNumber(17,
                    Track->TrackElementAt(821, FirstTVPos).Conn[FirstELinkPos],
                    Track->TrackElementAt(822,
                    FirstTVPos).ConnLinkPos[FirstELinkPos], LinkedRouteNumber)
                    == TAllRoutes::AutoSigsRoute) {
                    AllRoutes->GetFixedRouteAt(169, LinkedRouteNumber)
                        .SetRouteSignals(0);
    ...
    { // this is included in case a train starts with LeadElement on no route and MidElement on a route
        if (!SignalsSet) {
            RouteNumber = -1;
            if (AllRoutes->GetRouteTypeAndNumber(14, NewTrain->MidElement,
                NewTrain->MidEntryPos, RouteNumber)
                == TAllRoutes::AutoSigsRoute)
            { // below added in place of SetRouteSignals in v2.4.0 as don't want to set signals from start of route for a new train addition
                int RouteStartPosition;
                TAllRoutes::TRouteElementPair FirstPair, SecondPair;
                FirstPair = AllRoutes->GetRouteElementDataFromRoute2MultiMap(22,
                    Track->TrackElementAt(957, RearPosition).HLoc,
                ...
                AllRoutes->SetAllRearwardsSignals(11, 0, RouteNumber,
                    RouteStartPosition);
                SignalsSet = true;
                // AllRoutes->GetFixedRouteAt(, RouteNumber).SetRouteSignals();  above substituted in v2.4.0
            }
            ...
            {
                TPrefDirElement TempPDE =
                    AllRoutes->GetFixedRouteAt(184, RouteNumber)
                    .GetFixedPrefDirElementAt(196, 0);
                int FirstTVPos = TempPDE.GetTrackVectorPosition();
                int FirstELinkPos = TempPDE.GetELinkPos();
                while (TempPDE.GetTrackVectorPosition() != (unsigned int)
                    (NewTrain->MidElement)) {
                    AllRoutes->RemoveRouteElement(18, TempPDE.HLoc,
                        TempPDE.VLoc, TempPDE.GetELink());
                    TempPDE = AllRoutes->GetFixedRouteAt(185, RouteNumber)
                        .GetFixedPrefDirElementAt(197, 0);
                }
                if (TempPDE.GetTrackVectorPosition() == (unsigned int)
                    (NewTrain->MidElement)) {
                    AllRoutes->RemoveRouteElement(19, TempPDE.HLoc,
                        TempPDE.VLoc, TempPDE.GetELink());
                    // remove the last element under LeadElement
                }
                AllRoutes->RebuildRailwayFlag = true;
                // to force ClearandRebuildRailway at next clock tick if not in zoom-out mode
                // now deal with a rear linked autosigs route
                if (Track->TrackElementAt(823,
                    FirstTVPos).Conn[FirstELinkPos] > -1) {
                    int LinkedRouteNumber = -1;
                    if (AllRoutes->GetRouteTypeAndNumber(19,
                        Track->TrackElementAt(824,
                        FirstTVPos).Conn[FirstELinkPos],
                        Track->TrackElementAt(825,
                        FirstTVPos).ConnLinkPos[FirstELinkPos],
                        LinkedRouteNumber) == TAllRoutes::AutoSigsRoute) {
                        AllRoutes->GetFixedRouteAt(170, LinkedRouteNumber)
                            .SetRouteSignals(1);
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",EntryPos," + AnsiString(TrainIDIn) + "," +
...
TTrain &TTrainController::TrainVectorAtIdent(int Caller, int TrainID) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",TrainVectorAtIdent," + AnsiString(TrainID));
    for (unsigned int x = 0; x < TrainVector.size(); x++) {
    ...
    throw Exception
        ("Error - No Train identified in TrainVectorAtIdent with ID = " +
        AnsiString(TrainID));
}
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",TrainExistsAtIdent," + AnsiString(TrainID));
    for (unsigned int x = 0; x < TrainVector.size(); x++) {
...
TDateTime TTrainController::GetControllerTrainTime(int Caller, TDateTime Time,
    int RepeatNumber, int IncrementalMinutes) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",GetControllerTrainTime," +
        AnsiString(RepeatNumber) + "," + Utilities->Format96HHMMSS(Time));
    TDateTime RepeatTime = TrainController->GetRepeatTime(47, Time,
        RepeatNumber, IncrementalMinutes);
...
AnsiString TTrainController::ContinuationEntryFloatingTTString(int Caller,
    TTrainDataEntry *TTDEPtr, int RepNum, int IncMins, int IncDig)
    // Enter with Ptr pointing to first action to be listed (i.e. next action)
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",ContinuationEntryFloatingTTString" + "," +
        TTDEPtr->HeadCode);
    AnsiString RetStr = "", PartStr = "";
    int Count = 0;
    ...
    do {
        ...
        if (Ptr->FormatType == TimeTimeLoc) {
            if (Ptr->ArrivalTime == Ptr->DepartureTime) {
                PartStr =
                    Utilities->Format96HHMM(GetControllerTrainTime(0,
                    Ptr->ArrivalTime, RepNum, IncMins)) +
                    ": Arrive & depart from " + Ptr->LocationName;
            }
            else {
                PartStr =
                    Utilities->Format96HHMM(GetControllerTrainTime(1,
                    Ptr->ArrivalTime, RepNum, IncMins)) + ": Arrive at " +
                    Ptr->LocationName + '\n' + Utilities->Format96HHMM
                    (GetControllerTrainTime(2, Ptr->DepartureTime, RepNum,
                    IncMins)) + ": Depart from " + Ptr->LocationName;
                Count++; // because there are 2 entries
            }
        }
        else if ((Ptr->FormatType == TimeLoc) && (Ptr->ArrivalTime != TDateTime
            (-1))) {
            PartStr = Utilities->Format96HHMM(GetControllerTrainTime(3,
                Ptr->ArrivalTime, RepNum, IncMins)) + ": Arrive at " +
                Ptr->LocationName;
        }
        else if ((Ptr->FormatType == TimeLoc) && (Ptr->ArrivalTime == TDateTime
            (-1))) {
            PartStr = Utilities->Format96HHMM(GetControllerTrainTime(4,
                Ptr->DepartureTime, RepNum, IncMins)) + ": Depart from " +
                Ptr->LocationName;
        }
        else if (Ptr->FormatType == PassTime) // new
        {
            PartStr = Utilities->Format96HHMM(GetControllerTrainTime(5,
                Ptr->EventTime, RepNum, IncMins)) + ": Pass " +
                Ptr->LocationName;
        }
        else if (Ptr->Command == "Fns") {
            PartStr = Utilities->Format96HHMM(GetControllerTrainTime(6,
                Ptr->EventTime, RepNum, IncMins)) + ": Form new service " +
                TrainController->GetRepeatHeadCode(46, Ptr->OtherHeadCode,
                RepNum, IncDig) + " at " + Ptr->LocationName;
        }
        else if (Ptr->Command == "F-nshs") {
            PartStr = Utilities->Format96HHMM(GetControllerTrainTime(7,
                Ptr->EventTime, RepNum, IncMins)) + ": Form new service " +
                Ptr->NonRepeatingShuttleLinkHeadCode + " at " +
                Ptr->LocationName;
        }
        else if ((Ptr->Command == "Fns-sh") && (RepNum <
            (TTDEPtr->NumberOfTrains - 1))) // not the last repeat number
        {
            PartStr = Utilities->Format96HHMM(GetControllerTrainTime(8,
                Ptr->EventTime, RepNum, IncMins)) + ": Form new service " +
                TrainController->GetRepeatHeadCode(47, Ptr->OtherHeadCode,
                RepNum + 1, IncDig) + " at " + Ptr->LocationName;
            // use RepNum+1 because it's the repeat number of the NEXT shuttle service that is relevant
        }
        else if ((Ptr->Command == "Fns-sh") && (RepNum >=
            (TTDEPtr->NumberOfTrains - 1))) // last repeat number
        {
            PartStr = Utilities->Format96HHMM(GetControllerTrainTime(9,
                Ptr->EventTime, RepNum, IncMins)) + ": Form new service " +
                Ptr->NonRepeatingShuttleLinkHeadCode,
            + " at " + Ptr->LocationName;
        }
        else if ((Ptr->Command == "Frh-sh") && (RepNum <
            (TTDEPtr->NumberOfTrains - 1))) // not the last repeat number
        {
            PartStr = Utilities->Format96HHMM(GetControllerTrainTime(10,
                Ptr->EventTime, RepNum, IncMins)) + ": Form new service " +
                TrainController->GetRepeatHeadCode(48, Ptr->OtherHeadCode,
                RepNum + 1, IncDig) + " at " + Ptr->LocationName;
            // use RepNum+1 because it's the repeat number of the NEXT shuttle service that is relevant
        }
        else if ((Ptr->Command == "Frh-sh") && (RepNum >=
            (TTDEPtr->NumberOfTrains - 1))) // last repeat number
        {
            ...
            PartStr = Utilities->Format96HHMM(GetControllerTrainTime(11,
                Ptr->EventTime, RepNum, IncMins)) + ": Exit railway" +
                TrainController->GetExitLocationAndAt(3, Ptr->ExitList);
        }
        else if (Ptr->Command == "Fjo") {
            PartStr = Utilities->Format96HHMM(GetControllerTrainTime(12,
                Ptr->EventTime, RepNum, IncMins)) + ": Join " +
                TrainController->GetRepeatHeadCode(49, Ptr->OtherHeadCode,
                RepNum, IncDig) + " at " + Ptr->LocationName;
        }
        else if (Ptr->Command == "jbo") {
            PartStr = Utilities->Format96HHMM(GetControllerTrainTime(13,
                Ptr->EventTime, RepNum, IncMins)) + ": Joined by " +
                TrainController->GetRepeatHeadCode(50, Ptr->OtherHeadCode,
                RepNum, IncDig) + " at " + Ptr->LocationName;
        }
        else if (Ptr->Command == "fsp") {
            PartStr = Utilities->Format96HHMM(GetControllerTrainTime(14,
                Ptr->EventTime, RepNum, IncMins)) + ": Front split to " +
                TrainController->GetRepeatHeadCode(51, Ptr->OtherHeadCode,
                RepNum, IncDig) + " at " + Ptr->LocationName;
        }
        else if (Ptr->Command == "rsp") {
            PartStr = Utilities->Format96HHMM(GetControllerTrainTime(15,
                Ptr->EventTime, RepNum, IncMins)) + ": Rear split to " +
                TrainController->GetRepeatHeadCode(52, Ptr->OtherHeadCode,
                RepNum, IncDig) + " at " + Ptr->LocationName;
        }
        else if (Ptr->Command == "cdt") {
            PartStr = Utilities->Format96HHMM(GetControllerTrainTime(16,
                Ptr->EventTime, RepNum, IncMins)) + ": Change direction at " +
                Ptr->LocationName;
        }
        if (RetStr != "")
            RetStr = RetStr + '\n' + PartStr;
    ...
    while (Ptr < TTDEPtr->ActionVector.end() && (Count < 33) &&
...
bool TTrainController::TimetableIntegrityCheck(int Caller, char *FileName,
    bool GiveMessages, bool CheckLocationsExistInRailway) // true for success
{
    ...
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",TimetableIntegrityCheck," +
        AnsiString(FileName));
    // new for v0.2b
    ...
    for (unsigned int x = 0; x < Track->TrackVector.size(); x++) {
        ...
        if ((Track->TrackVector.at(x).ActiveTrackElementName != "") &&
            (Track->ContinuationNameMap.find(Track->TrackVector.at(x)
            .ActiveTrackElementName)) == Track->ContinuationNameMap.end())
        { // exclude any name that appears in a continuation, error message given in tt validation if try to include such a name in a tt
            ActiveTrackElementNameMapEntry.first =
                Track->TrackVector.at(x).ActiveTrackElementName;
            ActiveTrackElementNameMapEntry.second = 0; // this is a dummy value
            Track->ActiveTrackElementNameMap.insert
                (ActiveTrackElementNameMapEntry);
        }
    }
    ...
    if (TTBLFile.is_open()) {
        ...
        while ((Count == 0) && !ProcessOneTimetableLine(5, Count, OneLine,
            EndOfFile, FinalCallFalse, GiveMessages,
            CheckLocationsExistInRailway))
        ...
        while (!EndOfFile) {
            ...
            bool FinalCallFalse = false;
            if (!ProcessOneTimetableLine(6, Count, OneLine, EndOfFile,
                FinalCallFalse, GiveMessages, CheckLocationsExistInRailway))
...
bool TTrainController::ProcessOneTimetableLine(int Caller, int Count,
    AnsiString OneLine, bool &EndOfFile, bool FinalCall, bool GiveMessages,
     ...
     */ {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",ProcessOneTimetableLine," + AnsiString(Count) +
        "," + OneLine + "," + AnsiString((short)FinalCall) + "," +
        AnsiString((short)CheckLocationsExistInRailway));
    ...
    bool AllCommas = true;
    for (int x = 1; x < OneLine.Length() + 1;
    x++) // check for nothing but commas (may be all commas if created from Excel) or a blank line
    {
        if (OneLine[x] != ',')
            AllCommas = false;
    }
    ...
    int RearStartOrRepeatMins = 0, FrontStartOrRepeatDigits = 0,
        NumberOfRepeats = 0;
    ...
    if (Count == 0) // no start time found yet
    {
        ...
        if (Pos == 0) {
            ...
            TimetableMessage(GiveMessages,
                "Error in timetable - entry incomplete: see '" +
                OneLine.SubString(1, SubStringLength) + "'....");
            Utilities->CallLogPop(766);
            return false;
        }
        TrainInfoStr = OneLine.SubString(1, Pos - 1);
        if (!SplitTrainInfo(0, TrainInfoStr, HeadCode, Description, StartSpeed,
            MaxRunningSpeed, Mass, MaxBrakeRate, PowerAtRail, SignallerSpeed,
        ...
        if (FinalCall) {
            ...
            TempTrainDataEntry.TrainOperatingDataVector.push_back
                (TempTrainOperatingData); // push empty vector for now
        }
        AnsiString NewRemainder =
            OneLine.SubString(Pos + 1, OneLine.Length() - Pos);
        ...
        while (NewRemainder[NewRemainder.Length()] == ',') {
            if (NewRemainder.Length() > 1) {
                NewRemainder =
                    NewRemainder.SubString(1, NewRemainder.Length() - 1);
            }
            else {
                NewRemainder = "";
                break;
        ...
        if (NewRemainder == "") {
            TimetableMessage(GiveMessages,
                "Error in timetable - no events following train: '" +
                OneLine + "'");
        ...
        if (CommaCount == 0) {
            if ((NewRemainder.SubString(7, 3) != "Snt") ||
                (NewRemainder[NewRemainder.Length()] != 'S')) {
                int SubStringLength = 20;
                if (OneLine.Length() < 20)
                    SubStringLength = OneLine.Length();
                TimetableMessage(GiveMessages,
        ...
        for (int x = 0; x < CommaCount + 1; x++) {
                ...
                // be a finish but can't be a repeat
            {
                if (CommaCount == 0) {
                    ...
                    NewRemainder =
                        NewRemainder.SubString(Pos + 1,
                        NewRemainder.Length() - Pos);
                }
                First = "";
                Second = "";
                Third = "";
                ...
                NumberOfRepeats = 0;
                if (!SplitEntry(0, OneEntry, GiveMessages,
                    CheckLocationsExistInRailway, First, Second, Third, Fourth,
                    RearStartOrRepeatMins, FrontStartOrRepeatDigits, FormatType,
                    LocationType, SequenceType, ShuttleLinkType, ExitList,
                    Warning)) {
                    TimetableMessage(GiveMessages,
                ...
                if (x == 0) // should be start event
                {
                    if (SequenceType != Start) {
                        TimetableMessage(GiveMessages,
                            "Error in timetable - First event not a start event: '" +
                            OneEntry + "'");
                        Utilities->CallLogPop(784);
                        return false;
                    }
                    if ((Second == "Snt") && (Fourth == 'S') &&
                        (NewRemainder != "")) {
                        if (NewRemainder[1] != 'R') {
                            TimetableMessage(GiveMessages,
                    ...
                    {
                        if ((HeadCode == "") || (Description == "") ||
                            (MaxRunningSpeed == 0) || (Mass == 0) ||
                            (MaxBrakeRate == 0)) // ||
                        ...
                        // service continuation - need at least non-default value for HeadCode
                    {
                        ...
                        if ((StartSpeed != 0) || (MaxRunningSpeed != 0) ||
                            (Mass != 0) || (MaxBrakeRate != 0) ||
                            (PowerAtRail != 0)) {
                            TimetableMessage(GiveMessages,
                ...
                if (FinalCall) {
                    ...
                    else if (FormatType == PassTime) // new
                    {
                        ...
                        if (CheckTimeValidity(2, First,
                            ActionVectorEntry.ArrivalTime)) {;
                        }
                        if (CheckTimeValidity(3, Second,
                            ActionVectorEntry.DepartureTime)) {;
                        }
                        ActionVectorEntry.LocationName = Third;
                    }
                    else if (FormatType == TimeCmd) {
                        if (CheckTimeValidity(4, First,
                            ActionVectorEntry.EventTime)) {;
                        }
                        ActionVectorEntry.Command = Second;
                    }
                    else if (FormatType == ExitRailway) {
                        if (CheckTimeValidity(18, First,
                            ActionVectorEntry.EventTime)) {;
                        }
                        ActionVectorEntry.Command = Second;
                        ...
                        if (CheckTimeValidity(5, First,
                            ActionVectorEntry.EventTime)) {;
                        }
                        ActionVectorEntry.Command = Second;
                        ActionVectorEntry.RearStartOrRepeatMins =
                            RearStartOrRepeatMins;
                        ActionVectorEntry.FrontStartOrRepeatDigits =
                            FrontStartOrRepeatDigits;
                        if (Fourth == 'S')
                            ActionVectorEntry.SignallerControl = true;
                    }
                    else if (FormatType == SNTShuttle) {
                        if (CheckTimeValidity(6, First,
                            ActionVectorEntry.EventTime)) {;
                        }
                        ActionVectorEntry.Command = Second;
                        ActionVectorEntry.RearStartOrRepeatMins =
                        ...
                        if (CheckTimeValidity(7, First,
                            ActionVectorEntry.EventTime)) {;
                        }
                        ActionVectorEntry.Command = Second;
                        ActionVectorEntry.OtherHeadCode = Third;
                        ActionVectorEntry.NonRepeatingShuttleLinkHeadCode
                            = Fourth;
                    }
                    else if (FormatType == TimeCmdHeadCode) {
                        if (CheckTimeValidity(8, First,
                            ActionVectorEntry.EventTime)) {;
                        }
                        ActionVectorEntry.Command = Second;
                        ActionVectorEntry.OtherHeadCode = Third;
                    ...
                    TempTrainDataEntry.ActionVector.push_back
                        (ActionVectorEntry);
                }
            }
            else // last entry, & not entered under signaller control with no repeats, i.e. could be finish or repeat
            {
                    ...
                    // already had a finish entry
                {
                    TimetableMessage(GiveMessages,
                        "Error in timetable - Event: '" + OneEntry +
                        "' - only a repeat can follow a finish event");
                    Utilities->CallLogPop(79);
                    return false;
                }
                if (OneEntry[1] != 'R') // must be finish
                {
                    if (!SplitEntry(1, OneEntry, GiveMessages,
                        CheckLocationsExistInRailway, First, Second, Third,
                        Fourth, RearStartOrRepeatMins, FrontStartOrRepeatDigits,
                        FormatType, LocationType, SequenceType, ShuttleLinkType,
                        ExitList, Warning)) {
                        TimetableMessage(GiveMessages,
                            "Error in timetable - Event: '" + OneEntry + "'");
                    ...
                    if (SequenceType != Finish) {
                        TimetableMessage(GiveMessages,
                            "Error in timetable - last event should be a finish: '" +
                            OneEntry + "'");
                    ...
                    if (FinalCall) {
                        ...
                        if (FormatType == TimeCmd) {
                            if (CheckTimeValidity(11, First,
                                ActionVectorEntry.EventTime)) {;
                            }
                            ActionVectorEntry.Command = Second;
                        }
                        else if (FormatType == TimeCmdHeadCode) {
                            if (CheckTimeValidity(12, First,
                                ActionVectorEntry.EventTime)) {;
                            }
                            ActionVectorEntry.Command = Second;
                            ActionVectorEntry.OtherHeadCode = Third;
                ...
                {
                    if (!SplitRepeat(0, OneEntry, RearStartOrRepeatMins,
                        FrontStartOrRepeatDigits, NumberOfRepeats,
                    GiveMessages)) {
                    ...
                    if (FinalCall) {
                        ...
                        ActionVectorEntry.ShuttleLinkType =
...
bool TTrainController::Last2CharactersBothDigits(int Caller,
    AnsiString HeadCode) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",Last2CharactersBothDigits," + HeadCode);
    if ((HeadCode[HeadCode.Length() - 1] < '0') ||
        (HeadCode[HeadCode.Length() - 1] > '9')) {
        Utilities->CallLogPop(1890);
        return false;
    }
    if ((HeadCode[HeadCode.Length()] < '0') || (HeadCode[HeadCode.Length()] >
        '9')) {
...
bool TTrainController::CheckTimeValidity(int Caller, AnsiString TimeStr,
    TDateTime &Time)
    // 1st 5 chars must be HH:MM, anything else will be ignored
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CheckTimeValidity," + TimeStr);
    if (TimeStr.Length() < 5) {
    ...
    double WholeHours = (AnsiString(TimeStr[1]) + AnsiString(TimeStr[2]))
        .ToDouble();
    double FracHour =
        ((AnsiString(TimeStr[4]) + AnsiString(TimeStr[5])).ToDouble()) / 60.0;
...
bool TTrainController::SplitEntry(int Caller, AnsiString OneEntry,
    bool GiveMessages, bool CheckLocationsExistInRailway, AnsiString &First,
    AnsiString &Second, AnsiString &Third, AnsiString &Fourth,
    int &RearStartOrRepeatMins, int &FrontStartOrRepeatDigits,
    TTimetableFormatType &FormatType, TTimetableLocationType &LocationType,
    TTimetableSequenceType &SequenceType,
    TTimetableShuttleLinkType &ShuttleLinkType, TExitList &ExitList,
    bool &Warning)
     ...
     */ {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",SplitEntry," + OneEntry);
    Warning = false;
    TDateTime TempTime;
        ...
        // next segment is a time so this is a TimeTimeLoc & 3rd seg has to be a location to be valid
    {
        ...
        if (!CheckLocationValidity(0, Third, GiveMessages,
            CheckLocationsExistInRailway)) {
    ...
    if (Pos == 0) // no third segment so second must be a location, or cdt
    {
        ...
        if (!CheckLocationValidity(1, Second, GiveMessages,
            CheckLocationsExistInRailway)) {
        ...
        // third has to be 2 element idents with a space between
    {
        ...
        AnsiString FrontStartStr =
            Third.SubString(SpacePos + 1, Third.Length() - SpacePos);
        // int RearPosition=0, FrontPosition=0, RearExitPos=0;
        if (CheckLocationsExistInRailway) {
            if (!CheckStartPositionValidity(0, RearStartStr, FrontStartStr,
                GiveMessages)) {
                Utilities->CallLogPop(920);
                return false;
            }
            RearStartOrRepeatMins = Track->GetTrackVectorPositionFromString(3,
                RearStartStr, GiveMessages);
            FrontStartOrRepeatDigits = Track->GetTrackVectorPositionFromString
                (4, FrontStartStr, GiveMessages);
        }
        if (Second == "Snt") {
            FormatType = StartNew;
            SequenceType = Start;
    ...
    if (Second == "pas") // third has to be a location
    {
        ...
        SequenceType = Intermediate;
        ShuttleLinkType = NotAShuttleLink;
        if (!CheckLocationValidity(2, Third, GiveMessages,
            CheckLocationsExistInRailway)) {
    ...
    if ((Second != "Fns") && (Second != "Fjo") && (Second != "jbo") &&
        (Second != "fsp") && (Second != "rsp") && (Second != "Sfs") &&
        (Second != "Sns") && (Second != "Frh-sh")) {
...
bool TTrainController::CheckLocationValidity(int Caller, AnsiString LocStr,
    bool GiveMessages, bool CheckLocationsExistInRailway) {
    ...
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CheckLocationValidity," + LocStr);
    if (LocStr == "") {
    ...
    if (CheckLocationsExistInRailway) {
        if (!Track->TimetabledLocationNameAllocated(3, LocStr)) {
            TimetableMessage(GiveMessages, "Location name '" + LocStr +
                "' appears in the timetable but is not a valid name.  To be valid there must be track (but not a continuation) at a correspondingly named location.");
...
bool TTrainController::CheckHeadCodeValidity(int Caller, bool GiveMessages,
    AnsiString HeadCode) {
    ...
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + "," + AnsiString((short)GiveMessages) +
        ",CheckHeadCodeValidity," + HeadCode);
    ...
    for (int x = 3; x >= 0; x--) {
        if (((HeadCode[HeadCode.Length() - x] < 'A') ||
            (HeadCode[HeadCode.Length() - x] > 'Z')) &&
            ((HeadCode[HeadCode.Length() - x] < 'a') ||
            (HeadCode[HeadCode.Length() - x] > 'z')) &&
            ((HeadCode[HeadCode.Length() - x] < '0') ||
            (HeadCode[HeadCode.Length() - x] > '9'))) {
            TimetableMessage(GiveMessages, "Headcode error in '" + HeadCode +
...
bool TTrainController::CheckAndPopulateListOfIDs(int Caller, AnsiString IDSet,
    TExitList &ExitList, bool GiveMessages)
    // set of legitimate track element IDs, separated by spaces, and at least 1 present
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CheckSetOfIDs," + IDSet);
    ExitList.clear();
    ...
    for (int x = 1; x <= IDSet.Length(); x++) {
        ...
        {
            TimetableMessage(GiveMessages,
                "Illegal character in the set of element IDs following 'Fer' in '" +
                IDSet + "'");
    ...
    while (true) {
        ...
        int VecPos = Track->GetTrackVectorPositionFromString(7, CurrentID,
            GiveMessages);
        if (VecPos == -1) {
            ...
            if (Track->TrackElementAt(722, VecPos).TrackType != Continuation) {
                TimetableMessage(GiveMessages, "The element ID '" + CurrentID +
                ...
                if (!ExitList.empty()) {
                    for (TExitListIterator ELIT = ExitList.begin();
                    ELIT != ExitList.end(); ELIT++) {
                        if (*ELIT == VecPos) {
                            TimetableMessage(GiveMessages,
...
// ---------------------------------------------------------------------------
bool TTrainController::SplitTrainInfo(int Caller, AnsiString TrainInfoStr,
    AnsiString &HeadCode, AnsiString &Description, int &StartSpeed,
    int &MaxRunningSpeed, int &Mass, double &MaxBrakeRate, double &PowerAtRail,
    int &SignallerSpeed, bool GiveMessages)
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",SplitTrainInfo," + TrainInfoStr);
    int Pos = 0;
    ...
    if ((SemiColonCount != 6) && (SemiColonCount != 7) && (SemiColonCount != 1)
        && (SemiColonCount != 0)) {
        TimetableMessage(GiveMessages, "Error in train information in '" +
            TrainInfoStr +
    ...
    if (SemiColonCount == 1) // headcode & description only
    {
        ...
        Description = TrainInfoStr.SubString(Pos + 1,
            TrainInfoStr.Length() - Pos);
        if (!CheckHeadCodeValidity(9, GiveMessages, HeadCode)) {
            Utilities->CallLogPop(883);
            return false;
        }
        if (Description == "") {
            TimetableMessage(GiveMessages, "Train description missing in '" +
                TrainInfoStr + "'");
        ...
        if (Description.Length() > 60) {
            TimetableMessage(GiveMessages,
                "Train description too long, limit of 60 characters '" +
                TrainInfoStr + "'");
        ...
        for (int x = 1; x < Description.Length() + 1; x++) {
            if ((Description[x] < ' ') || (Description[x] > '~')) {
                TimetableMessage(GiveMessages,
                    "Train description contains invalid characters in '" +
                    TrainInfoStr + "'");
    ...
    Remainder = Remainder.SubString(Pos + 1, Remainder.Length() - Pos);
    if (StartSpeedStr == "") {
        TimetableMessage(GiveMessages, "Train starting speed missing in '" +
            TrainInfoStr + "'");
    ...
    for (int x = 1; x < StartSpeedStr.Length() + 1; x++) {
        if ((StartSpeedStr[x] < '0') || (StartSpeedStr[x] > '9')) {
            TimetableMessage(GiveMessages,
                "Train start speed contains invalid characters in '" +
                TrainInfoStr + "'");
    ...
    for (int x = 1; x < MaxRunningSpeedStr.Length() + 1; x++) {
        if ((MaxRunningSpeedStr[x] < '0') || (MaxRunningSpeedStr[x] > '9')) {
            TimetableMessage(GiveMessages,
                "Train maximum running speed contains invalid characters in '" +
                TrainInfoStr + "'");
        ...
        // changed at v0.6 to prevent low max speeds - can cause problems in SetTrainMovementValues
    {
        ...
        {
            TimetableMessage(GiveMessages,
                "Train maximum running speed can't be less than 10km/h in '" +
                TrainInfoStr + "', it will be set to 10km/h");
            MRSLow = true;
        }
    }
    ...
    Remainder = Remainder.SubString(Pos + 1, Remainder.Length() - Pos);
    if (MassStr == "") {
        TimetableMessage(GiveMessages, "Train mass missing in '" +
            TrainInfoStr + "'");
    ...
    for (int x = 1; x < MassStr.Length() + 1; x++) {
        if ((MassStr[x] < '0') || (MassStr[x] > '9')) {
            TimetableMessage(GiveMessages,
                "Train mass contains invalid characters in '" +
                TrainInfoStr + "'");
    ...
    if (Mass > TTrain::MaximumMassLimit) // 10,000tonnes
    {
        ...
        {
            TimetableMessage(GiveMessages, "Train mass > 10,000 tonnes in '" +
                TrainInfoStr + "'.  Setting it to 10,000 tonnes");
            MassHigh = true;
        }
    }
    if (Mass == 0) {
        TimetableMessage(GiveMessages, "Train mass zero in '" +
            TrainInfoStr + "'");
    ...
    Remainder = Remainder.SubString(Pos + 1, Remainder.Length() - Pos);
    if (MaxBrakeForceStr == "") {
        TimetableMessage(GiveMessages, "Train braking force missing in '" +
            TrainInfoStr + "'");
        Utilities->CallLogPop(898);
        return false;
    }
    for (int x = 1; x < (MaxBrakeForceStr.Length() + 1); x++) {
        if ((MaxBrakeForceStr[x] != '.') && ((MaxBrakeForceStr[x] < '0') ||
            (MaxBrakeForceStr[x] > '9'))) {
            TimetableMessage(GiveMessages,
                "Train braking force contains invalid characters in '" +
                TrainInfoStr + "'");
    ...
    if ((MaxBrakeForce / Mass) > 1) // gives 'g' braking - 9.81m/s/s
    {
        ...
        {
            TimetableMessage(GiveMessages, "Train braking force too high in '" +
                TrainInfoStr + "'.  Setting it to the same as the train mass");
            BFHigh = true;
        }
    }
    if ((MaxBrakeForce / Mass) < 0.01) {
        ...
        {
            TimetableMessage(GiveMessages, "Train braking force too low in '" +
                TrainInfoStr + "'.  Setting it to 1% of the train mass");
            BFLow = true;
        }
    }
    // convert to m/s/s
    ...
    else // must be 7
    {
        ...
        SignallerSpeedStr = Remainder.SubString(Pos + 1,
            Remainder.Length() - Pos);
    }
    // deal with GrossPower
    if (GrossPowerStr == "") {
        TimetableMessage(GiveMessages, "Train power missing in '" +
            TrainInfoStr + "'");
    ...
    for (int x = 1; x < GrossPowerStr.Length() + 1; x++) {
        if ((GrossPowerStr[x] < '0') || (GrossPowerStr[x] > '9')) {
            TimetableMessage(GiveMessages,
                "Train power contains invalid characters in '" +
                TrainInfoStr + "'");
    ...
    if (GrossPower > TTrain::MaximumPowerLimit) // 100MW
    {
        ...
        if (!PwrHigh) {
            TimetableMessage(GiveMessages, "Train power > 100,000kW in '" +
                TrainInfoStr + "'.  Setting it to 100,000kW");
            PwrHigh = true;
        }
    }
    ...
    if (SignallerSpeedStr == "") {
        TimetableMessage(GiveMessages, "Signaller speed not set in '" +
            TrainInfoStr +
    ...
    for (int x = 1; x < SignallerSpeedStr.Length() + 1; x++) {
        if ((SignallerSpeedStr[x] < '0') || (SignallerSpeedStr[x] > '9')) {
            TimetableMessage(GiveMessages,
                "Signaller speed contains invalid characters in '" +
                TrainInfoStr + "'");
    ...
    if (SignallerSpeed > TTrain::MaximumSpeedLimit) {
        ...
        if (!SigSHigh) {
            TimetableMessage(GiveMessages, "Signaller speed > 400km/h in '" +
                TrainInfoStr + "'.  Setting it to 400km/h");
            SigSHigh = true;
        }
    }
    if (SignallerSpeed < 10)
        // changed at v0.6 to prevent low max speeds - can cause problems in SetTrainMovementValues
    {
        ...
        if (!SigSLow) {
            TimetableMessage(GiveMessages,
                "Signaller speed can't be less than 10km/h in '" +
                TrainInfoStr + "', it will be set to 10km/h");
            SigSLow = true;
        }
...
bool TTrainController::SplitRepeat(int Caller, AnsiString OneEntry,
    int &RearStartOrRepeatMins, int &FrontStartOrRepeatDigits,
    int &NumberOfRepeats, bool GiveMessages) {
    ...
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",SplitRepeat," + OneEntry);
    if (OneEntry.Length() < 7) {
     ...
     */
    if (!Last2CharactersBothDigits(0, ServiceReference) &&
        (FrontStartOrRepeatDigits > 0))
    ...
    if (NumberOfRepeats == 0) {
        TimetableMessage(GiveMessages, "Number of repeats is zero in:  '" +
            OneEntry +
...
bool TTrainController::SecondPassActions(int Caller, bool GiveMessages)
     ...
     */ {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",SecondPassActions,");
    if (TrainDataVector.empty()) {
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) // (1)
    {
        ...
        if (TrainDataVector.at(x).ActionVector.empty()) {
            SecondPassMessage(GiveMessages,
                "Error in timetable - the following service has no listed events, there must be at least one: " +
                TDEntry.HeadCode);
            TrainDataVector.clear();
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) // (2)
    {
        ...
        if (!(AVEntry0.SignallerControl)) {
            if (TrainDataVector.at(x).ActionVector.size() == 1) {
                SecondPassMessage(GiveMessages,
                    "Error in timetable - service must have a start event and at least one other for: " +
                    TDEntry.HeadCode);
                TrainDataVector.clear();
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) // (3)
    {
        ...
        if (AVEntry0.SignallerControl) {
            ...
            if (TrainDataVector.at(x).ActionVector.size() > 1) {
                TActionVectorEntry AVEntry1 =
                    TrainDataVector.at(x).ActionVector.at(1);
                if (AVEntry1.FormatType != Repeat) {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - a signaller control service cannot have any other than a repeat after the start event, see: " +
                        TDEntry.HeadCode);
                    TrainDataVector.clear();
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) // (4)
    {
        ...
        if (AVEntry0.SequenceType != Start) {
            SecondPassMessage(GiveMessages,
                "Error in timetable - the first event must be a start for: " +
                TDEntry.HeadCode);
            TrainDataVector.clear();
            ...
            // and others for a located Snt, but those checks done later
        {
            ...
            if ((AVEntry1.SequenceType == Finish) && (AVEntry1.Command != "Frh")
                && (AVEntry1.Command != "Fjo") && (AVEntry1.Command != "Fer")) {
                SecondPassMessage(GiveMessages,
                    "Error in timetable - the only finish events immediately following Snt must be Frh, Fjo or Fer for: " +
                    TDEntry.HeadCode);
                TrainDataVector.clear();
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) // (5)
    {
        ...
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            const TActionVectorEntry &AVEntry =
                TrainDataVector.at(x).ActionVector.at(y);
            if ((AVEntry.SequenceType == Start) && (y != 0)) {
                SecondPassMessage(GiveMessages,
                    "Error in timetable - a start event is present that is not the first event for: " +
                    TDEntry.HeadCode);
                TrainDataVector.clear();
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) // (6)
    {
        ...
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            ...
            if ((AVEntry.FormatType == Repeat) &&
                (y != (TrainDataVector.at(x).ActionVector.size() - 1))) {
                SecondPassMessage(GiveMessages,
                    "Error in timetable - a repeat is present that is not the last item for: " +
                    TDEntry.HeadCode);
                TrainDataVector.clear();
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) // (7)
    {
        ...
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            ...
            if (y == (TrainDataVector.at(x).ActionVector.size() - 1)) {
                if ((AVEntry.FormatType != Repeat) &&
                    (AVEntry.SequenceType != Finish)) {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - the last item must be either a finish event or a repeat for: " +
                        TDEntry.HeadCode);
                    TrainDataVector.clear();
                ...
                if (AVEntry.FormatType == Repeat) {
                    const TActionVectorEntry &LastAVEntry =
                        TrainDataVector.at(x).ActionVector.at(y - 1);
                    if (LastAVEntry.SequenceType != Finish) {
                        SecondPassMessage(GiveMessages,
                            "Error in timetable - the last event before the repeat must be a finish for: " +
                            TDEntry.HeadCode);
                        TrainDataVector.clear();
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) // (8)
    {
        ...
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            ...
            if (AVEntry.SequenceType == Finish) {
                if ((y != (TrainDataVector.at(x).ActionVector.size() - 1)) &&
                    (y != (TrainDataVector.at(x).ActionVector.size() - 2))) {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - a finish event must be either the last or last but one for: " +
                        TDEntry.HeadCode);
                    TrainDataVector.clear();
                ...
                if (y == (TrainDataVector.at(x).ActionVector.size() - 2)) {
                    if (TrainDataVector.at(x).ActionVector.at(y + 1)
                        .FormatType != Repeat) {
                        SecondPassMessage(GiveMessages,
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        const TTrainDataEntry &TDEntry = TrainDataVector.at(x);
        TActionVectorEntry &AVEntry0 =
            TrainDataVector.at(x).ActionVector.at(0);
        ...
        if ((AVEntry0.Command == "Snt") || (AVEntry0.Command == "Snt-sh")) {
                ...
                // it is at a location
            {
                if (TDEntry.StartSpeed == 0) // stopped
                {
                    ...
                    if (!AVEntry0.SignallerControl) {
                        const TActionVectorEntry &AVEntry1 =
                            TrainDataVector.at(x).ActionVector.at(1);
            ...
            else // check not Snt-sh & carry out successor validity checks for unlocated Snt that isn't a SignallerControl entry
            {
                if (AVEntry0.Command == "Snt-sh") {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - 'Snt-sh' event not at stop location for: " +
                        TDEntry.HeadCode);
                    TrainDataVector.clear();
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        bool FoundFlag = false;
        const TTrainDataEntry &TDEntry = TrainDataVector.at(x);
        TActionVectorEntry &AVEntry =
            TrainDataVector.at(x).ActionVector.at(0);
        // use reference so can change internals
        if ((AVEntry.Command == "Sfs") || (AVEntry.Command == "Sns") ||
            (AVEntry.Command == "Sns-sh") || (AVEntry.Command == "Sns-fsh")) {
            for (unsigned int y = 1;
            y < TrainDataVector.at(x).ActionVector.size(); y++) {
                const TActionVectorEntry &AVEntry2 =
                    TrainDataVector.at(x).ActionVector.at(y);
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            ...
            if ((AVEntry.FormatType == TimeLoc) ||
                ((AVEntry.SequenceType == Start) &&
                (AVEntry.LocationType == AtLocation))) {
                ...
                {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable for " + TDEntry.HeadCode +
                ...
                for (unsigned int z = y + 1;
                z < TrainDataVector.at(x).ActionVector.size(); z++) {
                    TActionVectorEntry &AVEntry2 =
                        TrainDataVector.at(x).ActionVector.at(z);
                    // use reference so can change internals where necessary
                    if ((AVEntry2.Command != "") &&
                        (AVEntry2.LocationType == AtLocation)) {
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            ...
            if ((AVEntry.SequenceType == Finish) &&
                    ...
                    // i.e at least one more, must be a repeat
                {
                    if (TrainDataVector.at(x).ActionVector.at(y + 1)
                        .FormatType != Repeat) {
                        SecondPassMessage(GiveMessages,
                            "Error in timetable - only a repeat can follow a finish entry for: " +
                            TDEntry.HeadCode);
                        TrainDataVector.clear();
            ...
            if (AVEntry.Command == "F-nshs") {
                ...
                {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - F-nshs (shuttle link) must be the last event for: " +
                        TDEntry.HeadCode);
                    TrainDataVector.clear();
            ...
            if (AVEntry.Command == "pas") {
                if (y >= (TrainDataVector.at(x).ActionVector.size() - 1)) {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - a 'pas' can't be the last event for: " +
                        TDEntry.HeadCode);
                    TrainDataVector.clear();
            ...
            if (AVEntry.Command == "jbo") {
                if (y >= (TrainDataVector.at(x).ActionVector.size() - 1)) {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - a 'jbo' can't be the last event for: " +
                        TDEntry.HeadCode);
                    TrainDataVector.clear();
                ...
                if (!AtLocSuccessor(AVEntry2)) {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - a jbo event is followed by an illegal event for: " +
                        TDEntry.HeadCode +
                        ". The event isn't valid for a stationary train.");
                    TrainDataVector.clear();
            ...
            if ((AVEntry.Command == "fsp") || (AVEntry.Command == "rsp")) {
                ...
                if (!AtLocSuccessor(AVEntry2)) {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - a train split is followed by an illegal event for: " +
                        TDEntry.HeadCode +
                        ". The event isn't valid for a stationary train.");
                    TrainDataVector.clear();
            ...
            if (AVEntry.Command == "cdt") {
                if (y >= (TrainDataVector.at(x).ActionVector.size() - 1)) {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - a 'cdt' can't be the last event for: " +
                        TDEntry.HeadCode);
                    TrainDataVector.clear();
                ...
                if (!AtLocSuccessor(AVEntry2)) {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - a 'cdt' is followed by an illegal event for: " +
                        TDEntry.HeadCode +
                        ". The event isn't valid for a stationary train.");
                    TrainDataVector.clear();
            ...
            if (AVEntry.FormatType == TimeTimeLoc) {
                if (y >= (TrainDataVector.at(x).ActionVector.size() - 1)) {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - a timed arrival and departure can't be the last event for: " +
                        TDEntry.HeadCode);
                    TrainDataVector.clear();
            ...
            if (AVEntry.FormatType == PassTime) {
                if (y >= (TrainDataVector.at(x).ActionVector.size() - 1)) {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - a pass time can't be the last event for: " +
                        TDEntry.HeadCode);
                    TrainDataVector.clear();
                ...
                if (!MovingSuccessor(AVEntry2)) {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - a pass time is followed by an illegal event for: " +
                        TDEntry.HeadCode +
                        ". The event isn't valid for a moving train.");
                    TrainDataVector.clear();
            ...
            if (AVEntry.FormatType == Repeat) {
                if (y != (TrainDataVector.at(x).ActionVector.size() - 1)) {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - a repeat is not the last item for: " +
                        TDEntry.HeadCode);
                    TrainDataVector.clear();
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        const TActionVectorEntry &AVEntry0 =
            TrainDataVector.at(x).ActionVector.at(0);
            ...
            // StartSpeed may or may not be 0, but train will move forwards (if capable of doing so), & next TimeLoc will be an arrival, whether or not after one or more TimeTimeLocs
        {
            ...
            y < TrainDataVector.at(x).ActionVector.size(); y++) {
                ...
                if (AVEntry.FormatType == TimeLoc)
                {
                    if ((AVEntry.ArrivalTime > TDateTime(-1)) ||
                        (AVEntry.DepartureTime > TDateTime(-1)) ||
                        (AVEntry.EventTime == TDateTime(-1))) {
                        throw Exception
                            ("Timetable error, TimeLoc times not as initially set for " +
                            TDEntry.HeadCode);
                    }
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            const TActionVectorEntry &AVEntry =
                TrainDataVector.at(x).ActionVector.at(y);
            if ((AVEntry.FormatType == TimeLoc) && (AVEntry.ArrivalTime >=
                TDateTime(0))) // arrival
                // TimeLoc (arr) ->  No starts, repeats, Fer or TimeTimeLoc; TimeLoc (dep) or any other OK
            {
                if (y >= (TrainDataVector.at(x).ActionVector.size() - 1)) {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - a timed arrival can't be the last event for: " +
                        TDEntry.HeadCode);
                    TrainDataVector.clear();
                ...
                if (!AtLocSuccessor(AVEntry2)) {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - a timed arrival is followed by an illegal event for: " +
                        TDEntry.HeadCode +
                        ". The event isn't valid for a stationary train.");
                    TrainDataVector.clear();
            ...
            if ((AVEntry.FormatType == TimeLoc) && (AVEntry.DepartureTime >=
                TDateTime(0))) // departure
                // TimeLoc (dep) ->  Fer, TimeLoc (arr), TimeTimeLoc, (new) pas OK, no others
            {
                if (y >= (TrainDataVector.at(x).ActionVector.size() - 1)) {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - a timed departure can't be the last event for: " +
                        TDEntry.HeadCode);
                    TrainDataVector.clear();
                ...
                if (!MovingSuccessor(AVEntry2)) {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - a timed departure is followed by an illegal event for: " +
                        TDEntry.HeadCode +
                        ". The event isn't valid for a moving train.");
                    TrainDataVector.clear();
        ...
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            ...
            if (AVEntry.FormatType == TimeLoc) {
                if (AVEntry.EventTime != TDateTime(-1)) {
                    throw Exception
                        ("Timetable error, TimeLoc entry has EventTime not -1 for " +
                        TDEntry.HeadCode);
                }
                if ((AVEntry.ArrivalTime == TDateTime(-1)) &&
                    (AVEntry.DepartureTime == TDateTime(-1))) {
                    throw Exception
                        ("Timetable error, TimeLoc entry has neither arrival nor departure time set for " +
                        TDEntry.HeadCode);
                }
            }
            if (AVEntry.FormatType == TimeTimeLoc) {
                if (AVEntry.EventTime != TDateTime(-1)) {
                    throw Exception
                        ("Timetable error, TimeTimeLoc entry has EventTime not -1 for " +
                        TDEntry.HeadCode);
                }
                if ((AVEntry.ArrivalTime == TDateTime(-1)) ||
                    (AVEntry.DepartureTime == TDateTime(-1))) {
                    throw Exception
                        ("Timetable error, TimeTimeLoc entry has either arrival or departure time not set for " +
                        TDEntry.HeadCode);
                }
            }
            if ((AVEntry.FormatType == TimeCmd) ||
                (AVEntry.FormatType == TimeCmdHeadCode) ||
                (AVEntry.FormatType == StartNew) ||
                (AVEntry.FormatType == SNTShuttle) ||
                (AVEntry.FormatType == SNSShuttle) ||
                (AVEntry.FormatType == FNSNonRepeatToShuttle) ||
                (AVEntry.FormatType == FSHNewService) ||
                (AVEntry.FormatType == PassTime)) {
                if (AVEntry.EventTime == TDateTime(-1)) {
                    throw Exception
                        ("Timetable error, Cmd or PassTime entry has EventTime not set for " +
                        TDEntry.HeadCode);
                }
                if ((AVEntry.ArrivalTime != TDateTime(-1)) ||
                    (AVEntry.DepartureTime != TDateTime(-1))) {
                    throw Exception
                        ("Timetable error, Cmd or PassTime entry has either arrival or departure time set for " +
                        TDEntry.HeadCode);
                }
            }
            if (AVEntry.FormatType == Repeat) {
                if ((AVEntry.EventTime != TDateTime(-1)) ||
                    (AVEntry.ArrivalTime != TDateTime(-1)) ||
                    (AVEntry.DepartureTime != TDateTime(-1))) {
                    throw Exception
                        ("Timetable error, Repeat entry has a time set for " +
                        TDEntry.HeadCode);
                }
            }
        }
        ...
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            ...
            if (AVEntry.FormatType == TimeLoc) {
                if (AVEntry.ArrivalTime >= TDateTime(0)) {
                    if (AVEntry.ArrivalTime < CurrentTime) {
                        SecondPassMessage(GiveMessages,
                            "Error in timetable - a timed location event has a time that is too early for: " +
                            TDEntry.HeadCode);
                        TrainDataVector.clear();
            ...
            {
                SecondPassMessage(GiveMessages,
                    "Error in timetable - a train event has a time that is set too early for: " +
                    TDEntry.HeadCode + ", may be before timetable start time");
                TrainDataVector.clear();
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        if ((TrainDataVector.at(x).ActionVector.at(0).Command == "Snt") &&
            (TrainDataVector.at(x).ActionVector.at(0).LocationType == EnRoute))
        {
            LastEntryIsAnArrival = false;
            LastLocationName = TrainDataVector.at(x).ActionVector.at(0)
                .LocationName; // should be ""
            if (LastLocationName != "") {
                throw Exception
                    ("Timetable error, moving Snt entry has LocationName set for " +
                    TDEntry.HeadCode);
            }
            for (unsigned int y = 1;
            y < TrainDataVector.at(x).ActionVector.size();
                ...
                // if it's a SignallerControl entry then the condition isn't met
            {
                ...
                else if ((AVEntry.FormatType == TimeCmdHeadCode) ||
                    (AVEntry.FormatType == FNSNonRepeatToShuttle)) {
                    if (AVEntry.LocationName != LastLocationName) {
                        SecondPassMessage(GiveMessages,
                            "Error in timetable - a location event is inconsistent for: " +
                            TDEntry.HeadCode + " && " + AVEntry.Command);
                        TrainDataVector.clear();
                    ...
                    // cdt is the only TimeCmd
                {
                    ...
                    {
                        SecondPassMessage(GiveMessages,
                            "Error in timetable - a location event in a timed arrival and departure is the same as the last location for: " +
                            TDEntry.HeadCode);
                        TrainDataVector.clear();
                    ...
                    if (LastEntryIsAnArrival &&
                        (AVEntry.LocationName != LastLocationName)) {
                        SecondPassMessage(GiveMessages,
                            "Error in timetable - a location event for a timed departure is different from the arrival location for: " +
                            TDEntry.HeadCode);
                        TrainDataVector.clear();
                        Utilities->CallLogPop(826);
                        return false;
                    }
                    else if (!LastEntryIsAnArrival &&
                        (AVEntry.LocationName == LastLocationName)) {
                        SecondPassMessage(GiveMessages,
                            "Error in timetable - a location event for a timed arrival is the same as the earlier departure location for: " +
                            TDEntry.HeadCode);
                        TrainDataVector.clear();
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        while (y < TDEntry.ActionVector.size())
            ...
            // if y == 1
        {
            if ((TDEntry.ActionVector.at(y).Command == "Fer") ||
                (TDEntry.ActionVector.at(y).FormatType == Repeat)) {
            ...
            for (unsigned int z = y; z < TDEntry.ActionVector.size(); z++) {
                ...
                if ((AVEntry.Command == "Fer") ||
                    (AVEntry.FormatType == Repeat)) {
                ...
                {
                    for (unsigned int a = z;
                    a < TDEntry.ActionVector.size(); a++) {
                        ...
                        if (TDEntry.ActionVector.at(a)
                            .LocationName == LocationNameToBeChecked) {
                            SecondPassMessage(GiveMessages,
                                "Error in timetable - a location entry appears twice inappropriately for: " +
                                TDEntry.HeadCode);
                            TrainDataVector.clear();
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            const TActionVectorEntry &AVEntry =
                TrainDataVector.at(x).ActionVector.at(y);
            if ((AVEntry.LocationName == "") && (AVEntry.Command != "Snt") &&
                (AVEntry.Command != "Fer") && (AVEntry.FormatType != Repeat)) {
                throw Exception
                    ("Error, non- 'Snt', 'Fer' or Repeat entry doesn't have a location name set for " +
                    TDEntry.HeadCode);
            }
            AnsiString LocName = "";
            // dummy, only used so can call IsSNTEntryLocated
            if ((AVEntry.Command == "Snt") && (IsSNTEntryLocated(1,
                TrainDataVector.at(x), LocName))) {
                if (AVEntry.LocationName == "") {
                    throw Exception
                        ("Error, 'Snt' entry at a stop location doesn't have a location name set for " +
                        TDEntry.HeadCode);
                }
            }
            if ((AVEntry.Command == "Snt") && !(IsSNTEntryLocated(2,
                TrainDataVector.at(x), LocName))) {
                if (AVEntry.LocationName != "") {
                    throw Exception
                        ("Error, 'Snt' unlocated entry has a location name set for " +
                        TDEntry.HeadCode);
                }
            }
        }
     ...
     */
    for (unsigned int x = 0; x < TrainDataVector.size();
    x++) // new test to ensure no duplicate links at all, other checks ensure none for shuttles,
        ...
        // these also set the pointers
    {
        ...
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            ...
            if (AVEntry.OtherHeadCode != "") {
                if (!CheckForDuplicateCrossReferences(0, TDEntry.HeadCode,
                    AVEntry.OtherHeadCode, GiveMessages)) {
                    Utilities->CallLogPop(1584);
                    return false; // error message given in called function
                }
            }
            if (AVEntry.NonRepeatingShuttleLinkHeadCode != "") {
                if (!CheckForDuplicateCrossReferences(1, TDEntry.HeadCode,
                    AVEntry.NonRepeatingShuttleLinkHeadCode, GiveMessages)) {
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            ...
            if ((AVEntry.Command != "Sns-sh") && (AVEntry.Command != "Snt-sh")
                && (AVEntry.Command != "Fns-sh") &&
                (AVEntry.Command != "Frh-sh")) {
                if (AVEntry.OtherHeadCode != "") {
                    if (!CheckCrossReferencesAndSetData(0, TDEntry.HeadCode,
                        AVEntry.OtherHeadCode, false, GiveMessages))
                        // false = non-shuttle
                    {
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            ...
            if ((AVEntry.Command == "Sns-sh") || (AVEntry.Command == "Snt-sh")
                ...
                (AVEntry.Command == "Frh-sh")) {
                if (AVEntry.OtherHeadCode != "") {
                    if (!CheckCrossReferencesAndSetData(1, TDEntry.HeadCode,
                        AVEntry.OtherHeadCode, true, GiveMessages))
                        // true = shuttle
                    {
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            const TActionVectorEntry &AVEntry =
                TrainDataVector.at(x).ActionVector.at(y);
            if (AVEntry.NonRepeatingShuttleLinkHeadCode != "") {
                if (!CheckNonRepeatingShuttleLinksAndSetData(0,
                    TDEntry.HeadCode, AVEntry.NonRepeatingShuttleLinkHeadCode,
                    GiveMessages)) {
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            ...
            if ((AVEntry.Command == "Sns-sh") || (AVEntry.Command == "Snt-sh"))
            {
                if (!CheckShuttleServiceIntegrity(0, &(TrainDataVector.at(x)),
                    GiveMessages)) {
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            ...
            if (AVEntry.FormatType == NoFormat) {
                throw Exception("Error - timetable ActionVector entry no. " +
                    AnsiString(y) + " has FormatType unset for: " +
                    TDEntry.HeadCode);
            }
            else if (AVEntry.SequenceType == NoSequence) {
                throw Exception("Error - timetable ActionVector entry no. " +
                    AnsiString(y) + " has SequenceType unset for: " +
                    TDEntry.HeadCode);
            }
            else if (AVEntry.LocationType == NoLocation) {
                throw Exception("Error - timetable ActionVector entry no. " +
                    AnsiString(y) + " has LocationType unset for: " +
                    TDEntry.HeadCode);
            }
            else if (AVEntry.ShuttleLinkType == NoShuttleLink) {
                throw Exception("Error - timetable ActionVector entry no. " +
                    AnsiString(y) + " has ShuttleLinkType unset for: " +
                    TDEntry.HeadCode);
            }
        }
    }
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        for (unsigned int y = 0;
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            AnsiString LocName = TrainDataVector.at(x).ActionVector.at(y)
                .LocationName;
            AnsiString HC = TrainDataVector.at(x).HeadCode;
            if (LocName != "") {
                if (Track->ContinuationNameMap.find(LocName)
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        int IncMinutes = 0;
        if (TrainDataVector.at(x).ActionVector.at
            (TrainDataVector.at(x).ActionVector.size() - 1)
            .FormatType == Repeat) {
            IncMinutes = TrainDataVector.at(x).ActionVector.at
                (TrainDataVector.at(x).ActionVector.size() - 1)
                .RearStartOrRepeatMins;
        }
        else
        ...
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            ...
            {
                if (((double)GetRepeatTime(32,
                    TrainDataVector.at(x).ActionVector.at(y).EventTime,
                    NumRepeats, IncMinutes) >= 3.9994)) {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - a repeat time exceeds 95h 59m, see service " +
                        HC); // 3d 23h 59m = 3.9993055556
                    TrainDataVector.clear();
                    Utilities->CallLogPop(1818);
                    return false;
                }
            }
            if ((double)TrainDataVector.at(x).ActionVector.at(y)
                .ArrivalTime > -1) {
                if (((double)GetRepeatTime(33,
                    TrainDataVector.at(x).ActionVector.at(y).EventTime,
                    NumRepeats, IncMinutes) >= 3.9994))
                    // 3d 23h 59m = 3.9993055556
                {
                    SecondPassMessage(GiveMessages,
                        "Error in timetable - a repeat entry time exceeds 95h 59m, see service " +
                        HC);
                    TrainDataVector.clear();
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        for (unsigned int y = 0;
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            StripExcessFromHeadCode(1, TrainDataVector.at(x).ActionVector.at(y)
                .OtherHeadCode);
            StripExcessFromHeadCode(2, TrainDataVector.at(x).ActionVector.at(y)
                .NonRepeatingShuttleLinkHeadCode);
        }
    }
...
bool TTrainController::MovingSuccessor(const TActionVectorEntry &AVEntry) {
    return ((AVEntry.FormatType == TimeLoc) ||
        (AVEntry.FormatType == TimeTimeLoc) || (AVEntry.Command == "pas") ||
        (AVEntry.Command == "Fer"));
}
...
bool TTrainController::AtLocSuccessor(const TActionVectorEntry &AVEntry) {
    return ((AVEntry.FormatType == TimeLoc) || (AVEntry.Command == "jbo") ||
        (AVEntry.Command == "fsp") || (AVEntry.Command == "rsp") ||
        (AVEntry.Command == "cdt") || (AVEntry.Command == "Frh") ||
        (AVEntry.Command == "Fns") || (AVEntry.Command == "Fjo") ||
        (AVEntry.Command == "Frh-sh") || (AVEntry.Command == "Fns-sh") ||
        (AVEntry.Command == "F-nshs"));
}
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",StripExcessFromHeadCode," + HeadCode);
...
bool TTrainController::CheckForDuplicateCrossReferences(int Caller,
    AnsiString MainHeadCode, AnsiString SecondHeadCode, bool GiveMessages) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CheckForDuplicateCrossReferences," +
        MainHeadCode + "," + SecondHeadCode);
    int ForwardCount = 0;
    int ReverseCount = 0;

    if (MainHeadCode == SecondHeadCode) {
        SecondPassMessage(GiveMessages, "Error in timetable - Service " +
            MainHeadCode + " has an event that references itself");
        TrainDataVector.clear();
    ...
    {
        SecondPassMessage(GiveMessages,
            "Error in timetable - found more than two references to " +
            SecondHeadCode + " from a train whose headcode is " + MainHeadCode);
        TrainDataVector.clear();
    ...
    if (ReverseCount == 0) {
        SecondPassMessage(GiveMessages,
            "Error in timetable - cross reference missing in either " +
            MainHeadCode + " or " + SecondHeadCode);
        TrainDataVector.clear();
        ...
        // can have 2 if one is a second shuttle leg with a link in from Fns, and it links out to the same service with Fxx-sh
    {
        SecondPassMessage(GiveMessages,
            "Error in timetable - found more than two references to " +
            MainHeadCode + " from a train whose headcode is " + SecondHeadCode);
        TrainDataVector.clear();
    ...
    if (ForwardCount != ReverseCount) {
        SecondPassMessage(GiveMessages, "Error in timetable - " + MainHeadCode +
...
bool TTrainController::CheckCrossReferencesAndSetData(int Caller,
    AnsiString MainHeadCode, AnsiString OtherHeadCode, bool Shuttle,
    bool GiveMessages)
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CheckCrossReferencesAndSetData," + MainHeadCode +
        "," + OtherHeadCode);
    int ForwardCount = 0;
    int ReverseCount = 0;
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        if (TDEntry.HeadCode == MainHeadCode) {
            ...
            y < TrainDataVector.at(x).ActionVector.size(); y++) {
                TActionVectorEntry &AVEntry =
                    TrainDataVector.at(x).ActionVector.at(y);
                if (!Shuttle && (AVEntry.Command != "Sns-sh") &&
                    (AVEntry.Command != "Snt-sh") &&
                    (AVEntry.Command != "Fns-sh") &&
                    (AVEntry.Command != "Frh-sh")) {
                ...
                else if (Shuttle && ((AVEntry.Command == "Sns-sh") ||
                    (AVEntry.Command == "Snt-sh") ||
                    (AVEntry.Command == "Fns-sh") ||
                    (AVEntry.Command == "Frh-sh"))) {
                    if (AVEntry.OtherHeadCode == OtherHeadCode) {
    ...
    if (ForwardCount > 1) {
        SecondPassMessage(GiveMessages,
            "Error in timetable - found more than one reference to " +
            OtherHeadCode + " from a train whose headcode is " + MainHeadCode);
        TrainDataVector.clear();
    ...
    if (ReverseCount > 1) {
        SecondPassMessage(GiveMessages,
            "Error in timetable - found more than one reference to " +
            MainHeadCode + " from a train whose headcode is " + OtherHeadCode);
        TrainDataVector.clear();
    ...
    bool ForwardShuttleStart =
        ((ForwardEntryPtr->Command == "Sns-sh") ||
        (ForwardEntryPtr->Command == "Snt-sh"));
    bool ForwardShuttleFinish =
        ((ForwardEntryPtr->Command == "Fns-sh") ||
        (ForwardEntryPtr->Command == "Frh-sh"));
    bool ReverseShuttleStart =
        ((ReverseEntryPtr->Command == "Sns-sh") ||
        (ReverseEntryPtr->Command == "Snt-sh"));
    bool ReverseShuttleFinish =
        ((ReverseEntryPtr->Command == "Fns-sh") ||
        (ReverseEntryPtr->Command == "Frh-sh"));

    if (Shuttle && MainTrainDataPtr->ActionVector.back().FormatType != Repeat) {
        SecondPassMessage(GiveMessages, "Error in timetable - shuttle train " +
            MainHeadCode + " does not have a repeat");
        TrainDataVector.clear();
    ...
    {
        SecondPassMessage(GiveMessages, "Error in timetable - shuttle train " +
            OtherHeadCode + " does not have a repeat");
        TrainDataVector.clear();
    ...
    if (ForwardEntryPtr->LocationName == "") {
        SecondPassMessage(GiveMessages,
            "Error in timetable - location error in cross referenced trains " +
            MainHeadCode + " and " + OtherHeadCode +
    ...
    if (ForwardEntryPtr->LocationName != ReverseEntryPtr->LocationName) {
        SecondPassMessage(GiveMessages,
            ...
            " is at a different location to the referencing train " +
            MainHeadCode);
        TrainDataVector.clear();
    ...
    if (!Shuttle) {
        if (ForwardEntryPtr->EventTime != ReverseEntryPtr->EventTime) {
            SecondPassMessage(GiveMessages,
                "Error in timetable - cross referenced train " + OtherHeadCode +
                " has a different event time to the referencing train " +
                MainHeadCode);
            TrainDataVector.clear();
    ...
    {
        if (!CheckShuttleRepeatTime(0, ForwardEntryPtr->EventTime,
            ReverseEntryPtr->EventTime, OtherTrainDataPtr->ActionVector.back()
            .RearStartOrRepeatMins)) {
            SecondPassMessage(GiveMessages,
                "Error in timetable - shuttle service " + MainHeadCode +
                " first repeat restart time not consistent with finish service " +
                OtherHeadCode);
            TrainDataVector.clear();
        ...
        // doesn't matter about ForwardEntryPtr being Sfs/Sns as called for every occurrence of an 'OtherHeadCode' so won't escape
    {
        if (ReverseTDVectorNumber == ForwardTDVectorNumber) {
            SecondPassMessage(GiveMessages,
                "Error in timetable - an 'Sfs' or 'Sns' event (" +
                OtherHeadCode +
                ") appears in the same sequence as the corresponding linked event " +
                MainHeadCode);
            TrainDataVector.clear();
    ...
    if (ForwardEntryPtr->Command == "Sfs") {
        if ((ReverseEntryPtr->Command != "fsp") &&
            (ReverseEntryPtr->Command != "rsp")) {
            SecondPassMessage(GiveMessages,
    ...
    if ((ForwardEntryPtr->Command == "fsp") ||
        (ForwardEntryPtr->Command == "rsp")) {
        if (ReverseEntryPtr->Command != "Sfs") {
            SecondPassMessage(GiveMessages,
            ...
            if (!(Track->TimetabledLocationNameAllocated(4,
                ForwardEntryPtr->LocationName))) {
                SecondPassMessage(GiveMessages,
                    "Error in timetable - can't find timetabled location '" +
                    ForwardEntryPtr->LocationName + "' in railway");
                TrainDataVector.clear();
                Utilities->CallLogPop(849);
                return false;
            }
            if (!(Track->OneNamedLocationElementAtLocation(0,
                ForwardEntryPtr->LocationName))) {
                SecondPassMessage(GiveMessages,
                    "Error in timetable - can't find any named location elements at '" +
                    ForwardEntryPtr->LocationName + "'");
                TrainDataVector.clear();
                Utilities->CallLogPop(850);
                return false;
            }
            if (!(Track->OneNamedLocationLongEnoughForSplit(0,
                ForwardEntryPtr->LocationName))) {
                SecondPassMessage(GiveMessages,
                    "Error in timetable - location too short to split a train at " +
                    ForwardEntryPtr->LocationName);
                TrainDataVector.clear();
            ...
            if (OtherTrainDataPtr->Description == "")
                OtherTrainDataPtr->Description =
                    MainTrainDataPtr->Description;
            ...
            OtherTrainDataPtr->MaxRunningSpeed =
                MainTrainDataPtr->MaxRunningSpeed;
    ...
    if (ForwardEntryPtr->Command == "Sns") {
        if (ReverseEntryPtr->Command != "Fns") {
            SecondPassMessage(GiveMessages,
                ...
                MainHeadCode + " and is formed from a service with headcode " +
                OtherHeadCode);
            TrainDataVector.clear();
    ...
    if (ForwardEntryPtr->Command == "Fns") {
        if (ReverseEntryPtr->Command != "Sns") {
            SecondPassMessage(GiveMessages,
                "Error in timetable - unable to find a corresponding 'Sns' event for the train whose headcode is " +
                MainHeadCode + " and forms a new service with headcode " +
                OtherHeadCode);
            TrainDataVector.clear();
    ...
    if (ForwardEntryPtr->Command == "jbo") {
        if (ReverseEntryPtr->Command != "Fjo") {
            SecondPassMessage(GiveMessages,
                "Error in timetable - unable to find a corresponding 'Fjo' event for the train whose headcode is " +
                MainHeadCode + " and is joined by a train with headcode " +
                OtherHeadCode);
            TrainDataVector.clear();
    ...
    if (ForwardEntryPtr->Command == "Fjo") {
        if (ReverseEntryPtr->Command != "jbo") {
            SecondPassMessage(GiveMessages,
                "Error in timetable - unable to find a corresponding 'jbo' event for the train whose headcode is " +
                MainHeadCode + " and joins a train with headcode " +
                OtherHeadCode);
            TrainDataVector.clear();
            ...
            ForwardEntryPtr->LinkedTrainEntryPtr = OtherTrainDataPtr;
            ReverseEntryPtr->LinkedTrainEntryPtr = MainTrainDataPtr;
            if ((MainTrainDataPtr->MaxRunningSpeed > 5) &&
                (MainTrainDataPtr->MaxRunningSpeed <
                OtherTrainDataPtr->MaxRunningSpeed))
                OtherTrainDataPtr->MaxRunningSpeed =
                    MainTrainDataPtr->MaxRunningSpeed;
        ...
        // (ForwardEntryPtr->Command == "Sns-sh") || (ForwardEntryPtr->Command == "Snt-sh"))
    {
        ...
        {
            SecondPassMessage(GiveMessages,
                "Error in timetable - incorrect shuttle link to train whose headcode is " +
                MainHeadCode + " from train whose headcode is " +
                OtherHeadCode + ", has to be Fns-sh, Frh-sh");
            TrainDataVector.clear();
        ...
        // (ReverseEntryPtr->Command == "Sns-sh") || (ReverseEntryPtr->Command == "Snt-sh"))
    {
            ...
            // (ForwardEntryPtr->Command != "Fns-sh") && (ForwardEntryPtr->Command != "Frh-sh"))
        {
            SecondPassMessage(GiveMessages,
                "Error in timetable - incorrect shuttle link to train whose headcode is " +
                OtherHeadCode + " from train whose headcode is " +
                MainHeadCode + ", has to be Fns-sh, Frh-sh");
            TrainDataVector.clear();
    ...
    if (MainTrainDataPtr->ActionVector.at(MainTrainDataPtr->ActionVector.size()
        - 1).FormatType == Repeat) {
        MainRepeat = true;
        MainRepeatEntry = MainTrainDataPtr->ActionVector.at
            (MainTrainDataPtr->ActionVector.size() - 1);
    }
    if (OtherTrainDataPtr->ActionVector.at(OtherTrainDataPtr->ActionVector.size
        () - 1).FormatType == Repeat) {
        OtherRepeat = true;
        OtherRepeatEntry = OtherTrainDataPtr->ActionVector.at
            (OtherTrainDataPtr->ActionVector.size() - 1);
    }
    if ((MainRepeat && !OtherRepeat) || (!MainRepeat && OtherRepeat)) {
        SecondPassMessage(GiveMessages,
            "Error in timetable - only one repeat is provided for the train whose headcode is " +
            MainHeadCode + " and the associated train with headcode " +
            OtherHeadCode);
        TrainDataVector.clear();
    ...
    if (MainRepeat && OtherRepeat) {
        if ((MainRepeatEntry.EventTime != OtherRepeatEntry.EventTime) ||
            (MainRepeatEntry.RearStartOrRepeatMins !=
            OtherRepeatEntry.RearStartOrRepeatMins) ||
            (MainRepeatEntry.FrontStartOrRepeatDigits !=
            OtherRepeatEntry.FrontStartOrRepeatDigits) ||
            (MainRepeatEntry.NumberOfRepeats !=
            OtherRepeatEntry.NumberOfRepeats)) {
            SecondPassMessage(GiveMessages,
                "Error in timetable - repeat items don't correspond for the train whose headcode is " +
                MainHeadCode + " and the associated train with headcode " +
                OtherHeadCode);
            TrainDataVector.clear();
    ...
    // strip both leading and trailing spaces at ends of text and spaces before and after all commas and semicolons within the text
{
    ...
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",StripSpaces," + AnsiString(Input));
    if (Input == "") {
...
bool TTrainController::IsSNTEntryLocated(int Caller,
    const TTrainDataEntry &TDEntry, AnsiString &LocationName)
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",IsSNTEntryLocated," +
        AnsiString(TDEntry.HeadCode));
    const TActionVectorEntry &AVEntry0 = TDEntry.ActionVector.at(0);
    LocationName = "";
    if ((AVEntry0.Command != "Snt") && (AVEntry0.Command != "Snt-sh")) {
        throw Exception
    ...
    if (Track->TrackElementAt(506, AVEntry0.RearStartOrRepeatMins)
        .TrackType == Continuation) {
        Utilities->CallLogPop(852);
        return false;
    }
    AnsiString LocRear = Track->TrackElementAt(507,
        AVEntry0.RearStartOrRepeatMins).ActiveTrackElementName;
    AnsiString LocFront = Track->TrackElementAt(508,
        AVEntry0.FrontStartOrRepeatDigits).ActiveTrackElementName;
    if (LocRear != "")
    ...
    if (((AVEntry1.Command == "Frh") || (AVEntry1.Command == "Fjo")) &&
        (AVEntry0.Command == "Snt")) // added Fjo at v2.0.0 for empty stock
    {
...
bool TTrainController::CheckStartPositionValidity(int Caller,
    AnsiString RearElementStr, AnsiString FrontElementStr, bool GiveMessages) {
    ...
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CheckStartPositionValidity," + RearElementStr +
        "," + FrontElementStr);
    int RearPosition = 0, FrontPosition = 0, RearExitPos = 0;
    RearPosition = Track->GetTrackVectorPositionFromString(5, RearElementStr,
        GiveMessages);
    if (RearPosition < 0)
    ...
    FrontPosition = Track->GetTrackVectorPositionFromString(6, FrontElementStr,
        GiveMessages);
    if (FrontPosition < 0)
        // error message given in GetTrackVectorPositionFromString
    {
    ...
    TTrackType RearType = RearTrackElement.TrackType, FrontType =
        FrontTrackElement.TrackType;
    ...
    for (int x = 0; x < 4; x++) {
        ...
        if (x == 3) {
            TimetableMessage(GiveMessages,
                "Front element: " + FrontTrackElement.ElementID +
                " not linked to rear element: " + RearTrackElement.ElementID);
    ...
    if (FrontType == Continuation) {
        TimetableMessage(GiveMessages,
            "Front of train attempting to start on a continuation at: " +
            FrontElementStr);
    ...
    if (Track->IsLCAtHV(43, FrontTrackElement.HLoc, FrontTrackElement.VLoc)) {
        TimetableMessage(GiveMessages,
            "Train attempting to start on a level crossing at: " +
            FrontElementStr);
    ...
    if ((RearType == Points) && (RearExitPos == 3)) {
        TimetableMessage(GiveMessages,
            "Front of train attempting to start on element connected to diverging points at: " +
            RearElementStr);
        Utilities->CallLogPop(936);
        return false;
    }

    if ((FrontType == Points) && (RearTrackElement.ConnLinkPos[RearExitPos]
        == 3)) {
        TimetableMessage(GiveMessages,
            "Rear of train attempting to start on element connected to diverging points at: " +
            FrontElementStr);
...
bool TTrainController::CheckStartAllowable(int Caller, int RearPosition,
    int RearExitPos, AnsiString HeadCode, bool ReportFlag,
    TActionEventType &EventType)
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CheckStartAllowable," + AnsiString(RearPosition)
    ...
    if (AllRoutes->IsElementInLockedRouteGetPrefDirElementGetLockedVectorNumber
        (12, FrontPosition, FrontEntryPos, PrefDirElement, LockedVectorNumber))
    {
        if (ReportFlag) {
    ...
    if (AllRoutes->IsElementInLockedRouteGetPrefDirElementGetLockedVectorNumber
        (13, RearPosition, RearExitPos, PrefDirElement, LockedVectorNumber)) {
        if (ReportFlag) {
            if (EventType == FailCreateTrain)
                EventType = FailCreateLockedRoute;
            else
    ...
    if (RearType == Bridge) {
        if ((RearExitPos > 1) &&
            (RearTrackElement.TrainIDOnBridgeTrackPos23 > -1)) {
            if (ReportFlag) {
        ...
        if ((RearExitPos < 2) &&
            (RearTrackElement.TrainIDOnBridgeTrackPos01 > -1)) {
            if (ReportFlag) {
    ...
    if (FrontType == Bridge) {
        if ((FrontEntryPos > 1) &&
            (FrontTrackElement.TrainIDOnBridgeTrackPos23 > -1)) {
            if (ReportFlag) {
                if (EventType == FailCreateTrain)
        ...
        if ((FrontEntryPos < 2) &&
            (FrontTrackElement.TrainIDOnBridgeTrackPos01 > -1)) {
            if (ReportFlag) {
                if (EventType == FailCreateTrain)
...
AnsiString TTrainController::GetRepeatHeadCode(int Caller,
    AnsiString BaseHeadCode, int RepeatNumber, int IncDigits) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",GetRepeatHeadCode," + BaseHeadCode + "," +
        AnsiString(RepeatNumber) + "," + AnsiString(IncDigits));
    ...
    AnsiString NextRepeatHeadCode = BaseHeadCode.SubString(1, 2) +
        NextRepeatDigitsStr;
...
TDateTime TTrainController::GetRepeatTime(int Caller, TDateTime BasicTime,
    int RepeatNumber, int IncMinutes) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",GetRepeatTime," + AnsiString(double(BasicTime)) +
        "," + AnsiString(RepeatNumber) + "," + AnsiString(IncMinutes));
    TDateTime NextRepeatTime =
        BasicTime + TDateTime(((double)(RepeatNumber * IncMinutes))
        / 1440.0); // 1440 = no. of minutes in 24h
...
bool TTrainController::CheckShuttleRepeatTime(int Caller,
    TDateTime ForwardEventTime, TDateTime ReverseEventTime, int RepeatMinutes)
    // For success the ForwardEventTime + repeat time should == ReverseEventTime (allow 10secs either way since converting to doubles)
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CheckShuttleRepeatTime," +
        AnsiString(double(ForwardEventTime)) + "," +
    ...
    if ((ForwardSecs > (ReverseSecs - RepeatSecs + 10)) ||
        (ForwardSecs < (ReverseSecs - RepeatSecs - 10))) {
...
bool TTrainController::CheckNonRepeatingShuttleLinksAndSetData(int Caller,
    AnsiString MainHeadCode, AnsiString NonRepeatingHeadCode, bool GiveMessages)
    // check for proper non-repeating link cross references and that they have no repeats & that times are consistent
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CheckNonRepeatingShuttleLinksAndSetData," +
        MainHeadCode + "," + NonRepeatingHeadCode);
    int ForwardCount = 0;
    int ReverseCount = 0;
    unsigned int ForwardTDVectorNumber, ReverseTDVectorNumber;
    TActionVectorEntry *ReverseEntryPtr = 0, *ForwardEntryPtr = 0;
    ...
    if (ForwardCount > 1) {
        SecondPassMessage(GiveMessages,
            "Error in timetable - found more than one reference to " +
            NonRepeatingHeadCode + " from a train whose headcode is " +
            MainHeadCode);
        TrainDataVector.clear();
    ...
    if (ReverseCount == 0) {
        SecondPassMessage(GiveMessages,
            "Error in timetable - cross reference missing in either " +
            MainHeadCode + " or " + NonRepeatingHeadCode);
        TrainDataVector.clear();
    ...
    if (ReverseCount > 1) {
        SecondPassMessage(GiveMessages,
            "Error in timetable - found more than one reference to " +
            MainHeadCode + " from a train whose headcode is " +
            NonRepeatingHeadCode);
        TrainDataVector.clear();
    ...
    if (((ForwardEntryPtr->Command == "F-nshs") ||
        (ForwardEntryPtr->Command == "Sns-fsh")) &&
        (MainTrainDataPtr->ActionVector.back().FormatType == Repeat)) {
        SecondPassMessage(GiveMessages,
    ...
    if ((ForwardEntryPtr->Command != "F-nshs") &&
        ...
        (MainTrainDataPtr->ActionVector.back().FormatType != Repeat)) {
        SecondPassMessage(GiveMessages, "Error in timetable - shuttle train " +
            MainHeadCode + " does not have a repeat item");
        TrainDataVector.clear();
    ...
    if (ForwardEntryPtr->LocationName == "") {
        SecondPassMessage(GiveMessages,
            "Error in timetable - location error in cross referenced trains " +
            MainHeadCode + " and " + NonRepeatingHeadCode +
            ".  One or other service does not have a location set");
        TrainDataVector.clear();
    ...
    if (ForwardEntryPtr->LocationName != ReverseEntryPtr->LocationName) {
        SecondPassMessage(GiveMessages,
            "Error in timetable - cross referenced train " +
            NonRepeatingHeadCode +
            " is at a different location to the referencing train " +
            MainHeadCode);
        TrainDataVector.clear();
        ...
        // i.e. the non repeating link into the shuttle service
    {
        if (ForwardEntryPtr->EventTime != ReverseEntryPtr->EventTime) {
            SecondPassMessage(GiveMessages,
                ...
                " finish time not consistent with start time of shuttle service " +
                NonRepeatingHeadCode);
            TrainDataVector.clear();
    ...
    {
        if (!CheckNonRepeatingShuttleLinkTime(0, ForwardEntryPtr->EventTime,
            ReverseEntryPtr->EventTime,
            MainTrainDataPtr->ActionVector.back().RearStartOrRepeatMins,
        ...
        // i.e. a non repeating link to or from the shuttle service
    {
        if (ReverseTDVectorNumber == ForwardTDVectorNumber) {
            SecondPassMessage(GiveMessages,
                "Error in timetable - the non repeating link service " +
                NonRepeatingHeadCode +
                " appears in the same sequence as the corresponding shuttle service " +
                MainHeadCode);
            TrainDataVector.clear();
    ...
    if (ForwardEntryPtr->Command == "Sns-sh") {
        if (ReverseEntryPtr->Command != "F-nshs") {
            SecondPassMessage(GiveMessages,
                ...
                MainHeadCode +
                " and is a new shuttle service formed from the service with headcode " +
                NonRepeatingHeadCode);
            TrainDataVector.clear();
    ...
    if (ForwardEntryPtr->Command == "F-nshs") {
        if (ReverseEntryPtr->Command != "Sns-sh") {
            SecondPassMessage(GiveMessages,
                ...
                MainHeadCode +
                " and forms a new shuttle service with headcode " +
                NonRepeatingHeadCode);
            TrainDataVector.clear();
    ...
    if (ForwardEntryPtr->Command == "Sns-fsh") {
        if (ReverseEntryPtr->Command != "Fns-sh") {
            SecondPassMessage(GiveMessages,
                "Error in timetable - unable to find a corresponding 'Fns-sh' event for the 'Sns-fsh' non-shuttle service whose headcode is " +
                MainHeadCode + " formed from a shuttle service with headcode " +
                NonRepeatingHeadCode);
            TrainDataVector.clear();
    ...
    if (ForwardEntryPtr->Command == "Fns-sh") {
        if (ReverseEntryPtr->Command != "Sns-fsh") {
            SecondPassMessage(GiveMessages,
                "Error in timetable - unable to find a corresponding 'Sns-fsh' event for the 'Fns-sh' shuttle service whose headcode is " +
                MainHeadCode +
                " and forms a new non-shuttle service with headcode " +
                NonRepeatingHeadCode);
            TrainDataVector.clear();
            ...
            ForwardEntryPtr->NonRepeatingShuttleLinkEntryPtr =
                OtherTrainDataPtr;
...
bool TTrainController::CheckNonRepeatingShuttleLinkTime(int Caller,
    TDateTime ForwardEventTime, TDateTime ReverseEventTime, int RepeatMinutes,
    int RepeatNumber)
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CheckNonRepeatingShuttleLinkTime," +
        AnsiString(double(ForwardEventTime)) + "," +
        AnsiString(double(ReverseEventTime)) + "," + AnsiString(RepeatMinutes) +
    ...
    if ((ReverseSecs > (ForwardSecs + RepeatSecs + 10)) ||
        (ReverseSecs < (ForwardSecs + RepeatSecs - 10))) {
...
bool TTrainController::CheckShuttleServiceIntegrity(int Caller,
    TTrainDataEntry *TDEntryPtr, bool GiveMessages)
    // check that each shuttle start ends either in Fns or Fxx-sh (though a single service can't end in Fxx-sh), and that
    // when the Fxx-sh is reached it references the original start and not another shuttle - not allowed to link two shuttles,
    // don't ever need to and as designed would skip repeats.
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CheckShuttleServiceIntegrity," +
        AnsiString(TDEntryPtr->HeadCode));
    if (TDEntryPtr->ActionVector.back().FormatType != Repeat) {
        throw Exception("Error - last entry in " + TDEntryPtr->HeadCode +
    ...
    AnsiString LastActionCommand = (TDEntryPtr->ActionVector.end() - 2)
        ->Command;
    if ((LastActionCommand != "Fns") && (LastActionCommand != "Fns-sh") &&
        (LastActionCommand != "Frh-sh")) {
        SecondPassMessage(GiveMessages,
            "Error in timetable - last event in shuttle service " +
            TDEntryPtr->HeadCode + " is not 'Fns', 'Fns-sh' or 'Frh-sh'");
        TrainDataVector.clear();
    ...
    while (LastActionCommand == "Fns") {
        ...
        if ((LastActionCommand != "Fns") && (LastActionCommand != "Fns-sh") &&
            (LastActionCommand != "Frh-sh")) {
            SecondPassMessage(GiveMessages,
                "Error in timetable - last event in a continuation shuttle service (i.e links back to a shuttle) whose headcode is " +
                TDEntryPtr->HeadCode + " is not 'Fns', 'Fns-sh' or 'Frh-sh'");
            TrainDataVector.clear();
    ...
    if ((TDEntryPtr->ActionVector.end() - 2)
        ->LinkedTrainEntryPtr != ShuttleStartAddress) {
        SecondPassMessage(GiveMessages,
            "Error in timetable - the event that ends service " +
            TDEntryPtr->HeadCode +
            " is a shuttle finish, but it doesn't link back to the start of the original shuttle starting service " +
            OriginalHeadCode +
...
void TTrainController::LogActionError(int Caller, AnsiString HeadCode,
    AnsiString OtherHeadCode, TActionEventType ActionEventType,
    AnsiString LocationID)
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",LogActionError," + HeadCode + "," +
        OtherHeadCode + "," + AnsiString(ActionEventType) + "," + LocationID);
...
void TTrainController::SaveTrainDataVectorToFile(int Caller) // test
{
     ...
     */
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",SaveTrainDataVectorToFile");
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        OutFile << TDEntry.HeadCode.c_str() << ',' << TDEntry.Description.c_str
            () << ',' << TDEntry.StartSpeed << ',' << TDEntry.MaxRunningSpeed <<
            ',' << TDEntry.NumberOfTrains << '\n' << '\n';

        OutFile << ',' << "FormatType" << ',' << "EventTime" << ',' <<
            "ArrivalTime" << ',' << "DepartureTime" << ',' << "LocationName" <<
            ',' << "Command" << ',' << "OtherHeadCode" << ',' <<
            "LinkedTrainEntryPtr" << ',' << "RearStartOrRepeatMins" << ',' <<
            "FrontStartOrRepeatDigits" << ',' << "RepeatNumber" << '\n' << '\n';
        for (unsigned int y = 0;
        y < TrainDataVector.at(x).ActionVector.size(); y++) {
            TActionVectorEntry &AVEntry =
                TrainDataVector.at(x).ActionVector.at(y);
            ...
            OutFile << ',' << TimetableEntryTypeStr.c_str()
                << ',' << Utilities->Format96HHMM(AVEntry.EventTime).c_str()
                << ',' << Utilities->Format96HHMM(AVEntry.ArrivalTime).c_str()
                << ',' << Utilities->Format96HHMM(AVEntry.DepartureTime).c_str()
                << ',' << AVEntry.LocationName.c_str()
                << ',' << AVEntry.Command.c_str()
                << ',' << AVEntry.OtherHeadCode.c_str()
                << ',' << AVEntry.LinkedTrainEntryPtr << ',' <<
                AVEntry.RearStartOrRepeatMins << ',' <<
                AVEntry.FrontStartOrRepeatDigits << ',' <<
                AVEntry.NumberOfRepeats << '\n';
        }
        OutFile << '\n';
        OutFile << ',' << ',' << "Mass" << ',' << "MaxBrakeRate" << ',' <<
            "PowerAtRail" << ',' << "TrainID" << ',' << "RunningEntry" <<
            '\n' << '\n';
        for (unsigned int y = 0;
        y < TrainDataVector.at(x).TrainOperatingDataVector.size(); y++) {
            TTrainOperatingData TOD =
                TrainDataVector.at(x).TrainOperatingDataVector.at(y);
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",StopTTClockMessage," + Message);
    StopTTClockFlag = true;
    // so TTClock stopped during MasterClockTimer function
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",SaveSessionTrains");
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",LoadSessionTrains");
    int NumberOfTrains = Utilities->LoadFileInt(SessionFile);
    TTrain *NewTrain = new TTrain(1, 0, 0, "", 0, 1, 0, 0, 0, (TTrainMode)0, 0,
        0, 0, 0, 0); // have to have >0 for mass, else have divide
    // by zero error in calculating AValue, use 1
    for (int x = 0; x < NumberOfTrains; x++) {
        *NewTrain = TTrain(2, 0, 0, "", 0, 1, 0, 0, 0, (TTrainMode)0, 0, 0,
            0, 0, 0); // have to have >0 for mass, else have divide
        // by zero error in calculating AValue, use 1
...
bool TTrainController::CheckSessionTrains(int Caller, std::ifstream &InFile) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CheckSessionTrains");
    int NumberOfTrains;
...
void TTrainController::SaveSessionLockedRoutes(int Caller,
    std::ofstream &SessionFile) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",SaveSessionLockedRoutes");
    ...
    for (unsigned int x = 0; x < AllRoutes->LockedRouteVector.size(); x++) {
        Utilities->SaveFileInt(SessionFile,
            AllRoutes->LockedRouteVector.at(x).RouteNumber);
        Utilities->SaveFileInt(SessionFile,
            AllRoutes->LockedRouteVector.at(x).TruncateTrackVectorPosition);
        Utilities->SaveFileInt(SessionFile,
            AllRoutes->LockedRouteVector.at(x).LastTrackVectorPosition);
        Utilities->SaveFileInt(SessionFile,
            AllRoutes->LockedRouteVector.at(x).LastXLinkPos);
        Utilities->SaveFileDouble(SessionFile,
            double(AllRoutes->LockedRouteVector.at(x).LockStartTime));
    }
...
void TTrainController::LoadSessionLockedRoutes(int Caller,
    std::ifstream &SessionFile) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",LoadSessionLockedRoutes");
    ...
    for (int x = 0; x < LockedRouteVectorSize; x++) {
        ...
        LockedRouteObject.TruncateTrackVectorPosition =
            Utilities->LoadFileInt(SessionFile);
        LockedRouteObject.LastTrackVectorPosition =
            Utilities->LoadFileInt(SessionFile);
...
bool TTrainController::CheckSessionLockedRoutes(int Caller,
    std::ifstream &SessionFile) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CheckSessionLockedRoutes");
    int LockedRouteVectorSize;
    if (!Utilities->CheckAndReadFileInt(SessionFile, 0, 10000,
        LockedRouteVectorSize)) {
...
void TTrainController::SaveSessionContinuationAutoSigEntries(int Caller,
    std::ofstream &SessionFile) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",SaveSessionContinuationAutoSigEntries");
    Utilities->SaveFileInt(SessionFile, ContinuationAutoSigVector.size());
    for (unsigned int x = 0; x < ContinuationAutoSigVector.size(); x++) {
        Utilities->SaveFileInt(SessionFile,
            ContinuationAutoSigVector.at(x).RouteNumber);
        Utilities->SaveFileInt(SessionFile,
            ContinuationAutoSigVector.at(x).AccessNumber);
        Utilities->SaveFileDouble(SessionFile,
            ContinuationAutoSigVector.at(x).FirstDelay);
        Utilities->SaveFileDouble(SessionFile,
            ContinuationAutoSigVector.at(x).SecondDelay);
        Utilities->SaveFileDouble(SessionFile,
            ContinuationAutoSigVector.at(x).ThirdDelay);
        Utilities->SaveFileDouble(SessionFile,
            double(ContinuationAutoSigVector.at(x).PassoutTime));
    }
...
void TTrainController::LoadSessionContinuationAutoSigEntries(int Caller,
    std::ifstream &SessionFile) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",LoadSessionContinuationAutoSigEntries");
    ...
    for (int x = 0; x < ContinuationAutoSigVectorSize; x++) {
        ContinuationAutoSigObject.RouteNumber =
            Utilities->LoadFileInt(SessionFile);
        ContinuationAutoSigObject.AccessNumber =
            Utilities->LoadFileInt(SessionFile);
        ContinuationAutoSigObject.FirstDelay =
            Utilities->LoadFileDouble(SessionFile);
        ContinuationAutoSigObject.SecondDelay =
            Utilities->LoadFileDouble(SessionFile);
        ContinuationAutoSigObject.ThirdDelay =
            Utilities->LoadFileDouble(SessionFile);
...
bool TTrainController::CheckSessionContinuationAutoSigEntries(int Caller,
    std::ifstream &SessionFile) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CheckSessionContinuationAutoSigEntries");
    int ContinuationAutoSigVectorSize;
    if (!Utilities->CheckAndReadFileInt(SessionFile, 0, 10000,
        ContinuationAutoSigVectorSize)) {
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",BuildContinuationTrainExpectationMultiMap");
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        const TActionVectorEntry &AVLastEntry =
            TDEntry.ActionVector.at(TDEntry.ActionVector.size() - 1);
            ...
            // new train (no need to include Snt-sh since they can't start at a continuation)
        {
            ...
            if (Track->TrackElementAt(665, AVFirstEntry.RearStartOrRepeatMins)
                .TrackType == Continuation) {
                ...
                if (AVLastEntry.FormatType == Repeat) {
                    CTEEntry.IncrementalMinutes =
                        AVLastEntry.RearStartOrRepeatMins;
                    // retains this value or 0 for all repeats
                    CTEEntry.IncrementalDigits =
                        AVLastEntry.FrontStartOrRepeatDigits;
                    // retains this value or 0 for all repeats
                }
                ...
                if (TDEntry.NumberOfTrains > 1) {
                    ...
                    for (int y = 1; y < TDEntry.NumberOfTrains; y++) {
                        ...
                        CTEEntry.HeadCode =
                            GetRepeatHeadCode(23, TDEntry.HeadCode, y,
                            AVLastEntry.FrontStartOrRepeatDigits);
                        // CTEEntry.VectorPosition stays same
                        CTEMMP.first = GetRepeatTime(3, AVFirstEntry.EventTime,
                            y, AVLastEntry.RearStartOrRepeatMins);
                        CTEMMP.second = CTEEntry;
                        ContinuationTrainExpectationMultiMap.insert(CTEMMP);
                    }
                }
...
void TTrainController::PlotAllTrainsInZoomOutMode(int Caller, bool Flash) {
    ...
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",PlotTrainsInZoomOutMode");
    if (!Display->ZoomOutFlag) {
...
TTrain &TTrainController::TrainVectorAt(int Caller, int VecPos) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",TrainVectorAt," + AnsiString(VecPos));
    if ((VecPos < 0) || (VecPos >= (int)TrainVector.size())) {
        throw Exception("Out of Range Error, vector size: " +
            AnsiString(TrainVector.size()) + ", VecPos: " + AnsiString(VecPos) +
            " in TrainVectorAt");
    }
...
void TTrainController::CreateFormattedTimetable(int Caller,
    AnsiString RailwayTitle, AnsiString TimetableTitle, AnsiString CurDir) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CreateFormattedTimetable");
    AnsiString RetStr = "", PartStr = "";
    ...
    AnsiString TTFileName = TDateTime::CurrentDateTime().FormatString
        ("dd-mm-yyyy hh.nn.ss");
    // format "16/06/2009 20:55:17"
    // avoid characters in filename:=   / \ : * ? " < > |
    TTFileName = CurDir + "\\Formatted timetables\\Timetable " + TTFileName +
        "; " + RailwayTitle + "; " + TimetableTitle + ".csv";

    AnsiString ShortTTName = "";
    for (int x = TTFileName.Length(); x > 0; x--) {
        if (TTFileName[x] == '\\') {
            ShortTTName = TTFileName.SubString(x + 1,
                TTFileName.Length() - x - 4);
            break;
        }
    }
    ...
    AnsiString TableTitle = "", TimetableTimeStr = "", MassStr = "", PowerStr =
        "", BrakeStr = "", MaxSpeedStr = "", FirstHeadCode = "", Header = "";
    TimetableTimeStr = Utilities->Format96HHMM
        (TrainController->TimetableStartTime);
    TableTitle = "Railway: " + RailwayTitle + "; Timetable: " + TimetableTitle +
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        if (TrainDataEntry.Mass > 0)
            MassStr = "; Mass " + AnsiString::FormatFloat(FormatNoDPStr,
            ((double)TrainDataEntry.Mass) / 1000) + "Te; ";
        if (TrainDataEntry.PowerAtRail > 0)
            PowerStr = "Power " + AnsiString::FormatFloat(FormatNoDPStr,
            TrainDataEntry.PowerAtRail / 1000 / 0.8) + "kW; ";
        if (TrainDataEntry.MaxBrakeRate > 0)
            BrakeStr = "Brake force " + AnsiString::FormatFloat(FormatNoDPStr,
            (TrainDataEntry.MaxBrakeRate * TrainDataEntry.Mass / 9810)) +
                "Te; ";
        if (TrainDataEntry.MaxRunningSpeed > 0)
            MaxSpeedStr = "Maximum speed " + AnsiString::FormatFloat
                (FormatNoDPStr, TrainDataEntry.MaxRunningSpeed) + " km/h";
        ...
        if (!ActionVector.empty()) {
            if (ActionVector.at(ActionVector.size() - 1).FormatType == Repeat) {
                IncDigits = ActionVector.at(ActionVector.size() - 1)
                    .FrontStartOrRepeatDigits;
                IncMinutes = ActionVector.at(ActionVector.size() - 1)
                    .RearStartOrRepeatMins;
            }
        }
        ...
        for (int y = 0; y < TrainDataEntry.NumberOfTrains; y++) {
            ...
            if ((TrainDataEntry.Description != "") && (MassStr != ""))
                OneTTLine.Header = TrainDataEntry.Description + MassStr +
                    PowerStr + BrakeStr + MaxSpeedStr;
            ...
            for (unsigned int z = 0; z < ActionVector.size(); z++) {
                ...
                OneTTTrain.HeadCode = GetRepeatHeadCode(24, FirstHeadCode, y,
                    IncDigits);
                ...
                if (ActionVectorEntry.SequenceType == Start) {
                    if (ActionVectorEntry.FormatType == StartNew) {
                        if (ActionVectorEntry.LocationName != "") {
                            if (Track->TrackElementAt(742,
                                ActionVectorEntry.RearStartOrRepeatMins)
                                .TrackType == Continuation) {
                                PartStr =
                                    "Enters at " +
                                    ActionVectorEntry.LocationName;
                            }
                            else {
                                PartStr =
                                    "Created at " +
                            ...
                            AnsiString LocName =
                                Track->TrackElementAt(739,
                                ActionVectorEntry.RearStartOrRepeatMins)
                                .ActiveTrackElementName;
                            if (Track->TrackElementAt(740,
                                ActionVectorEntry.RearStartOrRepeatMins)
                                .TrackType == Continuation) {
                                if (LocName != "") {
                                    ...
                                    PartStr =
                                        "Enters at " + Track->TrackElementAt
                                        (737, ActionVectorEntry.
                                        RearStartOrRepeatMins).ElementID;
                                }
                            }
                            else // not a continuation
                            {
                                    ...
                                    // but include anyway
                                {
                                    ...
                                    PartStr =
                                        "Created at " + Track->TrackElementAt
                                        (741, ActionVectorEntry.
                                        RearStartOrRepeatMins).ElementID;
                                }
                            }
                        }
                        TimeStr =
                            Utilities->Format96HHMM
                            (GetRepeatTime(20, ActionVectorEntry.EventTime, y,
                            IncMinutes));
                    }
                        ...
                        {
                            PartStr =
                                "Enters at " + ActionVectorEntry.LocationName;
                            TimeStr =
                                Utilities->Format96HHMM
                                (GetRepeatTime(21, ActionVectorEntry.EventTime,
                                y, IncMinutes));
                        }
                        else {
                            PartStr = "Repeat shuttle service at " +
                                ActionVectorEntry.LocationName + " from ";
                            TimeStr =
                                GetRepeatHeadCode(45,
                                ActionVectorEntry.OtherHeadCode, y - 1,
                                IncDigits) + " at " + Utilities->Format96HHMM
                                (GetRepeatTime(26, ActionVectorEntry.EventTime,
                                y, IncMinutes));
                        } // y-1 for headcode above since it is the last repeat value that the train is from
                    }
                    else if (ActionVectorEntry.Command == "Sfs") {
                        PartStr = "New service at " +
                            ActionVectorEntry.LocationName + " split from";
                        TimeStr =
                            GetRepeatHeadCode(33,
                            ActionVectorEntry.OtherHeadCode, y, IncDigits) +
                            " at " + Utilities->Format96HHMM
                            (GetRepeatTime(24, ActionVectorEntry.EventTime, y,
                            IncMinutes));
                    }
                        ...
                        PartStr = "New service at " +
                            ActionVectorEntry.LocationName + " from";
                        TimeStr =
                            GetRepeatHeadCode(34,
                            ActionVectorEntry.OtherHeadCode, y, IncDigits) +
                            " at " + Utilities->Format96HHMM
                            (GetRepeatTime(25, ActionVectorEntry.EventTime, y,
                            IncMinutes));
                    }
                        ...
                        if (y == 0) // first entry from shuttle
                        {
                            ...
                            TimeStr =
                                ActionVectorEntry.
                                NonRepeatingShuttleLinkHeadCode + " at " +
                                Utilities->Format96HHMM
                                (GetRepeatTime(27, ActionVectorEntry.EventTime,
                                y, IncMinutes));
                        }
                        else {
                            PartStr = "Repeat shuttle service at " +
                                ActionVectorEntry.LocationName + " from ";
                            TimeStr =
                                GetRepeatHeadCode(35,
                                ActionVectorEntry.OtherHeadCode, y - 1,
                                IncDigits) + " at " + Utilities->Format96HHMM
                                (GetRepeatTime(22, ActionVectorEntry.EventTime,
                                y, IncMinutes));
                        } // y-1 for headcode above since it is the last repeat value that the train is from
                    }
                    else if (ActionVectorEntry.FormatType ==
                        SNSNonRepeatFromShuttle) {
                        PartStr = "New service at " +
                            ActionVectorEntry.LocationName + " from";
                        // need repeat for the non-repeating headcode as it's the last train of the repeating shuttle
                        TTrainDataEntry *TDE =
                            ActionVectorEntry.LinkedTrainEntryPtr;
                        ...
                        int IncrementalDigits =
                            TDE->ActionVector.at(TDE->ActionVector.size() - 1)
                            .FrontStartOrRepeatDigits;
                        TimeStr = GetRepeatHeadCode(36, FirstHeadCode,
                            LastRepeatNumber, IncrementalDigits) + " at " +
                            Utilities->Format96HHMM
                            (GetRepeatTime(23, ActionVectorEntry.EventTime, y,
                            IncMinutes));
                    }
                }
                    ...
                    if (ActionVectorEntry.FormatType == TimeTimeLoc) {
                        ...
                        if (ActionVectorEntry.DepartureTime !=
                            ActionVectorEntry.ArrivalTime) {
                            PartStr =
                                "Arrives at " + ActionVectorEntry.LocationName;
                            TimeStr =
                                Utilities->Format96HHMM
                                (GetRepeatTime(4, ActionVectorEntry.ArrivalTime,
                                y, IncMinutes));
                            OneTTEntry.Action = PartStr;
                            OneTTEntry.Time = TimeStr;
                            OneTTTrain.OneFormattedTrainVector.push_back
                                (OneTTEntry);
                            PartStr =
                                "Departs from " +
                                ActionVectorEntry.LocationName;
                            TimeStr =
                                Utilities->Format96HHMM
                                (GetRepeatTime(5,
                                ActionVectorEntry.DepartureTime, y,
                                IncMinutes));
                        }
                        else {
                            PartStr = "Arrives & departs " +
                                ActionVectorEntry.LocationName;
                            TimeStr =
                                Utilities->Format96HHMM
                                (GetRepeatTime(29,
                                ActionVectorEntry.ArrivalTime, y, IncMinutes));
                        }
                    }
                    else if ((ActionVectorEntry.FormatType == TimeLoc) &&
                        (ActionVectorEntry.ArrivalTime != TDateTime(-1))) {
                        PartStr =
                            "Arrives at " + ActionVectorEntry.LocationName;
                        TimeStr =
                            Utilities->Format96HHMM
                            (GetRepeatTime(6, ActionVectorEntry.ArrivalTime, y,
                            IncMinutes));
                    }
                    else if ((ActionVectorEntry.FormatType == TimeLoc) &&
                        (ActionVectorEntry.ArrivalTime == TDateTime(-1))) {
                        PartStr =
                            "Departs from " + ActionVectorEntry.LocationName;
                        TimeStr =
                            Utilities->Format96HHMM
                            (GetRepeatTime(7, ActionVectorEntry.DepartureTime,
                            y, IncMinutes));
                    }
                        ...
                        TimeStr =
                            Utilities->Format96HHMM
                            (GetRepeatTime(8, ActionVectorEntry.EventTime, y,
                            IncMinutes));
                    }
                    else if (ActionVectorEntry.Command == "jbo") {
                        PartStr =
                            "Joined at " +
                            ActionVectorEntry.LocationName + " by";
                        TimeStr =
                            GetRepeatHeadCode(37,
                            ActionVectorEntry.OtherHeadCode, y, IncDigits) +
                            " at " + Utilities->Format96HHMM
                            (GetRepeatTime(9, ActionVectorEntry.EventTime, y,
                            IncMinutes));
                    }
                    else if (ActionVectorEntry.Command == "fsp") {
                        PartStr = "Splits from front at " +
                            ActionVectorEntry.LocationName + " to form";
                        TimeStr =
                            GetRepeatHeadCode(38,
                            ActionVectorEntry.OtherHeadCode, y, IncDigits) +
                            " at " + Utilities->Format96HHMM
                            (GetRepeatTime(10, ActionVectorEntry.EventTime, y,
                            IncMinutes));
                    }
                    else if (ActionVectorEntry.Command == "rsp") {
                        PartStr = "Splits from rear at " +
                            ActionVectorEntry.LocationName + " to form";
                        TimeStr =
                            GetRepeatHeadCode(39,
                            ActionVectorEntry.OtherHeadCode, y, IncDigits) +
                            " at " + Utilities->Format96HHMM
                            (GetRepeatTime(11, ActionVectorEntry.EventTime, y,
                            IncMinutes));
                    }
                    else if (ActionVectorEntry.Command == "cdt") {
                        PartStr = "Changes direction at " +
                            ActionVectorEntry.LocationName;
                        TimeStr =
                            Utilities->Format96HHMM
                            (GetRepeatTime(12, ActionVectorEntry.EventTime, y,
                            IncMinutes));
                    }
                }
                else if (ActionVectorEntry.SequenceType == Finish) {
                    if (ActionVectorEntry.Command == "Fns") {
                        PartStr = "At " + ActionVectorEntry.LocationName +
                            " forms new service";
                        TimeStr =
                            GetRepeatHeadCode(40,
                            ActionVectorEntry.OtherHeadCode, y, IncDigits) +
                            " at " + Utilities->Format96HHMM
                            (GetRepeatTime(13, ActionVectorEntry.EventTime, y,
                            IncMinutes));
                    }
                        ...
                        TimeStr =
                            ActionVectorEntry.NonRepeatingShuttleLinkHeadCode +
                            " at " + Utilities->Format96HHMM
                            (GetRepeatTime(17, ActionVectorEntry.EventTime, y,
                            IncMinutes));
                    }
                    else if ((ActionVectorEntry.Command == "Fns-sh") && (y <
                        (TrainDataEntry.NumberOfTrains - 1))) {
                        PartStr = "At " + ActionVectorEntry.LocationName +
                            " forms new service ";
                        TimeStr =
                            GetRepeatHeadCode(41,
                            ActionVectorEntry.OtherHeadCode, y + 1, IncDigits) +
                            " at " + Utilities->Format96HHMM
                            (GetRepeatTime(14, ActionVectorEntry.EventTime, y,
                            IncMinutes));
                        ...
                        TimeStr =
                            ActionVectorEntry.NonRepeatingShuttleLinkHeadCode +
                            " at " + Utilities->Format96HHMM
                            (GetRepeatTime(15, ActionVectorEntry.EventTime, y,
                            IncMinutes));
                    }
                    else if ((ActionVectorEntry.Command == "Frh-sh") && (y <
                        (TrainDataEntry.NumberOfTrains - 1))) {
                        PartStr = "At " + ActionVectorEntry.LocationName +
                            " forms new service";
                        TimeStr =
                            GetRepeatHeadCode(43,
                            ActionVectorEntry.OtherHeadCode, y + 1, IncDigits) +
                            " at " + Utilities->Format96HHMM
                            (GetRepeatTime(16, ActionVectorEntry.EventTime, y,
                            IncMinutes));
                        // y+1 because it's the NEXT service repeat number that is relevant
                    }
                    else if ((ActionVectorEntry.Command == "Frh-sh") && (y >=
                        (TrainDataEntry.NumberOfTrains - 1))) {
                        PartStr = "Terminates shuttle service at " +
                            ActionVectorEntry.LocationName;
                        // only used in chronological tt
                        TimeStr = "End at " + Utilities->Format96HHMM
                            (GetRepeatTime(28, ActionVectorEntry.EventTime, y,
                            IncMinutes));
                        ...
                        PartStr =
                            "Terminates at " + ActionVectorEntry.LocationName;
                        ...
                        {
                            if (ActionVector.at(z - 1)
                                .EventTime != TDateTime(-1)) {
                                TimeStr =
                                    Utilities->Format96HHMM
                                    (GetRepeatTime(30,
                                    ActionVector.at(z - 1).EventTime, y,
                                    IncMinutes));
                            }
                            else if (ActionVector.at(z - 1)
                                .ArrivalTime != TDateTime(-1)) {
                                TimeStr =
                                    Utilities->Format96HHMM
                                    (GetRepeatTime(31,
                                    ActionVector.at(z - 1).ArrivalTime, y,
                                    IncMinutes));
                            }
                            else
                        ...
                        PartStr = "Exits railway" + GetExitLocationAndAt(0,
                            ActionVectorEntry.ExitList);
                        TimeStr =
                            Utilities->Format96HHMM
                            (GetRepeatTime(18, ActionVectorEntry.EventTime, y,
                            IncMinutes));
                    }
                        ...
                        PartStr = "At " + ActionVectorEntry.LocationName +
                            " joins";
                        TimeStr =
                            GetRepeatHeadCode(44,
                            ActionVectorEntry.OtherHeadCode, y, IncDigits) +
                            " at " + Utilities->Format96HHMM
                            (GetRepeatTime(19, ActionVectorEntry.EventTime, y,
                            IncMinutes));
                    }
                }
                else if (ActionVectorEntry.SequenceType ==
    ...
    for (unsigned int x = 0; x < AllTTTrains->size(); x++) {
        ...
        for (int y = 0; y < AllTTTrains->at(x).NumberOfTrains;
        y++) // number of repeating trains
        {
            if (y < (AllTTTrains->at(x).NumberOfTrains - 1)) {
                TTFile << AllTTTrains->at(x).OneCompleteFormattedTrainVector.at
                    (y).HeadCode.c_str() << ',';
            }
            else {
                TTFile << AllTTTrains->at(x).OneCompleteFormattedTrainVector.at
        ...
        z < AllTTTrains->at(x).OneCompleteFormattedTrainVector.at(0)
            .OneFormattedTrainVector.size(); z++) {
            TTFile << AllTTTrains->at(x).OneCompleteFormattedTrainVector.at(0)
                .OneFormattedTrainVector.at(z).Action.c_str() << ',';
            for (int y = 0; y < AllTTTrains->at(x).NumberOfTrains;
            y++) // number of repeating trains
            {
                if (y < (AllTTTrains->at(x).NumberOfTrains - 1)) {
                    TTFile << AllTTTrains->at(x)
                        .OneCompleteFormattedTrainVector.at(y)
                        .OneFormattedTrainVector.at(z).Time.c_str() << ',';
                }
                else {
                    TTFile << AllTTTrains->at(x)
                        .OneCompleteFormattedTrainVector.at(y)
    ...
    AnsiString TTFileName2 = TDateTime::CurrentDateTime().FormatString
        ("dd-mm-yyyy hh.nn.ss");
    TTFileName2 = CurDir + "\\Formatted timetables\\Timetable " + TTFileName2 +
        "; " + RailwayTitle + "; " + TimetableTitle + ".txt";

    std::ofstream TTFile2(TTFileName2.c_str());
    if (TTFile2 == 0) {
        StopTTClockMessage(67,
    ...
    for (unsigned int x = 0; x < AllTTTrains->size(); x++) {
        ...
        {
            for (unsigned int z = 0;
            z < AllTTTrains->at(x).OneCompleteFormattedTrainVector.at(y)
                .OneFormattedTrainVector.size(); z++) {
                bool GiveMessagesFalse = false;
                AnsiString TimeString =
                    AllTTTrains->at(x).OneCompleteFormattedTrainVector.at(y)
                    .OneFormattedTrainVector.at(z).Time;
                AnsiString HeadCodeString =
                    AllTTTrains->at(x).OneCompleteFormattedTrainVector.at(y)
                    .HeadCode;
                AnsiString ActionString =
                    AllTTTrains->at(x).OneCompleteFormattedTrainVector.at(y)
                    .OneFormattedTrainVector.at(z).Action;
                if (CheckHeadCodeValidity(11, GiveMessagesFalse,
                    TimeString.SubString(1, 4)))
                ...
                AnsiString OneLine = TimeString + ' ' + HeadCodeString + ' ' +
                    ActionString + '\n';
...
AnsiString TTrainController::GetExitLocationAndAt(int Caller,
    TExitList &ExitList) const {
    // check all timetable names in list, if all same return " at [name]" else return ""
    if (ExitList.empty())
        return "";
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",GetExitLocationAndAt");
    AnsiString StartName = Track->TrackElementAt(735, *(ExitList.begin()))
        .ActiveTrackElementName;
    if (StartName == "") {
        if (ExitList.size() == 1) {
            AnsiString ID = Track->TrackElementAt(738, *(ExitList.begin()))
                .ElementID;
    ...
    for (TExitListIterator ELIT = ExitList.begin(); ELIT != ExitList.end();
    ELIT++) {
        if (Track->TrackElementAt(736,
            *ELIT).ActiveTrackElementName != StartName) {
...
void TTrainController::SendPerformanceSummary(int Caller,
    std::ofstream &PerfFile) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",SendPerformanceSummary");
    ...
    if (EarlyArrivals > 0)
        AvEarlyArrMins =
            FormatFloat(FormatStr, (TotEarlyArrMins / EarlyArrivals));
    if (LatePasses > 0)
        AvLatePassMins = FormatFloat(FormatStr, (TotLatePassMins / LatePasses));
    if (EarlyPasses > 0)
        AvEarlyPassMins =
            FormatFloat(FormatStr, (TotEarlyPassMins / EarlyPasses));
    if (LateDeps > 0)
    ...
    if (LateArrivals > 1)
        PerfFile << LateArrivals << " late arrivals (average " <<
            AvLateArrMins.c_str() << " min)" << '\n';
    else if (LateArrivals == 1)
        PerfFile << LateArrivals << " late arrival (" << AvLateArrMins.c_str()
            << " min)" << '\n';
    else
        PerfFile << LateArrivals << " late arrivals" << '\n';

    if (EarlyArrivals > 1)
        PerfFile << EarlyArrivals << " early arrivals (average " <<
            AvEarlyArrMins.c_str() << " min)" << '\n';
    else if (EarlyArrivals == 1)
        PerfFile << EarlyArrivals << " early arrival (" << AvEarlyArrMins.c_str
            () << " min)" << '\n';
    else
    ...
    if (LatePasses > 1)
        PerfFile << LatePasses << " late passes (average " <<
            AvLatePassMins.c_str() << " min)" << '\n';
    else if (LatePasses == 1)
        PerfFile << LatePasses << " late pass (" << AvLatePassMins.c_str()
            << " min)" << '\n';
    else
        PerfFile << LatePasses << " late passes" << '\n';

    if (EarlyPasses > 1)
        PerfFile << EarlyPasses << " early passes (average " <<
            AvEarlyPassMins.c_str() << " min)" << '\n';
    else if (EarlyPasses == 1)
        PerfFile << EarlyPasses << " early pass (" << AvEarlyPassMins.c_str()
            << " min)" << '\n';
    else
    ...
    if (LateDeps > 1)
        PerfFile << LateDeps << " late departures (average " <<
            AvLateDepMins.c_str() << " min)" << '\n';
    else if (LateDeps == 1)
        PerfFile << LateDeps << " late departure (" << AvLateDepMins.c_str()
            << " min)" << '\n';
    else
        PerfFile << LateDeps << " late departures" << '\n';

    AnsiString FormattedExcessLCDownMins =
        FormatFloat(FormatStr, ExcessLCDownMins);
    if (ExcessLCDownMins > 0.1)
        PerfFile << FormattedExcessLCDownMins.c_str()
    ...
    int LocsNotReached = (NotStartedTrainArrDep + OperatingTrainArrDep) / 2;
    // each location has an arrival and departure (generally) so divide by 2
    if (LocsNotReached > 0) {
        AvLateMinsLocsNotReached =
            FormatFloat(FormatStr,
            (OperatingTrainLateMins + NotStartedTrainLateMins) /
            (NotStartedTrainArrDep + OperatingTrainArrDep));
        PerfFile << LocsNotReached <<
            " locations that trains failed to reach (average lateness " <<
            AvLateMinsLocsNotReached.c_str() << " min)" << '\n';
    }
    ...
    TotArrDep = OnTimeArrivals + LateArrivals + EarlyArrivals + OnTimeDeps +
        LateDeps + NotStartedTrainArrDep + OperatingTrainArrDep;
    ...
    if (OverallScorePercent == 100) {
        if (TotArrDep > 0) {
            TotLateMinsFactor =
                exp((-0.1732) * (TotLateArrMins + TotLateDepMins +
                OperatingTrainLateMins + NotStartedTrainLateMins +
                ((OtherMissedEvents + UnexpectedExits + ExcessLCDownMins) * 15))
                / TotArrDep);
            ...
            MissedStopAndSPADRiskFactor =
                exp((-17.33) * (MissedStops + SPADRisks + IncorrectExits)
                / TotArrDep);
    ...
    if ((TotArrDep > 0) || DerailSPADFlag || CrashFlag)
        // flag condits added at v1.1.4 - see above for what the error was
    {
        ...
        if(NumFailures == 1) AddedString = OneFailureString;
        if(NumFailures > 1) AddedString = TwoOrMoreFailureString;
...
void TTrainController::SetWarningFlags(int Caller) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",SetWarningFlags");
    for (unsigned int x = 0; x < TrainVector.size(); x++) {
...
void TTrainController::CalcOperatingAndNotStartedTrainLateness(int Caller) {
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",CalcSignalStopLateness");
    ...
    for (unsigned int x = 0; x < TrainVector.size(); x++) {
        ...
        for (TActionVectorEntry * AVEntryPtr =
            &Train.TrainDataEntryPtr->ActionVector.front();
        AVEntryPtr < &Train.TrainDataEntryPtr->ActionVector.back();
        AVEntryPtr++) {
            if (AVEntryPtr < Train.ActionVectorEntryPtr)
                continue;
            if ((AVEntryPtr->ArrivalTime > TDateTime(-1)) && (GetRepeatTime(42,
                AVEntryPtr->ArrivalTime, Train.RepeatNumber,
                Train.IncrementalMinutes) < TTClockTime)) {
                OperatingTrainLateMins +=
                    1440 * double(TTClockTime - GetRepeatTime(43,
                    AVEntryPtr->ArrivalTime, Train.RepeatNumber,
                    Train.IncrementalMinutes));
                OperatingTrainArrDep++;
            }
            if ((AVEntryPtr->DepartureTime > TDateTime(-1)) &&
                (GetRepeatTime(44, AVEntryPtr->DepartureTime,
                Train.RepeatNumber, Train.IncrementalMinutes) < TTClockTime)) {
                OperatingTrainLateMins +=
                    1440 * double(TTClockTime - GetRepeatTime(45,
                    AVEntryPtr->DepartureTime, Train.RepeatNumber,
                    Train.IncrementalMinutes));
                OperatingTrainArrDep++;
            }
        }
    ...
    for (unsigned int x = 0; x < TrainDataVector.size(); x++) {
        ...
        for (int y = 0; y < TDEntry.NumberOfTrains; y++) {
            ...
            a < TrainController->TrainVector.size(); a++) {
                if ((TrainController->TrainVector.at(a)
                    .TrainDataEntryPtr == &TDEntry) &&
                    (TrainController->TrainVector.at(a).RepeatNumber == y)) {
            ...
            if (GetRepeatTime(46, TDEntry.ActionVector.at(0).EventTime, y,
                IncrementalMinutes) > TTClockTime) {
            ...
            for (unsigned int z = 0; z < TDEntry.ActionVector.size(); z++) {
                ...
                if (GetRepeatTime(35, AVEntry.EventTime, y, IncrementalMinutes)
                    > TTClockTime) {
                    break; // all the rest will also be greater (& default of -1 will be less)
                }
                if (GetRepeatTime(36, AVEntry.ArrivalTime, y,
                    IncrementalMinutes) > TTClockTime) {
                    break; // all the rest will also be greater (& default of -1 will be less)
                }
                if (GetRepeatTime(37, AVEntry.DepartureTime, y,
                    IncrementalMinutes) > TTClockTime) {
                    break; // all the rest will also be greater (& default of -1 will be less)
                }
                if ((AVEntry.ArrivalTime > TDateTime(-1)) && (GetRepeatTime(38,
                    AVEntry.ArrivalTime, y, IncrementalMinutes) < TTClockTime))
                {
                    NotStartedTrainLateMins +=
                        1440 * double(TTClockTime - GetRepeatTime(39,
                        AVEntry.ArrivalTime, y, IncrementalMinutes));
                    NotStartedTrainArrDep++;
                }
                if ((AVEntry.DepartureTime > TDateTime(-1)) &&
                    (GetRepeatTime(40, AVEntry.DepartureTime, y,
                    IncrementalMinutes) < TTClockTime)) {
                    NotStartedTrainLateMins +=
                        1440 * double(TTClockTime - GetRepeatTime(41,
                        AVEntry.DepartureTime, y, IncrementalMinutes));
                    NotStartedTrainArrDep++;
                }
            }
        }
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",RebuildOpTimeToActMultimap");
        ...
        // build OpTimeToActMultiMap entries for expected trains
    {
        ...
        TContinuationTrainExpectationMultiMapIterator CTEIt =
            ContinuationTrainExpectationMultiMap.begin();
        while (CTEIt != TrainController->ContinuationTrainExpectationMultiMap.
            end()) {
            LaterTrain = false;
            if (CTEIt->second.TrainDataEntryPtr->TrainOperatingDataVector.at
                (CTEIt->second.RepeatNumber).RunningEntry != NotStarted) {
                CTEIt++;
                continue; // not interested in running or exited trains
            }
            if (Track->TrackElementAt(934, CTEIt->second.VectorPosition)
                .TrainIDOnElement > 0) {
                CTEIt++;
                continue;
            ...
            if (!ContinuationEntryVecPosVector.empty()) {
                ...
                x < ContinuationEntryVecPosVector.size(); x++) {
                    if (CTEIt->second.VectorPosition ==
                        ContinuationEntryVecPosVector.at(x)) {
                        LaterTrain = true; ;
            ...
            ContinuationEntryVecPosVector.push_back
                (CTEIt->second.VectorPosition);
            ...
            DistanceToRedSignal = CalcDistanceToRedSignalandStopTime(1,
                CTEIt->second.VectorPosition, 0,
                // EntryPos always 0 for entering at a continuation
                SigControlAndCanPassRedSignal,
                &CTEIt->second.TrainDataEntryPtr->ActionVector.at(1),
                // at(1) to skip past the Start train value
                HeadCode, TrainID, CurrentStopTime, LaterStopTime,
                RecoverableTime, AvTrackSpeed);
            ...
            if (DistanceToRedSignal == -1) {
                ...
                int Speed = AvTrackSpeed;
                int MaxSpeed =
                    int(CTEIt->second.TrainDataEntryPtr->MaxRunningSpeed);
                if (AvTrackSpeed > MaxSpeed)
                    Speed = MaxSpeed;
                if (CTEIt->second.TrainDataEntryPtr->ActionVector.at(1)
                    .SignallerControl)
                ...
                TimeToAct =
                    LaterStopTime + DistanceToRedSignal * 3.6 / 60 / Speed;
                // accel & decel taken into account in
                // CalcDistanceToRedSignalandStopTime
                // 3.6 convertsKm/h to m/s & 60 converts seconds to minutes
                // don't need CurrentStopTime or RecoverableTime for continuation entries
                float MinsBefEnter =
                    double(CTEIt->first - TTClockTime) * 86400.0 / 60.0;
...
int TTrainController::CalcDistanceToRedSignalandStopTime(int Caller,
    int TrackVectorPosition, int TrackVectorPositionEntryPos,
    bool SigControlAndCanPassRedSignal, TActionVectorEntry *AVPtr,
    AnsiString HeadCode, int TrainID, float &CurrentStopTime,
    float &LaterStopTime, float &RecoverableTime, int &AvTrackSpeed)
    // new v2.2.0
...
{
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," +
        AnsiString(Caller) + ",DistanceToRedSignal, " +
        AnsiString(TrackVectorPosition) + ", " +
    ...
    TConfiguration CurrentExitConfig =
        Track->TrackElementAt(937, CurrentElement).Config[CurrentExitPos];
        ...
        // -1 for a continuation and can't be at a location as not yet entered
    {
        ...
        if (Train.StoppedAtLocation) {
            ...
            else if (!((Train.ActionVectorEntryPtr->FormatType == TimeTimeLoc)
                || (Train.ActionVectorEntryPtr->FormatType == TimeLoc))) {
            ...
            {
                double TimeToDepart =
                    double(Train.ReleaseTime - TrainController->TTClockTime)
                    * 86400 / 60; // mins to depart
    ...
    {
        int NextElement = Track->TrackElementAt(939, CurrentElement)
            .Conn[CurrentExitPos];
        int NextEntryPos = Track->TrackElementAt(940, CurrentElement)
            .ConnLinkPos[CurrentExitPos];
        int RouteNumber; // holder for referenced value, not used
        if (AllRoutes->GetRouteTypeAndNumber(33, NextElement, NextEntryPos,
            RouteNumber) == TAllRoutes::AutoSigsRoute) {
            ...
            // ignore signal and increment CurrentElement to NextElement
        {
            ...
            CurrentExitConfig = Track->TrackElementAt(943, CurrentElement)
                .Config[CurrentExitPos];
            CurrentAttribute = Track->TrackElementAt(944, CurrentElement)
                .Attribute;
        }
        else if ((TrainID > -1) && (TrainVectorAtIdent(40,
            TrainID).TrainMode == Timetable)) // ignore signallercontrol or will
        ...
        // not red signal next (in fwd direction) so enter loop to calc CumLength
    {
        ...
        if (CurrentEntryPos > 1) {
            DistanceToRedSignal += Track->TrackElementAt(916, CurrentElement)
                .Length23;
            CumTrackSpeed += Track->TrackElementAt(945, CurrentElement)
                .SpeedLimit23;
        }
        else {
            DistanceToRedSignal += Track->TrackElementAt(917, CurrentElement)
                .Length01;
            CumTrackSpeed += Track->TrackElementAt(946, CurrentElement)
                .SpeedLimit01;
        }
        TrackSpeedCount++;
        ...
        if (Track->TrackElementAt(948, CurrentElement)
            .ActiveTrackElementName != "") {
            double StopTimeDouble;
            while (AVPtr->FormatType == PassTime)
                AVPtr++; // skip past any passes
            if ((Track->TrackElementAt(949,
                CurrentElement).ActiveTrackElementName == AVPtr->LocationName)
                && ((AVPtr->FormatType == TimeLoc) ||
                ...
                // stop due here so calc dwell time & advance Ptr
            {
                if (AVPtr->FormatType == TimeTimeLoc) {
                    LaterStopNumber++;
                    StopTimeDouble =
                        double(AVPtr->DepartureTime - AVPtr->ArrivalTime)
                        * 86400.0 / 60.0;
                    ...
                    if ((LaterStopNumber == 1) && (TrainID > -1))
                        TrainVectorAtIdent(41, TrainID)
                            .FirstLaterStopRecoverableTime = RecoverableTime;
                    AVPtr++;
                }
                else if ((AVPtr->FormatType == TimeLoc) &&
                    (AVPtr->ArrivalTime != TDateTime(-1))) // must be an arrival
                {
                        ...
                        // must be a departure
                    {
                        LaterStopNumber++;
                        StopTimeDouble =
                            double((AVPtr + 1)->DepartureTime -
                            AVPtr->ArrivalTime) * 86400.0 / 60.0;
                        // can't convert a TDateTime to a float directly
                        if (StopTimeDouble < 0.5)
                            StopTimeDouble = 0.5;
                        // at least 30 secs delay at station
                        ...
                        RecoverableTime += StopTimeDouble - 0.5;
                        if ((LaterStopNumber == 1) && (TrainID > -1))
                            TrainVectorAtIdent(42, TrainID)
                                .FirstLaterStopRecoverableTime =
                                RecoverableTime;
                        AVPtr++;
                        AVPtr++;
                    }
        ...
        NextElement = Track->TrackElementAt(950, CurrentElement)
            .Conn[CurrentExitPos];
        ...
        NextEntryPos = Track->TrackElementAt(919, CurrentElement)
            .ConnLinkPos[CurrentExitPos];
        // get NextExitPos
        ...
        CurrentEntryPos = NextEntryPos;
        CurrentExitPos = NextExitPos;
        CurrentExitConfig = Track->TrackElementAt(922, CurrentElement)
            .Config[CurrentExitPos];
    ...
    {
        int NextElement = Track->TrackElementAt(924, CurrentElement)
            .Conn[CurrentExitPos];
        int NextEntryPos = Track->TrackElementAt(925, CurrentElement)
            .ConnLinkPos[CurrentExitPos];
        int RouteNumber; // holder for referenced value, not used
        if (AllRoutes->GetRouteTypeAndNumber(31, NextElement, NextEntryPos,
            RouteNumber) == TAllRoutes::AutoSigsRoute) {
    ...
    {
        if (CurrentEntryPos > 1) {
            MaxAllowableSpeed = Track->TrackElementAt(951, CurrentElement)
                .SpeedLimit23;
        }
        else {
            MaxAllowableSpeed = Track->TrackElementAt(952, CurrentElement)
                .SpeedLimit01;
        }
    ...
    if (LaterStopNumber > 0) {
        KmPerLocationStop = float(DistanceToRedSignal)
            / LaterStopNumber / 1000; // m to km
