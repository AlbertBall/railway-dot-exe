...
//inline functions
TTextItem() : TextString(""), HPos(0), VPos(0) {Font = new TFont;} //default constructor
TTextItem(int H, int V, AnsiString T, TFont *InputFont) //contructor, sets the values given
    : TextString(T), HPos(H), VPos(V) {Font = new TFont; Font->Assign(InputFont);}
...
//inline functions
void SetNewHPos(int Caller, int TextItem, int NewHPos) {TextPtrAt(24, TextItem)->HPos = NewHPos;} //change the value of HPos to NewHPos for
    //the text item at position TextItem in TextVector
void SetNewVPos(int Caller, int TextItem, int NewVPos) {TextPtrAt(25, TextItem)->VPos = NewVPos;} //change the value of VPos to NewVPos for
    //the text item at position TextItem in TextVector
