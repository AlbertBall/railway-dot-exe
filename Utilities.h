...
public:
    ...
    AnsiString DateTimeStamp(); //creates a string of the form 'dd/mm/yyyy hh:mm:ss' for use in call & event logging
...
//void LogEvent(AnsiString Str); //store Str to the event log - moved to TTrainController for v0.6 so can record the tt clock value
    ...
    //(inclusive), returns true for success
    ...
    bool CheckAndCompareFileString(std::ifstream &InFile, AnsiString InString); //checks that the value is a string ('0' or CR accepted as
